...
{	
   ...
   // FROM mymbook) b ON where a.mbno = b.mbno and a.rbn = '$runningbillno' and a.sheetid = '$abstsheetid'";
	...
	$select_mymbook_query = mysqli_query($dbConn,$select_mymbook_sql);
	//echo $select_mymbook_sql."<br/>";
	if(mysqli_num_rows($select_mymbook_query)>0)
	{
		while($MBKList = mysqli_fetch_object($select_mymbook_query))
		{
			...
			if($maxpage == 100)
			{
				...
				$update_mbookpage_query_2 = mysqli_query($dbConn,$update_mbookpage_sql_2);
			...
			$update_mbookpage_query = mysqli_query($dbConn,$update_mbookpage_sql);
		}
	}
	//exit;
	...
	if($SubdividSlmStr != "")
	{
		...
		for($x7=0; $x7<count($explodeAbstractStr); $x7+=8)
		{
			...
			if(!in_array($Subdivid_dmy, $explodeSubdividSlmStr))
			{
				...
				$insert_mbook_dummy_query 	= 	mysqli_query($dbConn,$insert_mbook_dummy_sql);
	...
	$abstmbno 				= 	$_POST['txt_abstmbno'];
										...
										//$insert_mybmook_query_3 = 	mysqli_query($dbConn,$insert_mybmook_sql_3);
	/*$update_asb_maxpage 	= 	"update mbookallotment set mbpage = '$max_page_abs' WHERE allotmentid	= '$abs_mbno_id' AND sheetid = '$abstsheetid'";
	$update_asb_maxpage_sql = 	mysqli_query($dbConn,$update_asb_maxpage);
	$oldmbook_query 		= 	"SELECT * from oldmbook WHERE sheetid = '$abstsheetid'";
	$oldmbook_sql 			= 	mysqli_query($dbConn,$oldmbook_query);
	if(mysqli_num_rows($oldmbook_sql)>0)
	{
		while($res = mysqli_fetch_array($oldmbook_sql))
		{
			$mbno 								= 	$res['mbname'];
			...
			$update_mbookallot_sql 	 			= 	mysqli_query($dbConn,$update_mbookallot_query);
			...
			$update_aggreement_mbookallot_sql 	= 	mysqli_query($dbConn,$update_aggreement_mbookallot_query); 
			$oldmbook  		   				   .= 	$res['mbname']."*"; 
										...
										//$insert_mybmook_query 				= 	mysqli_query($dbConn,$insert_mybmook_sql);
		} 
	} */
	...
	//$mbookquery					=	"INSERT INTO measurementbook  (measurementbookdate, staffid, sheetid, divid, subdivid, fromdate, todate, mbno, mbnopages, mbpage, mbremainpage, mbtotalpages, mbquantity, mbtotal, abstmbookno, abstmbpage, abstquantity, absttotal, pay_percent, flag, part_pay_flag, rbn, active, userid, is_finalbill, remarks) SELECT  now(), staffid, sheetid, divid, subdivid, fromdate, todate, mbno, mbnopages, mbpage, mbremainpage, mbtotalpages, mbquantity, mbtotal, abstmbookno, abstmbpage, abstquantity, absttotal, pay_percent, flag, part_pay_flag, rbn, active, userid, is_finalbill, remarks FROM measurementbook_temp WHERE sheetid = '$abstsheetid'";// WHERE flag =1 OR flag = 2";
   	//$mbooksql 					= 	mysqli_query($dbConn,$mbookquery);  
	...
	$SelectSheetTypeSql 	= mysqli_query($dbConn,$SelectSheetTypeQuery);
	if($SelectSheetTypeSql == true){
		$STList 		= 	mysqli_fetch_object($SelectSheetTypeSql);
    	$SectionType 	= 	$STList->section_type; 
	}
	
	
	$SelectMBTempQuery = "select * FROM measurementbook_temp WHERE sheetid = '$abstsheetid'";
	$SelectMBTempSql = mysqli_query($dbConn,$SelectMBTempQuery);
	if($SelectMBTempSql == true){
		if(mysqli_num_rows($SelectMBTempSql)>0){
			while($MBTempList = mysqli_fetch_object($SelectMBTempSql)){
				...
				$InsertMBPermSql 		= mysqli_query($dbConn,$InsertMBPermQuery); 
				$MBPermId				= mysqli_insert_id($dbConn);
				...
				if($SectionType == 'III'){
					...
					$UpdateQtySplitSql 		= mysqli_query($dbConn,$UpdateQtySplitQuery);
					...
					$UpdateMBDetailSql 		= mysqli_query($dbConn,$UpdateMBDetailQuery);
    ...
    $sheetsql 					= 	mysqli_query($dbConn,$sheetquery);
	...
	$mbookpage_sql 				= 	mysqli_query($dbConn,$mbookpage_query);
	while($result3 = mysqli_fetch_array($mbookpage_sql))
	{
		$mbno 					= 	$result3['mbno'];
		$selectmaxpage_query 	= 	"select max(mbpage) from mbookgenerate WHERE sheetid	= '$abstsheetid' AND mbno ='".$result3['mbno']."'";
		$selectmaxpage_sql 		= 	mysqli_query($dbConn,$selectmaxpage_query);
		$mbookmaxpage 			= 	@mysqli_result($selectmaxpage_sql,'mbpage');
										...
										//$insert_mybmook_query_2 = 	mysqli_query($dbConn,$insert_mybmook_sql_2);
		$upademaxpage_query 	= 	"update mbookallotment set mbpage = '$mbookmaxpage' WHERE sheetid = '$abstsheetid' AND mbno ='".$result3['mbno']."'";
		$upademaxpage_sql 		= 	mysqli_query($dbConn,$upademaxpage_query);
	}*/
	...
	$update_absremark_sql = mysqli_query($dbConn,$update_absremark_query);
	
	$update_accounts_query = "update send_accounts_and_civil set mb_ac = '', sa_ac = '', ab_ac = '' where sheetid = '$abstsheetid'";// and rbn = ''";
	$update_accounts_sql = mysqli_query($dbConn,$update_accounts_query);
	
	$update_escalation_query = "update escalation set flag = 1 where sheetid = '$abstsheetid'";// and rbn = ''";
	$update_escalation_sql = mysqli_query($dbConn,$update_escalation_query);
	
	$PODate = $_SESSION['PassOrderDate'];
	$PassOrderAmt = $_SESSION['PassOrderAmt'];
	$PassOrderCrBy = $_SESSION['PassOrderCrBy'];
	$PassOrderLevel = $_SESSION['PassOrderLevel'];
	
	$update_podate_query = "update abstractbook set pass_order_date = '$PODate', pass_order_dt = '$PODate', rab_status = 'C', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$PassOrderCrBy', pass_order_cr_level = '$PassOrderLevel' where sheetid = '$abstsheetid' and rbn = '$runningbillno'";// and rbn = ''";
	$update_podate_sql = mysqli_query($dbConn,$update_podate_query);
	
	$UpdateQuery1 	= "UPDATE memo_payment_accounts_edit SET pass_order_dt = '$PODate', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$PassOrderCrBy', pass_order_cr_level = '$PassOrderLevel' where sheetid = '$abstsheetid' and rbn = '$runningbillno'";// and is_adv_pay != 'Y'";
	...
	header('Location: PassOrderCreate.php');
}
...
<form name="form" method="post" onsubmit="return confirm('Are you sure want to confirm pass order ?');">
<?php
...
//*************THIS PART IS FOR DEDUCT PREVIOUS MEASUREMENT ( D.P.M. ) SECTION*******************//
	...
	if(mysqli_num_rows($mbookdpmquery_sql)>0)
	{
		...
		while($DPMList = mysqli_fetch_array($mbookdpmquery_sql))
		{
			...
			{
				$dpm_mesurementbook_details .= $DPMList['subdivid']."*".$DPMList['mbtotal']."*".$DPMList['mbno']."*".$DPMList['mbpage']."*".$DPMList['divid']."*".$DPMList['abstmbookno']."*".$DPMList['abstmbpage']."*".$DPMList['pay_percent']."*".$DPMList['flag']."*".$DPMList['part_pay_flag']."*".$DPMList['remarks']."*".$DPMList['rbn']."*".$DPMList['measurementbookid']."*";
...
$description = getscheduledescription_new($subdivisionlist[$i]);
//echo "D".$description;
...
<?php
	...
	//$flagtype = @mysqli_result($mbooktype_sql,0,'flag');
...
$total_recovery = $total_recovery + $sgst_amt + $cgst_amt + $igst_amt + $sd_amt + $wct_amt + $vat_amt + $mob_adv_amt_rec + $mob_adv_int_amt + $pl_mac_adv_rec + $pl_mac_adv_int_amt + $hire_charges + $lw_cess_amt + $incometax_amt + $it_cess_amt + $it_edu_amt + $land_rent + $liquid_damage + $other_recovery_1 + $other_recovery_2 + $non_dep_machine_equip + $non_dep_man_power + $nonsubmission_qa;
...
//echo $ThisBillSecAdvAmtMop;exit;
...
echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Mobilization Advance : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($ThisBillMobAdvAmtMop, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Upto Date Secured Advance : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($UpoDtSecAdvAmtMop, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
echo "<tr style='border:none'><td style='border:none' class='labelbold' align='right' colspan='6'>GRAND TOTAL : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelbold' align='right' colspan='5'>".number_format($GrandTotal, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
...
echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Since Last Bill Value : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($OverAllSlmAmount, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
...
if($sgst_percent != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>SGST @ ".number_format($sgst_percent, 2, '.', '')."%  On Rs. ".$bill_amt_gst.": <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($sgst_amt, 2, '.', '')."</td><td style='border:none' colspan='1'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
if($cgst_percent != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>CGST @ ".number_format($cgst_percent, 2, '.', '')."%  On Rs. ".$bill_amt_gst.": <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($cgst_amt, 2, '.', '')."</td><td style='border:none' colspan='1'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
if($igst_percent != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>IGST @ ".number_format($igst_percent, 2, '.', '')."%  On Rs. ".$bill_amt_gst.": <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($igst_amt, 2, '.', '')."</td><td style='border:none' colspan='1'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
...
{
$eb = 1;
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$ec_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>Security Deposit @ ".$sd_percent."% : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($sd_amt, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
...
echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Secured Advance : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($sec_adv_amount, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";

if($mob_adv_amt != 0){
	echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Mob. Advance : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($mob_adv_amt, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
}
if($pl_mac_adv_amt != 0){
	echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>P&M Advance : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($pl_mac_adv_amt, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
}
if($esc_amt != 0){
	echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Escalation : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='5'>".number_format($esc_amt, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
}


echo "<tr style='border:none'><td style='border:none' class='labelbold' align='right' colspan='6'>GRAND TOTAL : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelbold' align='right' colspan='5'>".number_format($GrandTotal, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
echo "<tr style='border:none'><td style='border:none' class='labelprint' align='right' colspan='6'>Less Previous Payment : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelprint' align='right' colspan='3'>&nbsp;</td><td colspan='2' align='right' class='labelprint' style='border:none;'>(-)&nbsp;&nbsp;".number_format($dpm_total_amount, 2, '.', '')."</td><td style='border:none;'>&nbsp;</td></tr>";

echo "<tr style='border:none'><td style='border:none' class='labelbold' align='right' colspan='6'>Net Total : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' class='labelbold' align='right' colspan='5'>".number_format($NetTotal, 2, '.', '')."</td><td style='border:none'>&nbsp;</td></tr>";
...
{
//$eb = 1;
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>Security Deposit @ ".$sd_percent."% : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($sd_amt, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
if($mob_adv_int_amt != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>Mob. Adv. Interest  : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($mob_adv_int_amt, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
if($pl_mac_adv_int_amt != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'>".$eb_text." (".$eb.")</td><td style='border:none' class='labelprint' align='right' colspan='4'>P&M Adv. Interest : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelprint'>&nbsp;&nbsp;".number_format($pl_mac_adv_int_amt, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
$eb++; $eb_text = "";
}
...
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'></td><td style='border:none' class='labelbold' align='right' colspan='4'>Total Recovery : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelbold'>&nbsp;&nbsp;".number_format($TotalRecovery, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
}

if($ChequeAmount != 0)
{
echo "<tr style='border:none'><td style='border:none' colspan='2' align='right' class='labelprint'></td><td style='border:none' class='labelbold' align='right' colspan='4'>Net Payable Amount : <i class='fa fa-inr' style='font-weight:normal; width:4px; height:5px;'> </td><td style='border:none' colspan='5' align='right' class='labelbold'>&nbsp;&nbsp;".number_format($ChequeAmount, 2, '.', '')."</td><td colspan='1' style='border:none'>&nbsp;</td></tr>";
}
...
<!--<div align="center" class="btn_outside_sect printbutton">
	...
	<div class="btn_inside_sect">
		...
		<div style="text-align:center; height:45px; line-height:45px;" class="printbutton">
			...
			<input type="button" name="Back" value="Back" id="back" class="info-btn" onclick="goBack();" /> 
			</div>
			<div class="buttonsection">
			<input type="Submit" name="Submit" class="info-btn" value="Confirm" id="Submit" /> 
			</div>
			<!--<div class="buttonsection">
			<input type="button" class="backbutton" name="print" value=" Print " onclick="printBook();" />
			</div>-->
