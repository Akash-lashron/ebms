...
if(isset($_POST['back'])){
	header('Location: CSTccnoEntryGenerate.php');
...
if(isset($_POST['btn_saveas_draft'])){
	//$PageId 	 = $_POST['txt_page_id'];
	$TenderId = $_POST['hid_txt_tr_id'];
	$CCNum 	 = $_POST['txt_ccno'];
	if(isset($TenderId)){
		$SelectCCNOWorksQuery		= "SELECT ccno FROM works WHERE ccno = '$CCNum'";
		$SelectCCNOWorksQuerySql 	= mysqli_query($dbConn,$SelectCCNOWorksQuery);
		$SelectCCNOSheetQuery		= "SELECT computer_code_no FROM sheet WHERE computer_code_no = '$CCNum'";
		$SelectCCNOSheetQuerySql 	= mysqli_query($dbConn,$SelectCCNOSheetQuery);
		if(($SelectCCNOWorksQuerySql == true) || ($SelectCCNOSheetQuerySql == true)){
			if((mysqli_num_rows($SelectCCNOWorksQuerySql)>0)|| ($SelectCCNOSheetQuerySql == true)){
				$msg = "Sorry...CCNo : ".$CCNum." Aldready Entered, Please Enter New CCNo..!!";
			}
		}else{
			$UpdateConfirmQuery = "UPDATE tender_register SET cst_acc_status = 'C', ncst_acc_status='C', cst_acc_conf_by='$UserId', ncst_acc_conf_by= NOW(), 
			cst_acc_conf_on='$UserId', ncst_acc_conf_on= NOW() WHERE tr_id = '$TenderId'";

			$UpdateConfirmQuerySql 	= mysqli_query($dbConn,$UpdateConfirmQuery);
			header('Location: CSTccnoEntryGenerate.php');
			/*if($PageId == '2'){
				header('Location: CSTccnoEntryGenerate.php?ncsid=2');
			}else if($PageId == '3'){
				header('Location: NegotiatedComparativeStatementGenerate.php?ncsid=3');
			}*/
		}
...
if(isset($_POST['btn_confirm'])){
	...
	$GlobID= '';
	$GlobIDQuery = "SELECT globid FROM tender_register WHERE tr_id = '$TenderId'";
	$GlobIDSql 	= mysqli_query($dbConn,$GlobIDQuery);
	if($GlobIDSql == true){
		if(mysqli_num_rows($GlobIDSql)>0){
			$List = mysqli_fetch_object($GlobIDSql);
	...
	if(isset($TenderId)){
		$CcnoExist = 0;
		$SelectCCNOWorksQuery		= "SELECT ccno FROM works WHERE ccno = '$CCNum' AND globid != '$GlobID'";
		$SelectCCNOWorksQuerySql 	= mysqli_query($dbConn,$SelectCCNOWorksQuery);
		if($SelectCCNOWorksQuerySql == true){
			if(mysqli_num_rows($SelectCCNOWorksQuerySql)>0){
				$CcnoExist = 1;
			}
		}
		$SelectCCNOSheetQuery		= "SELECT computer_code_no FROM sheet WHERE computer_code_no = '$CCNum' AND globid != '$GlobID'";
		$SelectCCNOSheetQuerySql 	= mysqli_query($dbConn,$SelectCCNOSheetQuery);
		if($SelectCCNOSheetQuerySql == true){
			if(mysqli_num_rows($SelectCCNOSheetQuerySql)>0){
				$CcnoExist = 1;
			}
		}
		//echo $SelectCCNOSheetQuery;exit;

		if($CcnoExist == 1){
			$msg = "Sorry...CCNo : ".$CCNum." Already Entered, Please Enter New CCNo..!!";
		}else{
			$UpdateConfirmQuery 	= "UPDATE tender_register SET cst_acc_status = 'C', ncst_acc_status = 'C', cst_acc_conf_by = '$UserId', ncst_acc_conf_by = NOW(), 
			cst_acc_conf_on = '$UserId', ncst_acc_conf_on = NOW() WHERE globid='$GlobID' AND tr_id = '$TenderId'";
			$UpdateConfirmQuerySql = mysqli_query($dbConn,$UpdateConfirmQuery);

			$UpdateWorksQuery 	= "UPDATE works SET ccno = '$CCNum' WHERE globid='$GlobID'";
			$UpdateWorksQuerySql	= mysqli_query($dbConn,$UpdateWorksQuery);
			
			if($UpdateConfirmQuerySql == true){
				$msg = "CCNO Created Successfully..!!";
			}else{
				$msg = "Error: CCNO could not be Created..!!";
			}
			header('Location: CSTccnoEntryGenerate.php');
			/*if($PageId == '2'){
				header('Location: CSTccnoEntryGenerate.php?ncsid=2');
...
<script type="text/javascript">
                            ...
                            <div class="container" align="center">
								...
								<table class="table itemtable rtable table2excel" width="100%">
									<thead>
										<tr>
											...
											<input type="hidden" name="hid_txt_tr_id" id="hid_txt_tr_id" value = "<?php if(isset($MastId)){ echo $MastId; } ?>" class="">
											<?php 
												//$NegoCont = null;
												$DeptTotalAmount = 0;
								...
								<div style="text-align:center; height:30px; line-height:30px;" class="lboxlabel">
									...
									<div class="div4" align="left">
										<input type="text" name="txt_ccno" id="txt_ccno" class="tboxsmclass" style="width:50%" value="">
									</div>
								</div>
							</div>
	...
	<script>
		...
		$("body").on("change","#txt_ccno", function(event){
			var ccnoval = $.trim($(this).val()); //alert(stateval);
			$.ajax({
				type: 'POST',
				...
				success: function (data) {
					...
					if(data != null){
						if(data == 1){
							BootstrapDialog.alert("CCNo Already Entered..!!");
							$("#txt_ccno").val('');
						}else{
				...
				BootstrapDialog.show({
					...
					buttons: [{
						...
						action: function(dialog) {
							dialog.close();
							window.location.replace('CSTccnoEntryGenerate.php');
						}
					}]
				});
			}
