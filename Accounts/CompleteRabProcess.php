...
if(isset($_POST['btn_save']) == " SAVE & NEXT "){
	...
	if($BillMode == "ON"){
		...
		$_SESSION['PassOrderDate'] 	= $SavePoDate;
		$_SESSION['emptypage'] 		= 0;//$emptypage;
		header('Location: AbstMBook_Bill_Confirm.php?workno='.$SaveSheetId); 
	}else{
		//$UpdateQuery1 	= "UPDATE memo_payment_accounts_edit SET pass_order_dt = '$SavePoDate', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$staffid', pass_order_cr_level = '".$_SESSION['levelid']."' where sheetid = '$SaveSheetId' and rbn = '$SaveRbn'";
		$UpdateQuery1 	= "UPDATE memo_payment_accounts_edit SET pass_order_dt = '$SavePoDate', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$staffid', pass_order_cr_level = '".$_SESSION['levelid']."' where sheetid = '$SaveSheetId' and rbn = '$SaveRbn'";
		$UpdateSql1 	= mysqli_query($dbConn,$UpdateQuery1);
		//$UpdateQuery2 	= "UPDATE abstractbook SET pass_order_date = '$SavePoDate', pass_order_dt = '$SavePoDate', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$staffid', pass_order_cr_level = '".$_SESSION['levelid']."' where sheetid = '$SaveSheetId' and rbn = '$SaveRbn'";
		$UpdateQuery2 	= "UPDATE abstractbook SET pass_order_date = '$SavePoDate', pass_order_dt = '$SavePoDate', pass_order_amt = '$PassOrderAmt', pass_order_cr_by = '$staffid', pass_order_cr_level = '".$_SESSION['levelid']."' where sheetid = '$SaveSheetId' and rbn = '$SaveRbn'";
		$UpdateSql2 	= mysqli_query($dbConn,$UpdateQuery2);
		if($UpdateQuery1 == true){
			$msg = "Pass order data saved successfully";
		}else{
			$msg = "Error : Pass order data not saved. Please try again.";
		}
	}
}
?>
																	...
																	<div class="row">
																		...
																		<div class="div8" align="left">
																			<select name="cmb_shortname" id="cmb_shortname" class="tboxsmclass">
																				...
																				<?php echo $objBind->BindWorksForClose(0); ?>
																		   	</select>
																		</div>
																		<div class="div12"></div>
																		<div class="div4">&nbsp;</div>
...
$(function() {
	...
	function GetWorkDetails(){
		...
		$.ajax({ 
			...
			data: ({ WorkId: WorkId, PageType:'PSO'}), 
					...
					$.each(data, function(index, element) {
						if(element.is_adv_pay_flag == "Y"){
							...
							if(element.bill_reg_status == "N"){
								...
								BootstrapDialog.alert("Bill process is already completed. You can't close this RAB again.");
								BillErr = 1;
							}else if(element.bill_vouch_status == "Y"){
								BootstrapDialog.alert("Voucher process is already completed. You can't close this RAB again.");
								BillErr = 1;
							}else if(element.bill_payord_status != "Y"){
								BootstrapDialog.alert("Pay Order process is not yet completed. You can't close this RAB.");
								BillErr = 1;
							}else if(element.bill_pasord_status != "Y"){
								BootstrapDialog.alert("Pass Order process is not yet completed. You can't close this RAB.");
								BillErr = 1;
							}else if((element.bill_ret_status == "Y")&&(element.bill_mode == "ON")){
								BootstrapDialog.alert("Bill is returned back to EIC. You can't close this RAB.");
								BillErr = 1;
							}else if((element.bill_level_flag == "H")&&(element.bill_mode == "ON")){
								BootstrapDialog.alert("Bill is forwarded to next checking level. You can't close this RAB.");
								BillErr = 1;
							}else if((element.bill_level_flag == "L")&&(element.bill_mode == "ON")){
								BootstrapDialog.alert("Bill is still waiting in previous checking level. You can't close this RAB.");
								BillErr = 1;
							}else if((element.bill_curr_level == element.bill_level_flag)&&(element.bill_curr_level != "C")&&(element.bill_mode == "ON")){
								BootstrapDialog.alert("Bill verification not yet completed. You can't close this RAB.");
								BillErr = 1;
							}else{
								$("#txt_workorder_no").val(element.work_order_no);
								$("#txt_rbn").val(element.rbn);
