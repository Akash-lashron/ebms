...
$PGDataArr = array();
$CurrDate = date('Y-m-d');
$MasterQuery1 = "SELECT a.sheet_id, b.bfdid, b.inst_exp_date,b.inst_type, b.inst_purpose FROM sheet a INNER JOIN bg_fdr_details b ON ( a.sheet_id = b.master_id ) 
...
if($ImpTrIds != ''){
$WorkQuery = "SELECT sch_comp_date,work_ext_date,sheetid FROM works WHERE sheetid IN ($ImpTrIds) AND '$CurrDate' <= sch_comp_date + INTERVAL 8 MONTH OR  '$CurrDate' <= work_ext_date + INTERVAL 8 MONTH";
//print_r($WorkQuery ); exit;
$WorkQuerysql = mysqli_query($dbConn,$WorkQuery);
if($WorkQuerysql == true){
	if(mysqli_num_rows($WorkQuerysql)>0){
		while($List2 = mysqli_fetch_object($WorkQuerysql)){
...
if($ImpDetIds != ''){
	$PgRetMasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, d.name_contractor, e.ccno, e.sch_comp_date, e.work_ext_date, e.eic, f.staffid, f.staffname FROM sheet a 
    ...
    WHERE b.bfdid IN ($ImpDetIds) AND b.inst_type = 'BG' AND b.inst_purpose = 'SD'  AND b.inst_exp_date < '$CurrDate'
									...
									<div class="div10">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">SD-Bank Guarantee Expired Reminder</div>
																...
																<table width="100%" align="center" class="dataTable table2excel mgtb-8">
																	<thead>
																		...
																		<th rowspan="2" valign="middle">SNo.</th>
																			...
																			<th colspan="7" valign="middle">PG Detail</th>
																		</tr>
																		<tr>
