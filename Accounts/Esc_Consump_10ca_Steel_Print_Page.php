...
$escal_fromdate_sql = mysqli_query($dbConn,$escal_fromdate_query);
if($escal_fromdate_sql == true)
{
	$MinDateList = mysqli_fetch_object($escal_fromdate_sql);
...
if($_POST["send_to_civil"] == " Return to EIC ")
{
	...
	// echo $acc_remarks_count;exit;
	 ...
	 $update_query 	= "update acc_log set status = 'SC', AC_status = 'R', comment ='$acc_comment_log', staffid = '$staffid_acc', levelid = '".$_SESSION['levelid']."',
					  ...
					  where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'SC' and genlevel = 'stl_consum'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 
	 $update_query 	= "update send_accounts_and_civil set mb_ac = 'SC', accounts_comment ='$acc_comment_log', locked_status = '', level_status = 'F', acc_staffid = '$staffid_acc' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'SC' and genlevel = 'stl_consum'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 if($update_sql == true)
	 {
		...
		$RABTransActDetStr = "Steel Consumption (Escalation) MBook - ".$sc_mbook_no." rejected to Civil in ".$RABTranFWRoleName." Level";
...
if($_POST["accept"] == " Accept MBook ")
{
	...
	//echo $update_query;exit;
	 ...
	 if($update_sql == true)
	 {
		...
		$_SESSION['lock'] = "";
		$RABTranFWRoleName = GetRoleName($_SESSION['levelid'],$_SESSION['staff_section']);
		$RABTransActDetStr = "Steel Consumption (Escalation) MBook - ".$sc_mbook_no." verified and final level accepted in ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
...
if(isset($_POST["forward"])){
	 ...
	 $update_query 	= "update send_accounts_and_civil set locked_status = '', acc_staffid = '".$_SESSION['sid_acc']."' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'SC' and genlevel = 'stl_consum'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 ...
	 if($RejCnt == 0){
		...
		$update_query = "update acc_log set 
		AC_status = '',  
		levelid = '$fw_level' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'SC' and genlevel = 'stl_consum'";
	 //echo $update_query; exit;
	 	$update_sql = mysqli_query($dbConn,$update_query);
	 ...
	 if($update_sql == true)
	 {
		...
		$_SESSION['lock'] = "";
		$RABTranFWRoleName = GetRoleName($_SESSION['levelid'],$_SESSION['staff_section']);
		$RABTransActDetStr = "Steel Consumption (Escalation) MBook - ".$sc_mbook_no." accepted in ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
...
if(isset($_POST["backward"])){
	 ...
	 if($update_sql == true)
	 {
		...
		$RABTransActDetStr = "Steel Consumption (Escalation) MBook - ".$sc_mbook_no." returned back to ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
	...
	if($date_wise_data != "")
	{
		...
		for($i=0; $i<$count; $i++)
		{
			...
			$insert_cement_consum_query = "insert into esc_consumption_10ca set 
											...
											modifieddate = NOW(),
											active = '1'";
			$insert_cement_consum_sql = mysqli_query($dbConn,$insert_cement_consum_query);
							...
							if(mysqli_num_rows($escal_measure_sql)>0)
							{
								...
								while($MList = mysqli_fetch_object($escal_measure_sql))
								{
								 ...
								 else
								 {
									...
									$month			 = date("F",$month_ts); //echo $MList->mdate."<br/>";
...
{ 
	...
	$sc_mbookno 	= $newmbookno;
						...
						$staff_roleid 		= $exp_staff_level_str[0];
						$staff_levelid 		= $exp_staff_level_str[1];
						$AccVerification 	= AccVerificationCheck($sheetid,$esc_rbn,$sc_mbookno,'staff',$staff_levelid,'MB');
						$AlStatusRes 		= AccountsLevelStatus($sheetid,$esc_rbn,$sc_mbookno,0,'SC','stl_consum');//($sheetid,$rbn);
						...
						?>
							...
							<input type="hidden" name="txt_mbook_no" id="txt_mbook_no" value="<?php echo $sc_mbookno; ?>"/>
