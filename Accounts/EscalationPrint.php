...
function GetTCAconsumMbookNo($sheetid,$esc_rbn,$esc_id,$itemcode,$month,$type)
{
	...
	$select_tca_cons_mb_sql 	= 	mysqli_query($dbConn,$select_tca_cons_mb_query);
	if($select_tca_cons_mb_sql == true)
	{
		if(mysqli_num_rows($select_tca_cons_mb_sql)>0)
		{
			$MBList = mysqli_fetch_object($select_tca_cons_mb_sql);
...
if($_POST["send_to_civil"] == " Return to EIC ")
{
	...
	// echo $acc_remarks_count;exit;
	 ...
	 $update_query 	= "update acc_log set status = 'SC', AC_status = 'R', comment ='$acc_comment_log', staffid = '$staffid_acc', levelid = '".$_SESSION['levelid']."',
					  ...
					  where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'E' and genlevel = 'escalation'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 
	 $update_query 	= "update send_accounts_and_civil set mb_ac = 'SC', accounts_comment ='$acc_comment_log', locked_status = '', level_status = 'F', acc_staffid = '$staffid_acc' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'E' and genlevel = 'escalation'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 if($update_sql == true)
	 {
		...
		$RABTransActDetStr = "Escalation MBook - ".$sc_mbook_no." rejected to Civil in ".$RABTranFWRoleName." Level";
...
if($_POST["accept"] == " Accept MBook ")
{
	...
	//echo $update_query;exit;
	 ...
	 if($update_sql == true)
	 {
		...
		$_SESSION['lock'] = "";
		$RABTranFWRoleName = GetRoleName($_SESSION['levelid'],$_SESSION['staff_section']);
		$RABTransActDetStr = "Escalation MBook - ".$sc_mbook_no." verified and final level accepted in ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
...
if(isset($_POST["forward"])){
	 ...
	 $update_query 	= "update send_accounts_and_civil set locked_status = '', acc_staffid = '".$_SESSION['sid_acc']."' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'E' and genlevel = 'escalation'";
	 $update_sql 	= mysqli_query($dbConn,$update_query);
	 ...
	 if($RejCnt == 0){
		...
		$update_query = "update acc_log set 
		AC_status = '',  
		levelid = '$fw_level' where sheetid = '$sc_sheetid' and rbn = '$sc_rbnno' and  zone_id = '$sc_zone_id' and mtype = 'E' and genlevel = 'escalation'";
	 //echo $update_query; exit;
	 	$update_sql = mysqli_query($dbConn,$update_query);
	 ...
	 if($update_sql == true)
	 {
		...
		$_SESSION['lock'] = "";
		$RABTranFWRoleName = GetRoleName($_SESSION['levelid'],$_SESSION['staff_section']);
		$RABTransActDetStr = "Escalation MBook - ".$sc_mbook_no." accepted in ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
...
if(isset($_POST["backward"])){
	 ...
	 if($update_sql == true)
	 {
		...
		$RABTransActDetStr = "Escalation MBook - ".$sc_mbook_no." returned back to ".$RABTranFWRoleName." Level";
		//UpdateWorkTransaction($sc_sheetid,$sc_rbnno,"R",$RABTransActDetStr,"");
	 }
	 else
	 {
...
$SelectAbstractBookQuery = "select * from abstractbook where sheetid = '$sheetid' and rbn = '$esc_rbn'";
$SelectAbstractBookSql = mysqli_query($dbConn,$SelectAbstractBookQuery);
if($SelectAbstractBookSql == true)
{
	if(mysqli_num_rows($SelectAbstractBookSql)>0)
	{
		$AsbBookList = mysqli_fetch_object($SelectAbstractBookSql);
		$AbstBookMBNo 	= $AsbBookList->mbookno;
		$AbstBookMBPage = $AsbBookList->mbookpage;
	}
}
...
$select_bid_sql_tca 	= mysqli_query($dbConn,$select_bid_query_tca);
if($select_bid_sql_tca == true)
{
	if(mysqli_num_rows($select_bid_sql_tca)>0)
	{
		while($BidListTCA = mysqli_fetch_object($select_bid_sql_tca))
		{
...
$select_tca_sql 	= mysqli_query($dbConn,$select_tca_query);
//echo $select_tca_query."<br/>";
if($select_tca_sql == true)
{
	if(mysqli_num_rows($select_tca_sql)>0)
	{
		$TCAprev_esc_month = ""; $TCAesc_qty_month_wise = 0;
		while($TCAList = mysqli_fetch_object($select_tca_sql))
		{
			...
			$select_tca_consum_sql = mysqli_query($dbConn,$select_tca_consum_query);
			if($select_tca_consum_sql == true )
			{
				if(mysqli_num_rows($select_tca_consum_sql)>0)
				{
					$TCA = mysqli_fetch_object($select_tca_consum_sql);
...
function GetElecricityRecovery($sheetid,$rbn)
{
	...
	$select_elec_rec_sql = mysqli_query($dbConn,$select_elec_rec_query);
	if($select_elec_rec_sql == true)
	{
		if(mysqli_num_rows($select_elec_rec_sql)>0)
		{
			while($EList = mysqli_fetch_object($select_elec_rec_sql))
			{
...
function GetWaterRecovery($sheetid,$rbn)
{
	...
	$select_water_rec_sql = mysqli_query($dbConn,$select_water_rec_query);
	if($select_water_rec_sql == true)
	{
		if(mysqli_num_rows($select_water_rec_sql)>0)
		{
			while($WList = mysqli_fetch_object($select_water_rec_sql))
			{
...
$select_bid_sql 	= mysqli_query($dbConn,$select_bid_query);
if($select_bid_sql == true)
{
	if(mysqli_num_rows($select_bid_sql)>0)
	{
		while($BidList = mysqli_fetch_object($select_bid_sql))
		{
			$bid = $BidList->bid;
			$BidStr .= $bid."*";
		}
...
//$ret = "Correct no need";
	...
	$select_rbndata_sql = mysqli_query($dbConn,$select_rbndata_query);
	if($select_rbndata_sql == true)
	{
		if(mysqli_num_rows($select_rbndata_sql)>0)
		{
			while($RList = mysqli_fetch_object($select_rbndata_sql))
			{
...
for($x7=0; $x7<$BidCnt; $x7++)
{
	...
	for($x4=0; $x4<$EscMonthCnt; $x4++)
	{
		...
		$select_tcc_sql 	= 	mysqli_query($dbConn,$select_tcc_query);
		if($select_tcc_sql == true)
		{
			$TCCList = mysqli_fetch_object($select_tcc_sql);
...
<table width='675' cellpadding='3' cellspacing='3' align='center' class='label table1 labelprint' bgcolor="#FFFFFF" id="table1">
	...
	<?php
		if($tccAbsmbook == 0){ $tccAbsmbook = $AbstBookMBNo; }
		if($tccAbsmbookpage == 0){ $tccAbsmbookpage = $AbstBookMBPage+1; }
	?>
	<tr>
...
<?php 
	...
	/*if(($total_esc_amt != "")&&($total_esc_amt != 0))
	{
		...
		$update_tcc_sql = mysqli_query($dbConn,$update_tcc_query);
	}*/
} 
$staffid_acc 		= $_SESSION['sid_acc'];
$staff_level_str 	= getstafflevel($staffid_acc);
...
$staff_roleid 		= $exp_staff_level_str[0];
$staff_levelid 		= $exp_staff_level_str[1];
$AccVerification 	= AccVerificationCheck($sheetid,$esc_rbn,$esc_mbook,'staff',$staff_levelid,'MB');
$AlStatusRes 		= AccountsLevelStatus($sheetid,$esc_rbn,$esc_mbook,0,'E','escalation');//($sheetid,$rbn);
...
<input type="hidden" name="txt_mbook_no" id="txt_mbook_no" value="<?php echo $esc_mbook; ?>"/>
