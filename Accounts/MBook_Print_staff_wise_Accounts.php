...
function getabstractpage($sheetid,$subdivid)
{
	...
	$List = mysqli_fetch_object($select_abs_page_sql);
	$abstmbookno = $List->abstmbookno;
	$abstractpage = $List->abstmbpage;
...
function getcompositepage($sheetid,$subdivid,$rbn,$zone_id)
{
	...
	$List = mysqli_fetch_object($select_sa_page_sql);
	$mbookno_compo = $List->sa_mbno;
	$mbookpageno_compo = $List->sa_page;
	if(($mbookno_compo == 0)&&($mbookpageno_compo == 0)){
		...
		$select_abs_page_sql = mysqli_query($dbConn,$select_abs_page_query);
		$List2 = mysqli_fetch_object($select_abs_page_sql);
		$mbookno_compo = $List2->mbno;
		$mbookpageno_compo = $List2->mbpage;
	}
...
echo $query ;exit;
*/
$query = "SELECT DATE_FORMAT( mbookheader.date , '%d/%m/%Y' ) AS date ,  mbookdetail.subdivid , schdule.sno as subdiv_name , mbookdetail.mbdetail_id, 
mbookdetail.descwork, mbookdetail.measurement_no , mbookdetail.measurement_l , mbookdetail.measurement_b, mbookdetail.structdepth_unit, mbookdetail.accounts_remarks,
mbookdetail.measurement_d , mbookdetail.measurement_contentarea , mbookdetail.remarks, schdule.measure_type, schdule.shortnotes, schdule.description, mbookheader.sheetid   
FROM schdule
INNER JOIN mbookheader ON (mbookheader.subdivid = schdule.subdiv_id)
INNER JOIN mbookdetail ON (mbookheader.mbheaderid = mbookdetail.mbheaderid)
WHERE  mbookheader.date  >= '$fromdate' AND mbookheader.date  <= '$todate' AND schdule.measure_type != 's' AND mbookdetail.mbdetail_flag != 'd' AND mbookheader.sheetid = '$sheetid' AND mbookheader.staffid = '$staffid' ".$zone_clause." ORDER BY mbookheader.date, mbookdetail.subdivid, mbookheader.mbheaderid, mbookdetail.mbdetail_id ASC" ;                //echo $query ;exit;
//echo $query;exit;
$sqlquery = mysqli_query($dbConn,$query);
 if ($sqlquery == true) 
{
	while ($List = mysqli_fetch_object($sqlquery)) 
	{
...
<?php 			
		...
		//echo $querycount."<br/>";
		$querycount_sql = mysqli_query($dbConn,$querycount);
		$res = mysqli_fetch_array($querycount_sql); 
		$rowcount = $res[0];
				if($prev_subdivid != "")
				{
		?>
