...
if(isset($_POST['btnSave'])){
	...
	if($DupExist == 0){
		$InsertQuery 	= "INSERT INTO memo_payment_accounts_edit SET mop_date = '$MopDate', sheetid = '$SaveSheetId', mis_item_id = '$SavePaymentFor', contid = '$SaveContId', cbdtid = '$SaveBankId', 
						  abstract_net_amt = '$SaveNetAmt', net_payable_amt = '$SaveNetAmt', edit_flag = 'EDIT', mop_type = '$SaveMopType', hoa = '$Hoa', hoaid = '$HoaId', 
						  shcode_id = '$SaveSCodeId', remarks = '$SaveRemarks', staffid = '".$_SESSION['sid']."', 
						  userid = '".$_SESSION['userid']."', modifieddate = NOW(), active = 1";
										...
										<div class="div12">
											<div class="card cabox" style="margin-bottom:1px;">
												<div class="face-static">
													<div class="card-header inkblue-card" align="left">&nbsp;Memo of Payment for SD Release / EPF / GST Reimbursement</div>
															...
															<div class="row">
																<div class="div12" align="center">
																	<div class="innerdiv2">
																		<div class="row" align="center">
																			<div class="div4 pd-lr-1">
																				<div class="lboxlabel">Payment for</div>
																				<div>
																					<select name="cmb_payment_for" id="cmb_payment_for" class="dynamicboxlg inp" required>
																						<option value=""> -- Select --</option>
																						<?php echo $objBind->BindAllMiscellItems(0,'CCNO'); ?>
																						<!--<option value="OTH">Others</option>-->
																					</select>
																					<input type="hidden" name="txt_mop_type" id="txt_mop_type" />
																					<input type="hidden" name="txt_misc_module" id="txt_misc_module" />
																				</div>
																			</div>
																			...
																			<div class="row" align="center">
																				<div class="div12 pd-lr-1" align="center">
																					...
																					<a href="MopSDRList.php" class="btn btn-info" name="btn_view" id="btn_view"> View All </a>
																				</div>
																			</div>
																			<div class="row clearrow"></div>
																		</div>
...
$(function() {
	...
	$("body").on("change","#cmb_payment_for", function(event){
		$("#txt_net_amt").val('');
		$("#txt_mop_type").val('');
		$("#txt_misc_module").val('');
		var PaymentFor = $(this).val();
		var MopType = $('option:selected',this).attr("data-moptype");
		var MiscModule = $('option:selected',this).attr("data-misc_module");
		$("#txt_mop_type").val(MopType);
		$("#txt_misc_module").val(MiscModule);
		if(PaymentFor == "OTH"){
	...
	$("body").on("click","#btnGo", function(event){
		...
		$("#cmb_payment_for").chosen('destroy');
		//$("#cmb_rec_desc_0").chosen('destroy');
		//$("#cmb_rec_hoa_scode_0").chosen('destroy');
		$(".inp").val('');
		//$(".Ldc").addClass('hide');
