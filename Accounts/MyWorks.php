...
if(isset($_GET['list']) != ""){
	...
	if($View == "L"){
		...
		$WhereClause = " active = 2";
	}else{
		$WhereClause = " (active = 1 OR active = 2)";
	}
}else{
	$WhereClause = " (active = 1 OR active = 2)";
	$View = "A";
...
$SelectWorkQuery 	= "SELECT * FROM sheet WHERE ".$WhereClause;
//}else{
	//$SelectWorkQuery 	= "SELECT * FROM sheet WHERE (eic = '$staffid' OR FIND_IN_SET($staffid,assigned_staff)) AND ".$WhereClause;
//}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<!--<div class="card-header inkblue-card" align="center">Live Works List</div>-->
															...
															<div class="table-responsive dt-responsive rtabdiv" id="table-stmt">
																...
																<div align="center">
																	...
																	<div class="div12 no-padding-lr" align="center">
																		<div class="col-status" data-page="<?php echo $Page; ?>" data-id='A'><div class="well well-sm well-A <?php if($View == "A"){ ?> active <?php } ?>"><span class="rlable-pink">Total Work List</span></div></div>
																		<div class="col-status" data-page="<?php echo $Page; ?>" data-id='L'><div class="well well-sm well-A <?php if($View == "L"){ ?> active <?php } ?>"><span class="rlable-pink">Live Works</span></div></div>
																		<div class="col-status" data-page="<?php echo $Page; ?>" data-id='C'><div class="well well-sm well-A <?php if($View == "C"){ ?> active <?php } ?>"><span class="rlable-pink">Completed Works</span></div></div>
																	</div>
																</div>
																...
																<?php $x = 2; foreach($SheetRABArr as $sheetid=>$rbn){ $CCNO = $SheetCCNoArr[$sheetid]; ?>
																	<div class="accordion">
																		<dl>
																			<dt>
																				<a href="#accordion<?php echo $sheetid; ?>" id="sheet-<?php echo $sheetid; ?>" aria-expanded="false" aria-controls="accordion<?php echo $sheetid; ?>" class="accordion-title accordionTitle js-accordionTrigger blue-bg <?php if($x == 1){ ?> is-collapsed is-expanded <?php } ?>">
																					...
																					&nbsp;&nbsp;<span data-page="<?php echo $Page; ?>" data-id="<?php echo $sheetid; ?>" class="clickbtnmd BillStatus">
																						<?php if($Page == "WORK"){ echo "Work Status"; }else if($Page == "WORK"){ echo "RAB Generate & Bill Status"; }else{ echo "RAB Generate & Bill Status"; } ?>
																					</span>
																				</a>
																			</dt>
																			<dd class="accordion-content accordionItem <?php if($x == 1){ ?> is-expanded animateIn <?php } else{ ?> is-collapsed <?php } ?>" id="accordion<?php echo $sheetid; ?>" aria-hidden="true" style="overflow:auto;">
																				<div align="left" style="padding:5px 5px; background:#fff;">
																					...
																					<span data-page="<?php echo $Page; ?>" data-id="<?php echo $sheetid; ?>" class="clickbtnmd BillStatus">RAB Generate & Bill Status</span>
																				</div>
																			</dd>
																		</dl>
																	</div>
...
$(document).ready(function(){ 
	...
	$('.col-status').click(function(event){
		...
		if((id != undefined)&&(id != '')){
			$(location).attr('href', 'MyWorks.php?page='+page+'&list='+id);
		}
	});
	$('body').on('click', '.BillStatus', function(){
		var id = $(this).attr('data-id');
		var page = $(this).attr('data-page');
		if(page == "BILL"){
			$(location).attr('href', 'RABStatus.php?work='+id);
		}else if(page == "WORK"){
			$(location).attr('href', '../WorkStatus/WorkStatus.php?work='+id);
		}else{
			$(location).attr('href', '../WorkStatus/WorkStatus.php?work='+id);
		}
	});
