...
$WorkQuery = "SELECT ccno,sch_comp_date,tr_id,globid,wo_no FROM works";
...
if($WorkQuerysql == true){
	if(mysqli_num_rows($WorkQuerysql)>0){
		while($List2 = mysqli_fetch_object($WorkQuerysql)){
			$SchDate = $List2->sch_comp_date;
			$Newdate =   date('Y-m-d', strtotime($SchDate. ' + 8 months'));
			$WorksWONoDataArr[$List2->globid] = $List2->wo_no;
			$WorksCCNoDataArr[$List2->globid] = $List2->ccno;
			if($Newdate < $CurrDate){
				$TrId1 	   = $List2->tr_id;
	...
	//print_r($WorkTRID);exit;
$ImpDetIds = implode(',',$WorkTRID);
$PgRetMasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, d.name_contractor FROM loi_entry a 
INNER JOIN bg_fdr_details b ON (a.loa_pg_id = b.master_id) 
INNER JOIN tender_register c ON (a.tr_id = c.tr_id) 
INNER JOIN contractor d ON (b.contid = d.contid) WHERE b.inst_purpose='PG' AND a.tr_id IN ($ImpDetIds) and b.inst_status != 'R' ORDER BY a.sheetid ASC";
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">PG Release Reminder <span class="ralignbox fright"><span class="xldownload" id="exportToExcel"> Download Excel <i class="fa fa-download"></i> </span></span></div>
