...
if(isset($_POST['btn_save']) == " SAVE & NEXT "){
	...
	$IsMbGen = 0;
	$SelectQuery = "SELECT DISTINCT rbn FROM measurementbook_temp WHERE sheetid = '$SaveSheetId' AND rbn = '$SaveRbn'";
	...
	if(($BillMode == "ON")||($IsMbGen == 1)){
		$abs_last_page = $_POST['txt_abs_last_page'];
		
		$_SESSION['abs_last_page'] 	= "";//$abs_last_page;
		$_SESSION['PassOrderDate'] 	= $SavePoDate;
		$_SESSION['PassOrderAmt'] 	= $PassOrderAmt;
		$_SESSION['PassOrderCrBy'] 	= $staffid;
		$_SESSION['PassOrderLevel'] = $_SESSION['levelid'];
...
$(function() {
	...
	function GetWorkDetails(){
		...
		$.ajax({ 
			...
			data: ({ WorkId: WorkId, PageType:'PSO', PayFlag:'FPAY'}), 
			success: function (data) {
				if(data != null){ 
					$.each(data, function(index, element) { //alert(element.is_adv_pay_flag);
