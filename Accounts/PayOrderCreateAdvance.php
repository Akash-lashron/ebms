...
if(isset($_POST['btn_save']) == " SAVE "){
	...
	if($IsAdvPayFlag == "Y"){
		$UpdateQuery1 	= "UPDATE memo_payment_accounts_edit SET pay_order_dt_adv_pay = '$SavePoDate', pay_order_amt_adv_pay = '$PayOrderAmt', pay_order_cr_by_adv_pay = '$staffid', pay_order_cr_level_adv_pay = '".$_SESSION['levelid']."' WHERE sheetid = '$SaveSheetId' AND rbn = '$SaveRbn' AND is_adv_pay = 'Y'";
	...
	if($UpdateQuery1 == true){
		$msg = "Pay order data for 75% advance saved successfully";
	}else{
		$msg = "Error : Pay order data for 75% advance not saved. Please try again.";
	}
}
?>
...
$(function() {
	...
	function GetWorkDetails(){
		...
		$.ajax({ 
			...
			data: ({ WorkId: WorkId, PageType: 'PYO', PayFlag:'PPAY'}), 
