...
if((isset($_GET["work"]))&&($_GET["work"] != "")){
	...
	if($RepView == 1){
		...
		if($SelectSqlB == true){
			$count10 	= mysqli_num_rows($SelectSqlB);
			if($count10 > 0){
		...
		$IsBillReg = 0;
		$SelectQueryC 	= "select * from bill_register where sheetid = '$sheetid' and rbn = '$rbn'";
		...
		$select_meas_book_temp_sql 	 = mysqli_query($dbConn,$select_meas_book_temp_query);
		if($select_meas_book_temp_sql == true){
			$count2 	= mysqli_num_rows($select_meas_book_temp_sql);
			$List2 		= mysqli_fetch_object($select_meas_book_temp_sql);
		...
		if($select_meas_gen_sql == true){
			$count3 	= mysqli_num_rows($select_meas_gen_sql);
			$List3 		= mysqli_fetch_object($select_meas_gen_sql);
		...
		if($select_meas_gen_staff_sql == true){
			$count4 = mysqli_num_rows($select_meas_gen_staff_sql);
		}
		/*if($fromdate1 != "" && $todate1){
			...
			$select_min_from_sql 	= mysqli_query($dbConn,$select_min_from_query);
			if($select_min_from_sql == true){
				$List4 		= mysqli_fetch_object($select_min_from_sql);
			...
			$select_max_to_sql 		= mysqli_query($dbConn,$select_max_to_query);
			if($select_max_to_sql == true){
				$List5 		= mysqli_fetch_object($select_max_to_sql);
		...
		if($fromdate != "" && $todate != ""){
			...
			$select_measurement_sql1 = mysqli_query($dbConn,$select_measurement_query1);
			if($select_measurement_sql1 == true){
				$count6 = mysqli_num_rows($select_measurement_sql1);
			}
		}else{
			$select_todate_query 	= "select max(todate) as mdate from mbookgenerate_staff where sheetid = '$sheetid' ";//and rbn = '$rbn'";
			$select_todate_sql 		= mysqli_query($dbConn,$select_todate_query);
			if($select_todate_sql == true){
				$List6 	= mysqli_fetch_object($select_todate_sql);
				$mdate 	= $List6->mdate;
			}
			if($mdate != ""){
				...
				$select_measurement_sql2 = mysqli_query($dbConn,$select_measurement_query2);
				if($select_measurement_sql2 == true){
					$count7 = mysqli_num_rows($select_measurement_sql2);
					//echo $count7 ;
				}
			}else{
				...
				$select_measurement_sql3 = mysqli_query($dbConn,$select_measurement_query3);
				if($select_measurement_sql3 == true){
					$count8 = mysqli_num_rows($select_measurement_sql3);
					//echo $count8;
				}
			}
		}
		...
		$select_zoin_sql 	= mysqli_query($dbConn,$select_zoneid_query);
		if($select_zoin_sql == true){
			$countZ 	= mysqli_num_rows($select_zoin_sql);
			while($ZList = mysqli_fetch_object($select_zoin_sql)){
		...
		if($select_mbook_sql == true){
			$count5 = mysqli_num_rows($select_mbook_sql);
			if(mysqli_num_rows($select_mbook_sql)>0){
				while($MBList = mysqli_fetch_object($select_mbook_sql)){
		...
		$select_abstractbook_sql 	 	= mysqli_query($dbConn,$select_abstractbook_query);
		if($select_abstractbook_sql == true){
			$count10 	= mysqli_num_rows($select_abstractbook_sql);
			$ListAB		= mysqli_fetch_object($select_abstractbook_sql);
								...
								<div class="row">
									...
									<div class="div10" style="width:89%; margin-left:-20px;">
										...
										<div class="row innerdiv style1">
										    ...
										    <?php  //if($count1 == 0){ ?>
												<div class="div12 style1" style="margin-top:0px;"><div class="row divhead style1 well-B" align="left">&nbsp;&nbsp; RAB - <?php echo $rbn; ?> Status :
												 <?php 
												 	if($RabStatus == "C"){
														...
														if($IsBillReg == 1){
															...
															echo "Bill not yet forwarded to Accounts";
														}else{
															echo "Bill In Progress ";
														}
										   ...
										   <?php if($RepView == 2){ ?>
													...
													<tr>
														<td>
														   ...
														   <span class="rlable-pink">Recovery Released Amount  : <?php echo $SheetDataArr[$sheetid][9]; ?></span>
														</td>  
													</tr>
													<?php
														...
														if(mysqli_num_rows($SelectStatusSql)>0){
															while($SList = mysqli_fetch_object($SelectStatusSql)){
																...
																if($MBType == 'G'){
																	...
																	$MBTypeStr = "Escalation";
																}else if($MBType == 'EA'){
																	$MBTypeStr = "Escalation Abstract";
																}else if($MBType == 'CC'){
																	$MBTypeStr = "Cement Consumption";
																}else if($MBType == 'SC'){
																	$MBTypeStr = "Steel Consumption";
																}
														...
														<td style="padding:0px; border:0px !important">
															<table class="table table-bordered">
																...
																if($TrTDStr != ''){
																	...
																	echo '<td colspan="19" align="center">Bill not yet forwarded to Accounts</td>';
																}
																?>
																</tbody>
															</table>
