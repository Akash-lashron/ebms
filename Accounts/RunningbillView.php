...
if($_POST["submit"] == " View ") 
{
	...
	//$_SESSION['RBN']	=	$_POST['cmb_rbn'];
	//$_SESSION['Sheetid']= 	$_POST['cmb_work_no'];
	$rbnView 		= 	$_POST['cmb_rbn'];
	$SheetidView	=	$_POST['cmb_work_no'];
	header('Location: RABBook.php?rbnView='.$rbnView.'&SheetidView='.$SheetidView); 
}
/*$rbn_no=trim($_POST['cmb_rbn']);
$mbookdetails="select sheet_id,mb_id from mbookgenerate  where rbn='$rbn_no'";
$sql_mbook=mysqli_query($dbConn,$mbookdetails);

if($sql_mbook == true) {
$List = mysqli_fetch_object($sql_mbook);
            ...
            xmlHttp.onreadystatechange = function ()
            {
                if (xmlHttp.readyState == 4)
                {
                    ...
                    {	
                        ...
                        var c = name.length;//alert("COUNT="+c);
                        //var a = c / 2;
                        //var b = a + 1;
                        for (i = 0 ; i < c ; i++)
                        {
                                ...
                                <table width="100%"  bgcolor="#E8E8E8" border="0" cellpadding="0" cellspacing="0" align="center" >
									...
									<tr> 
                                        ...
                                        <td  class="labeldisplay"><?php
                                         	//$sql_mbookheader="select DISTINCT measurementbook.rbn from measurementbook  INNER JOIN mbookheader ON (mbookheader.mbheaderid = measurementbook.measurementbookid) WHERE measurementbook .active='1'";
											$sql_mbookheader="select DISTINCT measurementbook.rbn from measurementbook";
											
                                            $rs_mbookheader = mysqli_query($dbConn,$sql_mbookheader);
                                            ?>
