<?php	
	...
	$SelectIdQuery 	= "select group_desc, id, par_id, type from group_datasheet_hc where id = '$group_id'";
	...
	$group3_desc	= $DESC;
	if($_POST['rad_type'] == 'N'){
		$sql_insert	= "insert into datasheet_master_hc(group_id,group3_description,group4_description,quantity,unit,id,par_id,type,new_merge,calc_type,cost_dt,ds_type,disp_qty_perc,to_unit,final_unit,hc_type)
						values('" . $group_id . "' ,
							   '" . $group3_desc . "' ,
		...
		$ref_id = mysqli_insert_id($dbConn);
		if($rs_insert == true){
			$tmp1 = 1;
		}else{
			$tmp1 = 0;
		...
		for($c=0;$c<count($ItemCodeArr);$c++){
			...
			if($ItemCode != ""){
				...
				$x		= $a1_rec[$c];
				$sno	= $c+1;
				$sql_insert_details = "insert into datasheet_a1_details_hc(sno,ref_id,item_ds_type,item_id,title,item_desc,calc_desc,qty_desc,item_alt_desc,calc_actions,actions_factors,calc_type,amt_type,quantity,merge_item_code,merge_ref_id,SI,new_merge)
							  values('" . $sno . "' ,
									 '" . $ref_id . "' ,
									 '" . $Type . "' ,
									 '" . $ItemId . "' ,
		...
		$sql_insert	= "insert into datasheet_master_hc(ref_id,group_id,group3_description,group4_description,quantity,unit,id,par_id,type,new_merge,calc_type,is_average,cost_dt,ds_type,disp_qty_perc,to_unit,final_unit,hc_type)
						values('" . $ref_id . "' ,
							   '" . $group_id . "' ,
							   '" . $group3_desc . "' ,
							   '" . $_POST['txt_group4_desc'] . "' ,
		...
		for($c=0;$c<count($ItemIdArr);$c++){
			...
			if((ItemId != "")&&($RefId != "")){
				$tmpX++;
				$sno = $c+1;
				$sql_insert_details = "insert into datasheet_a1_details_hc(sno,ref_id,item_ds_type,item_id,title,item_desc,calc_desc,item_alt_desc,calc_actions,actions_factors,quantity,SI,merge_item_code,merge_ref_id,new_merge)
							  values('" . $sno . "' ,
									 '" . $ref_id . "' ,
									 'D' ,
									 '' ,
