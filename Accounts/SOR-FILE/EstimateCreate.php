...
if(isset($_POST['btn_save']) == ' Save '){
	...
	if($InsertMasterSql == true){ $Temp1++; }
	$MasterId 			= mysqli_insert_id($dbConn);
	foreach($ItemNoArr as $key => $Value){
		$ItemNo 	= $ItemNoArr[$key];
		$ItemId 	= $ItemIdArr[$key];
		$RefId 		= $RefIdArr[$key];
		$ItemCode 	= $ItemCodeArr[$key];
		$ItemDesc 	= $ItemDescArr[$key];
		$ItemUnit 	= $ItemUnitArr[$key];
		$ItemQty 	= $ItemQtyArr[$key];
		$ItemRate 	= $ItemRateArr[$key];
		$ChildId 	= $ChildIdArr[$key];
		$ItemAmount = round(($ItemQty * $ItemRate),2);
		if(($ItemNo != '')&&($ItemDesc != "")&&($ItemCode != "")){
			$InsertDetailsQuery = "insert into parta_details set mastid = '$MasterId', sno = '$ItemNo', ref_id  = '$RefId', id = '$ChildId', group_id  = '$ItemId', unit  = '$ItemUnit', quantity  = '$ItemQty', supply = '$ItemRate', amount = '$ItemAmount'";
	...
	if(($Temp1 > 0)&&($Temp2 > 0)){
		...
		$msg = "Error : Estimate not created. Please try again.";
	}
}
?>
													...
													<tr>
														...
														<td class="labelcenter" nowrap="nowrap" valign="middle">
															...
															<datalist id="ItemCodeListNew0" style="color:#C80B5B; font-size:16px">
																<?php echo $objBind->BindDataSheetMainData(0); ?>
															</datalist>
														</td>
														<td class="labelcenter" nowrap="nowrap" valign="middle" width="60%">
															...
															<input type="hidden" class="tboxsmclass" style="width:100%" name="txt_ref_id[]" id="txt_ref_id0" value="" readonly=""  data-index="0" />
															<input type="hidden" class="tboxsmclass" style="width:100%" name="txt_child_id[]" id="txt_child_id0" value="" readonly=""  data-index="0" />
														</td>
														<td class="labelcenter" valign="middle">
...
$(document).ready(function(){
	...
	$("#tab_a1_material").on("change", ".icode", function(event) {
		...
		$("#txt_ref_id"+RowID).val('');
		$("#txt_child_id"+RowID).val('');
		...
		$.ajax({ 
			...
			success: function (data) {  
				if(data != null){ 
					...
					$("#txt_ref_id"+RowID).val(refid);
					$("#txt_child_id"+RowID).val(id);
				}
			}
		});
	...
	$("#btn_add").click(function(event){ 
		...
		var refId 	 	= $("#txt_ref_id0").val();
		var ChildId 	= $("#txt_child_id0").val();
		var Error = 1;
		if(itemNo == ""){ 
			...
			$('#tab_a1_material').find('tr:last').prev().after('<tr><td class="labelcenter" valign="middle"><input type="text" class="tboxsmclass" size="10" style="width:100%;" name="txt_item_no[]" id="txt_item_no'+index+'" data-index="'+index+'" value="'+itemNo+'" /></td><td class="labelcenter" valign="middle"><input type="text" class="tboxsmclass icode" list="ItemCodeListNew'+index+'" size="10" style="width:100%;" name="txt_code[]" id="txt_code'+index+'" data-index="'+index+'" value="'+itemCode+'" /><datalist id="ItemCodeListNew'+index+'" style="color:#C80B5B; font-size:16px"></datalist><input type="hidden" class="labelfield" size="9" style="width:100%" name="txt_item_id[]" id="txt_item_id'+index+'" data-index="'+index+'" value="'+itemId+'" readonly="" /><input type="hidden" class="labelfield" size="9" style="width:100%" name="txt_ref_id[]" id="txt_ref_id'+index+'" data-index="'+index+'" value="'+refId+'" readonly="" /><input type="hidden" class="labelfield" size="9" style="width:100%" name="txt_child_id[]" id="txt_child_id'+index+'" data-index="'+index+'" value="'+ChildId+'" readonly="" /></td><td class="labelcenter" valign="middle" nowrap="nowrap"><input type="text" class="tboxsmclass idesc" list="ItemDescListNew'+index+'" data-index="'+index+'" size="25" style="width:100%;" name="txt_desc[]" id="txt_desc'+index+'" value="'+itemDesc+'" /><datalist id="ItemDescListNew'+index+'" style="color:#C80B5B; font-size:16px"></datalist></td><td class="labelcenter" valign="middle"><input type="text" class="tboxsmclass ctext disable" size="5" style="width:100%" name="txt_unit[]" id="txt_unit'+index+'" value="'+itemUnit+'" readonly="" /><input type="hidden" class="tboxsmclass rtext disable" size="5" style="width:100%" name="txt_rate[]" id="txt_rate'+index+'" value="'+itemRate+'" readonly="" /></td><td class="labelcenter" valign="middle"><input type="text" class="tboxsmclass rtext Qty" size="5" style="width:100%" data-index="'+index+'" name="txt_quantity[]" id="txt_quantity'+index+'" value="'+itemQty+'" /><input type="hidden" class="tboxsmclass rtext disable NewAmt" size="8" style="width:100%" name="txt_amount[]" id="txt_amount'+index+'" value="'+itemAmt+'" readonly="" /></td><td class="labelcenter" valign="middle"><i style="font-size:21px" class="fa faicon-del delete" name="btn_delete" id="btn_delete'+index+'">&#xf057;</i></td></tr>'); //add input box
