...
$RefId 		= $_POST['refid'];
//echo $GroupId;
$MasterQty  = 0; $MasterDesc = ""; $MasterUnit = ""; $DisQtyPerc = 0; $IsAvgRequir = "";
$SelectMasterQuery 	= "select * from datasheet_master where ref_id = '$RefId' and group_id = '$GroupId' and id = '$Id' and par_id = '$ParId'";
...
$TotalAmount = 0; $TotalAmount2a = 0; $TotalAmount2b = 0; $Str = ""; $AvgCnt = 0;
...
$SelectDetailQuery 	= "select a.*, b.item_desc, b.unit, b.price, b.item_code from datasheet_a1_details a left join item_master b on (a.item_id = b.item_id) where a.ref_id = '$RefId'";
...
if($SelectDetailSql == true){
	if(mysqli_num_rows($SelectDetailSql)>0){
		while($ListD = mysqli_fetch_object($SelectDetailSql)){
			//if($ListD->item_id != ''){
			if(($ListD->item_id != '')&&($ListD->item_id != 0)){ 
				$Qty 	= $ListD->quantity;
				...
				$AvgCnt++;
				$retVal = CalculateTSandIGCARRateMergeSubData($ListD->merge_ref_id,$conn); 
				...
				if($MRGCalcType == "WC"){
					if($ListD->amt_type == "GAMT"){
						...
						$SDAmount1 = $ForOneUnitRate;
						$SDAmount2 = $IGCARRate1;
					}
					
				}else{
					$SDAmount = $GrossAmount;
				} 
				//echo $MRGCalcType; exit;
				//$Str .= $SDAmount1."=".$SDAmount2."<br/>";
				...
				$TotalAmount2a  = $TotalAmount2a + $SDAmount1;
				$TotalAmount2b  = $TotalAmount2b + $SDAmount2;
			}
		}
	}
}
//echo $TotalAmount2a."=".$TotalAmount2b;exit;
if($TotalAmount != 0){
	...
	if(($IsAvgRequir == "Y")&&($AvgCnt > 0)){
		$TSRate 	= round(($TotalAmount2a/$AvgCnt),2);
		$IGCARRate 	= round(($TotalAmount2b/$AvgCnt),2);
		$IGCARRate1 = round(($TotalAmount2b/$AvgCnt),2);
	}else{
		$TSRate 	= $TotalAmount2a;
		$IGCARRate 	= $TotalAmount2b;
		$IGCARRate1 = $TotalAmount2b;
	}
}
if($DMDisposQty != 0){ 
