...
{
	$sql_update="update group_datasheet  set group_desc ='" . $_POST['txt_group_III']    . "',type ='" . $_POST['txt_type']    . "'
				 where    id   ='" . $_POST['txt_group_3']   . "'";
...
if (isset($_POST['btn_add_x']))
{
	...
	$cmb_group3=$_POST["txt_group_III"];
	...
	/*$sql_max_group4="select max(id) as id from group_datasheet where 
					 id like '".$cmb_group2. "%' and char_length(id) = '6'";
		$rs_max_group4=mysqli_query($dbConn,$sql_max_group4,$conn);
		$group_id_3=@mysqli_result($rs_max_group4,0,'id');
		if($group_id_3 == ""){
			$group_id_3 = $cmb_group2."01";
		}else{
			$group_id_3 = $group_id_3 + 1;
		}
		if (strlen($group_id_3)==5){
			$group_id_3 = '0'.$group_id_3;		
		}else{
			$group_id_3 = $group_id_3;
		}
	$ParId = 0;
	$SelectparIdQuery 	= "select id from group_datasheet where id = '$cmb_group2'";
	...
	$rs_update='';*/
	$insert_query="insert into group_datasheet set  group_desc='$cmb_group3', type='$txt_type', par_id = '$cmb_group2', disp = '$Display'";
	...
	if($insert_sql!="")
	{
		...
		<script type="text/javascript" language="javascript">
			...
			window.location="group3_edit.php";
		</script>
		<?php
	}
}
if($_GET['gp_id']!="")
{ 
	$GlobGrF1Id = $_GET['gp_id'];
	$sql_modify="select * from group_datasheet where id='" . $_GET['gp_id'] . "'";
	$rs_modify=mysqli_query($dbConn,$sql_modify,$conn);
    $sql_count="select count(ref_id) as ref_id from datasheet_master where id like '" . $_GET['gp_id'] . "%'";
    $rs_count=mysqli_query($dbConn,$sql_count,$conn);
    $ref_count=@mysqli_result($rs_count,0,'ref_id');
	
	$view 		= 1;  $countG =0; $slno=0;
	$select_parId_querys = "select * from group_datasheet where id ='$GlobGrF1Id' ";
	$SelectparIdSqls 	= mysqli_query($dbConn,$select_parId_querys);
	if($SelectparIdSqls == true){
	$countG 	= mysqli_num_rows($SelectparIdSqls);
		if(mysqli_num_rows($SelectparIdSqls)>0){
			$IdLists = mysqli_fetch_object($SelectparIdSqls);
			$GlobGrIIId 	    = $IdLists->par_id;
		}
	}
	$select_G1_query = "select * from group_datasheet where id ='$GlobGrIIId' ";
	$Selectg1Sql 	= mysqli_query($dbConn,$select_G1_query);
	if($Selectg1Sql == true){
	$countG1 	= mysqli_num_rows($Selectg1Sql);
		if(mysqli_num_rows($Selectg1Sql)>0){
			$IdList = mysqli_fetch_object($Selectg1Sql);
			$GlobGrGF1id 	= $IdList->par_id;
		}
	}
...
if(isset($_POST['btn_delete_x']))
{
	...
	if($select_sql == true){
		...
		$sql_delete="update group_datasheet set delete_In='D' where id = '" . $_POST['txt_group_3'] . "'";
...
{
    $GlobGr1Id 	= $_POST['cmb_group1'];
	$GlobGr2Id 	= $_POST['cmb_group2'];
	$view 		= 1;  $count1 =0; $slno=0;
	$select_parId_query = "select id from group_datasheet where id ='$GlobGr2Id' ";
...
if($_GET['group_id_III']!="")
{
	$GlobGr1Id    =  $_GET['group_id_III'];
	$GlobGr2Id    =  $_GET['group_id_I'];
	$view 		= 1;  $count1 =0; $slno=0;
	$select_parId_query = "select * from group_datasheet where id ='" . $_GET['group_id_III'] . "' ";
	$SelectparIdSql 	= mysqli_query($dbConn,$select_parId_query);
	if($SelectparIdSql == true){
	$count1 	= mysqli_num_rows($SelectparIdSql);
		if(mysqli_num_rows($SelectparIdSql)>0){
 ...
 .labelboldleft {
    ...
    <body class="page1" id="top" oncontextmenu="return false"onload="noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
						...
						<blockquote class="bq1" style="overflow:auto">
							<table width="825" border="0" align="center" cellpadding="0" cellspacing="0">
								<tr class="heading">
									...
									<td width="664" height="28" background="Title bar/Titlebar_Centre_Piece.jpg" align="left">Group III - Create</td>
							...
							<table width="825" border="0" align="center" cellpadding="0" cellspacing="0" class="color1">
									...
									<td colspan="2">
										<select class="text group" style="width:400px;height:21px;" name="cmb_group1" ID="cmb_group1" data-group = "1">
										   <option value="Select">----------------- Select Group I ----------------- </option>
											 ...
											 <?php echo $objBind->BindGroupI($GlobGrGF1id, 'ALL'); ?>
										   <?php }else{ echo $objBind->BindGroupI($GlobGr1Id,'ALL'); }?>
										</select>
									</td>
								</tr>
									...
									<td colspan="2">
										<select class="text group" style="width:400px;height:21px;" name="cmb_group2" ID="cmb_group2" data-group = "2">
											<option value="Select">----------------- Select Group II -----------------</option>
											 <?php if($_GET['group_id_III'] != ''){  ///echo  $GlobGr2Id;?>
											 <?php echo $objBind->BindGroup2Master($GlobGr2Id, $GlobGr1Id); ?>
											 <?php }else if($_GET['gp_id'] != ''){?>
											 <?php echo $objBind->BindGroup2Master($GlobGrGF1id, $GlobGrIIId); ?>
											 <?php }else{ echo $objBind->BindGroup2($GlobGr2Id,'ALL'); }?>
										</select>
									</td>
								</tr>
								<tr><td style="line-height:13px;">&nbsp;&nbsp;</td></tr>
								<tr>
									...
									<td width="357" align="left" class="text" colspan="3"><input type="text" name="txt_group_III" id="txt_group_III" class="group" value="<?php if($_GET['gp_id']!="") echo @mysqli_result($rs_modify,0,'group_desc'); else echo ''; ?>" size="53" /></td>
								</tr>
								<tr><td style="line-height:13px;">&nbsp;&nbsp;</td></tr>
								<tr>
									<td width="50">&nbsp;</td>
								...
								<input type="hidden" name="txt_group_3" id="txt_group_3" value="<?php echo @mysqli_result($rs_modify,0,'id'); ?>"/>										
								<tr><td style="line-height:13px;">&nbsp;&nbsp;</td></tr>
								<tr align="center">
									<td colspan="5">
									<?php if($_GET['gp_id']!=""){ ?>
											...
											<center><input type="image" name="btn_back" id="btn_back" value="Back"  src="Buttons/Back_Normal.png" onMouseOver="this.src='Buttons/Back_Over.png'" onMouseOut="this.src='Buttons/Back_Normal.png'"  onclick="func_back()"/> &nbsp;&nbsp;&nbsp;<input type="image" name="btn_view_list" id="btn_view_list" value="View" src="Buttons/View_Normal.png" onMouseOver="this.src='Buttons/View_Over.png'" onMouseOut="this.src='Buttons/View_Normal.png'" onClick="return func_group_id3()"/> &nbsp;&nbsp;&nbsp;<input type="image" name="btn_update" id="btn_update" value="Update" src="Buttons/Update_Normal.png" onMouseOver="this.src='Buttons/Update_Over.png'" onMouseOut="this.src='Buttons/Update_Normal.png'" onClick="return update_func_group_id3()"/>&nbsp;&nbsp;&nbsp;<input type="image" name="btn_delete" id="btn_delete" value="Delete" src="Buttons/Delete_Normal.png" onMouseOver="this.src='Buttons/Delete_Over.png'" onMouseOut="this.src='Buttons/Delete_Normal.png'" onClick="return func_group_id3_del()"/></center>
									<?php }else{ ?>
											...
											<input type="image" name="btn_view_list" id="btn_view_list" value="View" src="Buttons/View_Normal.png" onMouseOver="this.src='Buttons/View_Over.png'" onMouseOut="this.src='Buttons/View_Normal.png'" onClick="return func_group_id3()"/>
							...
							<table width="825" border="0" align="center" cellpadding="0" cellspacing="0">
								 <tr class="heading">
									 ...
									 <td width="664" height="28" background="Title bar/Titlebar_Centre_Piece.jpg" align="left">Group III List</td>
									 <td width="36" height="28" background="Title bar/Titlebar_Right_Piece.jpg">&nbsp;</td>
								 </tr>
							</table>
							<table width="825" border="1" align="center" cellpadding="0" cellspacing="0" class="color2">
								 ...
								 <tr>
									 ...
									 <td align="center" width="100" height="28" valign="middle"><a href="group3_edit.php?gp_id=<?php echo $List->id;?>" class="btn-primary" style="padding:3px 15px; border-radius:5px">Edit</a>
							...
							<table width="825" border="1" align="center" cellpadding="0" cellspacing="0" class="color2">
								 ...
								 <?php $slno = 1; $count2 =0; if($countG > 0){ 
									   $select_group2_query  = "select * from group_datasheet where par_id	='$GlobGrIIId' and delete_In !='D'";
									   $select_group2_sql 	 = mysqli_query($dbConn,$select_group2_query);
										   if($select_group2_sql == true){
											 $count2 	= mysqli_num_rows($select_group2_sql);
											 if($count2 > 0){ 
		...
		function func_group_id()
		{
			...
			if(document.form.txt_group_III.value == "")
			{
			...
			document.form.action="group3_edit.php?group_id_I="+document.form.cmb_group1.value+document.form.cmb_group1.value
		...
		{
			document.form.method="post";
			document.form.action="group3_edit.php";
			document.form.submit();
		}
		function func_group_id3()
		{
			if(document.form.cmb_group1.value == "Select")
			{
				alert("Please Select Group I");
			...
			document.form.method="post";
			document.form.action="group3_edit.php?group_id_III="+document.form.cmb_group2.value+"&"+"group_id_I=" +document.form.cmb_group1.value
			document.form.submit();
		}
		function update_func_group_id3()
		{ 
			if(document.form.cmb_group1.value == "Select")
			{
				alert("Please Select Group I");
