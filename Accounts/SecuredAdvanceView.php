...
if((isset($_GET["workno"]))&&(isset($_GET["rbn"])))
{
	$sheetid = $_GET["workno"];
	$rbn 	 = $_GET["rbn"];
}
if(($sheetid != '')&&($rbn != ''))
{
	...
	$select_mb_page_sql = mysqli_query($dbConn,$select_mb_page_query);
	if($select_mb_page_sql == true){
		$MBPGList = mysqli_fetch_object($select_mb_page_sql);
...
$selectRbnSql 		= mysqli_query($dbConn,$selectRbnQuery);
if($selectRbnSql == true){
	$RabList 		= mysqli_fetch_object($selectRbnSql);
	$PrevRbn 		= $RabList->rbn;
	...
	$selectPrevAmtSql 		= mysqli_query($dbConn,$selectPrevAmtQuery);
	if($selectPrevAmtSql == true){
		while($PrevAmtList 	= mysqli_fetch_object($selectPrevAmtSql)){
...
$select_mb_page_sql = mysqli_query($dbConn,$select_mb_page_query);
if($select_mb_page_sql == true){
	if(mysqli_num_rows($select_mb_page_sql)>0){
		$MBPgList = mysqli_fetch_object($select_mb_page_sql);
                    ...
                    <blockquote class="bq1" style="background-color:#FFFFFF;overflow:auto;">
						...
						<table width='90%'  bgcolor='#FFFFFF' border='0' cellpadding='1' cellspacing='1' align='center' class='table1 labelprint' >
							...
							<tr>
								...
								<td align="center" nowrap="nowrap">(6)=(3)-(4)+(5)</td>
						...
						$SecuredAdvSql = mysqli_query($dbConn,$SecuredAdvQuery); 
						if($SecuredAdvSql == true){
							while($SList = mysqli_fetch_object($SecuredAdvSql)){
