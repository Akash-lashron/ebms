...
$staffList = mysqli_fetch_object($staff_design_query);
...
$Rbnresult = mysqli_fetch_object($select_rbn_sql);
$rbn = $Rbnresult->rbn;
...
$select_new_mbook_no_sql = mysqli_query($dbConn,$select_new_mbook_no_query);
if($select_new_mbook_no_sql == true)
{
	if(mysqli_num_rows($select_new_mbook_no_sql)>0)
	{
		$NMBList = mysqli_fetch_object($select_new_mbook_no_sql);
...
//$Mbsteelgeneratedelsql_qry = mysqli_query($dbConn,$Mbsteelgeneratedelsql);
...
$wodataquerysql = mysqli_query($dbConn,$wodataquery);
if ($wodataquerysql == true) 
    {
    $Res = mysqli_fetch_object($wodataquerysql);
...
function getabstractpage($sheetid,$subdivid)
{
	...
	$ResList1 = mysqli_fetch_object($select_abs_page_sql);
	$abstmbookno = $ResList1->abstmbookno;
	$abstractpage = $ResList1->abstmbpage;
...
function getcompositepage($sheetid,$subdivid,$rbn,$zone_id)
{
	...
	$select_sa_page_sql = mysqli_query($dbConn,$select_sa_page_query);
	$ResList1 = mysqli_fetch_object($select_sa_page_sql);
	$mbookno_compo = $ResList1->sa_mbno;
	$mbookpageno_compo = $ResList1->sa_page;
	//$mbookno_compo = @mysqli_result($select_sa_page_sql,0,'sa_mbno');
	//$mbookpageno_compo = @mysqli_result($select_sa_page_sql,0,'sa_page');
	if(($mbookno_compo == 0)&&($mbookpageno_compo == 0)){
		...
		$select_abs_page_sql = mysqli_query($dbConn,$select_abs_page_query);
		$ResList2 = mysqli_fetch_object($select_abs_page_sql);
		$mbookno_compo = $ResList2->mbno;
		$mbookpageno_compo = $ResList2->mbpage;
		//$mbookno_compo = @mysqli_result($select_abs_page_sql,0,'mbno');
		//$mbookpageno_compo = @mysqli_result($select_abs_page_sql,0,'mbpage');
	}
...
<td style="color:#FFFFFF; border:none; font-weight:bold; font-size:20px;">STEEL MEASUREMENT BOOK</td>
		...
		<input type="hidden" name="txt_steelmbno_id" value="<?php echo $steelmbno_id."*".$mbookno."*"."S"."*".$staffid."*".$sheetid; ?>" id="txt_steelmbno_id" />
			<?php
			//echo "<div align='center'><div align='right' style='width:1087px;'><a href='ViewSheet.php?sheetid=".$sheetid."' class='spanbtn' target='_blank'><span>SOQ<span></a> &nbsp;<a href='ComparativeStatement.php?sheetid=".$sheetid."' class='spanbtn' target='_blank'><span>CST<span></a>&nbsp;<a href='#' class='spanbtn' target='_blank'><span>Previous RAB<span></a></div></div>";
			echo "<div align='center'><div align='right' style='width:1087px;'><a href='ViewSheet.php?sheetid=".$sheetid."' class='spanbtn' target='_blank'><span>SOQ<span></a> </div></div>";
			...
			<?php echo $table1; ?>
			   ...
			   $sqlmeasurequery = mysqli_query($dbConn,$measurequery);    
               if($sqlmeasurequery == true)
               {
                while ($List = mysqli_fetch_object($sqlmeasurequery)) 
                { //echo $mpage;
...
$AlStatusRes 		= AccountsLevelStatus($sheetid,$rbn,$mbookno,$zone_id,'S','staff');
...
//echo $_SESSION['levelid'];exit;
	...
	//	echo $AccVerification." - ".$AcLevel." - ".$_SESSION['levelid']." - ".$AcStatus." - ".$TranRes['Max'];exit;
