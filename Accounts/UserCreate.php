													...
													<div class="div4" align="center">
														<?php if($_GET['userid'] != ""){ ?>
															<select name="cmb_engname" id="cmb_engname" class="group" style="width:100%">
																<?php
																	$SelectQuery = "SELECT a.* FROM staff a WHERE a.staffid = '$staffid' AND a.active = 1 AND a.sectionid != 2 order by a.staffcode asc";
																	$SelectSql	 = mysqli_query($dbConn,$SelectQuery);
																	if($SelectSql == true){
																		if(mysqli_num_rows($SelectSql)>0){
																			$SList 	= mysqli_fetch_object($SelectSql);
														...
														<?php }else{ ?>
															<select name="cmb_engname" id="cmb_engname" class="group" style="width:100%">
																...
																<?php
																	$SelectQuery = "SELECT a.* FROM staff a WHERE NOT EXISTS (SELECT * FROM users b WHERE b.staffid = a.staffid AND b.active = 1 AND b.sectionid != 2) AND a.active = 1 AND a.sectionid != 2 order by a.staffcode asc";
																	$SelectSql	 = mysqli_query($dbConn,$SelectQuery);
																	if($SelectSql == true){
																		if(mysqli_num_rows($SelectSql)>0){
