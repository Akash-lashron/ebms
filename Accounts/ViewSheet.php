...
$schdule=mysqli_query($dbConn,$schdulesql);
//echo $schdulesql;
$rebate_perc_sql = "SELECT rebate_percent FROM sheet WHERE sheet_id = '$sheetid' AND active = 1";
$rebate_perc_query = mysqli_query($dbConn,$rebate_perc_sql);
$RebateList = mysqli_fetch_object($rebate_perc_query);//,0,'rebate_percent');
$rebate_percent = $RebateList->rebate_percent;
 $RowCount =0;
                ...
                <div class="container_12">  
                    <div class="grid_12" align="center"> 
                        <blockquote class="bq1" id="bq1" style="overflow:scroll; position:relative">
                            <div class="container">
                                <?php 
                                if ($schdule == false) {  } else {        $RowCount = mysqli_num_rows($schdule);    }
							...
							<table width="90%" class="table1 table2">
                                ...
                                <?php 
								$overall_total_amt = 0;
								while ($List = mysqli_fetch_object($schdule)) 
								{ 
							...
							<div class="buttonsection">
								<?php if($_GET['sheetid'] != ''){ ?>
								<a onClick="CloseWin()" class="btn btn-info">Back</a>
								<?php }else{ ?>
								<a data-url="ViewAgreementSheet" class="btn btn-info">Back</a>
								<?php } ?>
							</div>
