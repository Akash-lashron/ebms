...
function check_workorder_measurements($sheetid)
{
	...
	$SelectItemIdSql 	= mysqli_query($dbConn,$SelectItemIdQuery);
	if(mysqli_num_rows($SelectItemIdSql)>0){
		return 1;
	}else{
		return 0;
	}
	...
	$check_workorder_measurements_query = mysqli_query($dbConn,$check_workorder_measurements_sql);
	if(mysqli_num_rows($check_workorder_measurements_query)>0)
	{
		return 1;
	}
	else
...
$schdulequery = mysqli_query($dbConn,$schdulesql);
$RowCount =0;
if(isset($_POST['back']))
{
															...
															<div class="table-responsive dt-responsive ResultTable">
																<div class="table-responsive dt-responsive rtabdiv" id="table-stmt">
																	<table width="100%" border="0" align="center" class="table1 table2" id="dataTable">
																		...
																		<tbody>
																		<?php $RowCount = mysqli_num_rows($schdulequery); if($RowCount == 0){ ?>
																			<tr>
																				<td colspan="8">No Records Found</td>
																			</tr>
																		<?php }else{ $sno = 1; while($List = mysqli_fetch_object($schdulequery)){ 
																		?>
																			<tr>
