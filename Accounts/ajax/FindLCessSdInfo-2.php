...
if($Code == "LCESS"){
	...
	if($Type == "ALL"){
		$LCessAmt = 0;
		$SelectQuery1 = "select a.*, b.*, c.*, c.name_contractor as cont_name, a.rbn as bill_rbn from memo_payment_accounts_edit a inner join sheet b on (a.sheetid = b.sheet_id) inner join contractor c on (b.contid = c.contid) where a.payment_dt >= '$FromDate' and a.payment_dt <= '$ToDate' order by payment_dt asc";
		$SelectSql1   = mysqli_query($dbConn,$SelectQuery1);
