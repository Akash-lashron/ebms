...
$rs_workorder=mysqli_query($dbConn,$sql_workorder);
/*$workorder_no=@mysqli_result($rs_workorder,0,'work_order_no');

$worknames=@mysqli_result($rs_workorder,0,'work_name');
...
$rs_itemno=mysqli_query($dbConn,$sql_itemno);

$id='group2';
$desc='group2';*/
while($rows=mysqli_fetch_assoc($rs_workorder))
{
	/*$id=$id . '*' . $rows['rbn'];
	$desc=$desc . '*' . $rows['rbn'];*/
	$rbn .= $rows['rbn']."*"; 
