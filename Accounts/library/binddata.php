...
class BindList
{
	...
	{   
		...
		$SelectSCodeQuery 	= "SELECT a.*, b.* FROM hoa_master a INNER JOIN shortcode_master b ON (a.shortcode_id = b.shortcode_id OR FIND_IN_SET(b.shortcode_id,a.shortcode_id)) WHERE a.fin_year ='$FinYear' AND a.active = 1 ORDER BY b.shortcode_desc ASC";
		$SelectSCodeSql 	= mysqli_query($dbConn,$SelectSCodeQuery);
		if($SelectSCodeSql == true ){
			while($row = mysqli_fetch_array($SelectSCodeSql)){
	...
	public function BindHoaWithFullCode($HoaId)
	{   
		global $dbConn;
		$CurrYear = date('Y');
		$NextYear = date('Y', strtotime('+1 year'));
		...
		if($SelectSCodeSql == true ){
			while($row = mysqli_fetch_array($SelectSCodeSql)){
				...
				$SCodes .=  '<option value="'.$row['hoamast_id'].'" data-scodeid="'.$row['shortcode_id'].'" data-hoa="'.$row['new_hoa_no'].'" '.$sel.'>'.$row['new_hoa_no'].'</option>';     
			}            
		}
		return $SCodes;     
	}
	...
	public function BindCCNoListAccountsPo($workordernolistvalue){
   		global $dbConn;
		$sheetquery = "SELECT sheet_id, work_name, short_name, assigned_staff, computer_code_no FROM sheet WHERE (active = 1 OR active = 2) AND under_civil_sheetid = 0 AND work_name != '' ORDER BY CAST(computer_code_no AS UNSIGNED) ASC";
		$sheetsqlquery = mysqli_query($dbConn,$sheetquery);
		if ($sheetsqlquery == true ){
			while($row = mysqli_fetch_array($sheetsqlquery)){
				if ($workordernolistvalue == $row['sheet_id']){
	...
	public function BindCCNoMopStmt($workordernolistvalue){
   		global $dbConn;
		/*$sheetquery = "SELECT a.sheet_id, a.work_name, a.short_name, a.assigned_staff, a.computer_code_no, b.sheetid FROM sheet a 
		...
		$sheetquery = "SELECT a.sheet_id, a.work_name, a.short_name, a.assigned_staff, a.computer_code_no FROM sheet a 
					   WHERE a.active = '1' AND a.under_civil_sheetid = 0 AND a.work_name != '' 
					   ORDER BY CAST(a.computer_code_no AS UNSIGNED) ASC";
		$sheetsqlquery = mysqli_query($dbConn,$sheetquery);
		if ($sheetsqlquery == true ){
			while($row = mysqli_fetch_array($sheetsqlquery)){
				...
				$sheet .=  '<option value="'. $row['sheet_id'].'"'.$sel.'>'.$row['computer_code_no'].' - '.$row['work_name'].'</option>';
			}            
		}
		return $sheet;            
	} 
	...
	public function BindPGReturnTrNo($TrId)
	{
		global $dbConn;
		$TrArr = array();
		$sheettablequery = "SELECT DISTINCT globid FROM bg_fdr_details WHERE inst_status != 'R' AND inst_purpose = 'PG' ORDER BY globid ASC";
		$sheetNegoquerysql = mysqli_query($dbConn,$sheettablequery);
		if ($sheetNegoquerysql == true )
		{
			while($row1 = mysqli_fetch_array($sheetNegoquerysql))
		...
		$ImplodeTrArr = implode(',',$TrArr);
		if(($ImplodeTrArr != NULL)&&($ImplodeTrArr != '')){
			$sheetquery = "SELECT * FROM tender_register WHERE loi_issue = 'Y' AND globid IN (".$ImplodeTrArr.") ORDER BY tr_id ASC";
			$sheetsqlquery = mysqli_query($dbConn,$sheetquery);
			if ($sheetsqlquery == true )
			{
				while($row = mysqli_fetch_array($sheetsqlquery))
				{
					...
					$sheet .=  '<option value="'. $row['tr_id'].'" '.$sel.'>'.$row['ccno'].' - '.$row['work_name'].'</option>';
				}
			}
		}
		return $sheet;
	...
	/*public function BindSDReturn($workordernolistvalue)
	{
		global $dbConn;
		$TrArr = array();
		$sheettablequery = "SELECT DISTINCT globid FROM bg_fdr_details WHERE inst_status != 'R' AND inst_purpose = 'SD' ORDER BY globid ASC";
		$sheetNegoquerysql = mysqli_query($dbConn,$sheettablequery);
		if ($sheetNegoquerysql == true )
		{
			while($row1 = mysqli_fetch_array($sheetNegoquerysql))
		...
		$ImplodeTrArr = implode(',',$TrArr);
		if(($ImplodeTrArr != NULL)&&($ImplodeTrArr != '')){
			$sheetquery = "SELECT sheet_id,short_name,computer_code_no FROM sheet WHERE active = 1 AND globid IN (".$ImplodeTrArr.") ORDER BY CAST(computer_code_no AS UNSIGNED INTEGER) ASC";
    		$sheetsqlquery = mysqli_query($dbConn,$sheetquery);
    		if ($sheetsqlquery == true ) {
    			while($row = mysqli_fetch_array($sheetsqlquery))
    			{
	...
	public function BindEMDReturnTrNo($TrId)
	{
		...
		$ImplodeTrArr = implode(',',$TrArr);
		if(($ImplodeTrArr != NULL)&&($ImplodeTrArr != '')){
			$sheetquery = "SELECT tr_id,tr_no FROM tender_register WHERE loi_issue = 'Y' AND globid IN (".$ImplodeTrArr.") ORDER BY tr_id ASC";
			$sheetsqlquery = mysqli_query($dbConn,$sheetquery);
			if ($sheetsqlquery == true )
			{
				while($row = mysqli_fetch_array($sheetsqlquery))
	...
	public function BindWorksForClose($workordernolistvalue)
	{
		global $dbConn;
		$SheetIdArr = array();
		$SelectQuery1 = "SELECT DISTINCT rbn FROM measurementbook_temp";
		$SelectSql1 = mysqli_query($dbConn, $SelectQuery1);
		if($SelectSql1 == true ){
			if(mysqli_num_rows($SelectSql1)>0){
				while($List = mysqli_fetch_object($SelectSql1)){
					array_push($SheetIdArr,$List->sheetid);
				}
			}
		}
		if(count($SheetIdArr)>0){
			$SheetIdStr = implode(",",$SheetIdArr);
		}else{
			$SheetIdStr = "";	
		}
		
		$sheetquery = "SELECT * FROM sheet WHERE active = 1 AND sheetid IN ($SheetIdStr) ORDER BY CAST(computer_code_no AS UNSIGNED INTEGER) ASC";
		$sheetsqlquery = mysqli_query($dbConn, $sheetquery);
		if ($sheetsqlquery == true )
		{
			while($row = mysqli_fetch_array($sheetsqlquery))
