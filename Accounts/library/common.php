...
$SQLQuery = mysqli_query($dbConn,$Query);
    if ($SQLQuery == true) {
        $subdiv = mysqli_fetch_object($SQLQuery);
...
$shortnotes_query = mysqli_query($dbConn,$shortnotes_sql);
if(mysqli_num_rows($shortnotes_query)>0)
{
	 $scheduleshortnotes= mysqli_fetch_object($shortnotes_query);
...
else
{
	...
	$SQLQuery = mysqli_query($dbConn,$Query);
		if ($SQLQuery == true) {
			 $scheduledescription= mysqli_fetch_object($SQLQuery);
...
$SQLQuery = mysqli_query($dbConn,$Query);
    if ($SQLQuery == true) {
         $runnaccbill= mysqli_fetch_object($SQLQuery);
...
$SQLQuerys = mysqli_query($dbConn,$Querys);
    if ($SQLQuerys == true) {
         $scheduledescriptions= mysqli_fetch_object($SQLQuerys);
...
$AbstSQLQuerys = mysqli_query($dbConn,$AbstQuerys);
    if ($AbstSQLQuerys == true) {
         $AbsSQLQuerys= mysqli_fetch_object($AbstSQLQuerys);
...
function getscheduledescription_new($id)
{
	...
	$select_desc_query = mysqli_query($dbConn,$select_desc_sql);
	$result = mysqli_fetch_object($select_desc_query);
...
function getItemDetails($sheetid,$subdivid)
{
	...
	$Itemdetails_query 	= 	mysqli_query($dbConn,$Itemdetails_sql);
	$ItemData			=	mysqli_fetch_object($Itemdetails_query);
...
function getdivisionname($sheetid,$divid)
{
	...
	$select_div_query = mysqli_query($dbConn,$select_div_sql);
	$result1 = mysqli_fetch_object($select_div_query);
	...
	$select_desc_query = mysqli_query($dbConn,$select_desc_sql);
	$result2 = mysqli_fetch_object($select_desc_query);
...
function getzonename($sheetid,$zone_id)
{
	...
	$select_zonename_query = mysqli_query($dbConn,$select_zonename_sql);
	$result1 = mysqli_fetch_object($select_zonename_query);
...
function getsheetdata($sheetid)
{
	...
	$select_sheet_query = mysqli_query($dbConn,$select_sheet_sql);
	$result1 = mysqli_fetch_object($select_sheet_query);
...
function GetRoleName($levelid,$sectionid){
	...
	$select_role_name_sql	=	mysqli_query($dbConn,$select_role_name_query);
	if($select_role_name_sql == true){
		if(mysqli_num_rows($select_role_name_sql)>0){
			$List 	= mysqli_fetch_object($select_role_name_sql);
...
function AccVerificationCheck($sheetid,$rbn,$mbookno,$genlevel,$levelid,$type)// Already Level 2 checked and Again The button will not enable.
{
	...
	$select_check_sql 	= mysqli_query($dbConn,$select_check_query);
	$result1 			= mysqli_fetch_object($select_check_sql);
	if($select_check_sql == true)
	{
		if(mysqli_num_rows($select_check_sql)>0)
		{
			$check = 1;
		}
		else
...
function getSuppAggNoOf($sheetid,$supp_sheet_id)
{
	...
	$select_supp_query = mysqli_query($dbConn,$select_supp_sql);
	$result1 = mysqli_fetch_object($select_supp_query);
...
function getSecuredAdvanceAmt($sheetid,$rbn)
{
	...
	$result1 = mysqli_fetch_object($select_sql);
...
function getEscalationAmt($sheetid,$rbn)
{
	...
	$select_sql1 = mysqli_query($dbConn,$select_query1);
	while($result1 = mysqli_fetch_object($select_sql1)){
	...
	$select_sql2 = mysqli_query($dbConn,$select_query2);
	while($result2 = mysqli_fetch_object($select_sql2)){
...
function getICNO($staffid)
{
	...
	$select_icno_query = mysqli_query($dbConn,$select_icno_sql);
	$result1 = mysqli_fetch_object($select_icno_query);
...
function getWorkOrderValue($sheetid)
{
	...
	$select_soq_sql		= mysqli_query($dbConn,$select_soq_query);
	if($select_soq_sql == true){
		while($List = mysqli_fetch_object($select_soq_sql)){
...
function AccountsLevelStatus($sheetid,$rbn,$mbookno,$zone_id,$mtype,$genlevel){
	...
	$SABVeriCheck = 0;
	$ABSTVeriCheck = 0;
	//echo $sheetid."*".$rbn."*".$mbookno."*".$zone_id."*".$mtype."*".$genlevel;exit;
	...
	$select_al_as_query 	= "SELECT * FROM acc_log WHERE sheetid = '$sheetid' AND rbn = '$rbn' ORDER BY logid ASC";
	$select_al_as_sql 		= mysqli_query($dbConn,$select_al_as_query);
	if($select_al_as_sql == true){
		while($ALASList 	= mysqli_fetch_object($select_al_as_sql)){
			$ACLevel 	= $ALASList->levelid;
			$ACStatus 	= $ALASList->AC_status;
			...
			if(($ALASList->mbookno == $mbookno)&&($ALASList->zone_id == $zone_id)&&($ALASList->mtype == $mtype)&&($ALASList->genlevel == $genlevel)){
				//	echo $ALASList->logid." -> ".$ALASList->mbookno." - ".$mbookno." - ".$ALASList->zone_id." - ".$zone_id." - ".$ALASList->mtype." - ".$mtype." - ".$ALASList->genlevel." - ".$genlevel."</br>";
				array_push($AlStatus,$ACLevel);
				array_push($AlStatus,$ACStatus);
			...
			if(($EndLevel == $MinLevel)&&($EndLevel != $MaxLevel)){ //NO
				if($Status == "R"){
					...
					if($EndLevel == $CurrLevel){
						return "<span class='btn1 btn1-default btn1-sm' style='height:15px; padding:4px 10px; line-height:1.2;'><font style='color:red; font-size:11px; font-weight:bold;' aria-hidden='true'>X</font> Not yet verified</span>";
					}else{
						return "<span class='btn1 btn1-default btn1-sm' style='height:15px; padding:4px 10px; line-height:1;'><i class='fa fa-check' style='color:#14AF52; font-size:19px;' aria-hidden='true'></i> Forwared to Next Level</span>";
					}
				}
	...
	if($Modify == 1){
		...
		if($SelectSql2 == true){
			if(mysqli_num_rows($SelectSql2)>0){
				$Modify = 0;
				while($List = mysqli_fetch_object($SelectSql2)){
					$MbAc 	= $List->mb_ac;
					$SaAc 	= $List->sa_ac;
					$AbAc 	= $List->sa_ac;
					if($MbAc == "SC"){ $Modify = 1; }
