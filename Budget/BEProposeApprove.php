...
include "common.php";
checkUser();
$PageName = $PTPart1.$PTIcon.'Budget Expenditure Proposal & Approval';
...
$SelectQuery1 = "SELECT * FROM hoa WHERE active = 1";
...
if((isset($_POST['btn_prop']) == ' Proposed ')||(isset($_POST['btn_appr']) == ' Approved ')){
	...
	foreach($HeadofAccArr as $key => $Value){
		...
		$UpdateDetailsQuery = "UPDATE hoa_be SET fin_year = '$FinYear', hoa_no = '$HeadofAcc', pin_id  = '$PinId', 
		pin_no = '$PinNo', be_prop_amt  = '$BudValue', status  = '$PropFlag', userid  = '$UserId', active = '1' WHERE hbeid='$HoaBeId'";

		$UpdateDetailsSql 	= mysqli_query($dbConn, $UpdateDetailsQuery);
	}
	if($UpdateDetailsSql == true){
		if($PropFlag == 'P'){
			$msg = "Proposed successfully.";
		}else{
			$msg = "Approved successfully.";
		}
	}else{
		if($PropFlag == 'P'){
			$msg = "Error : Not Proposed. Please try again..!!";
		}else{
			$msg = "Error : Not Approved. Please try again..!!";
		}
	}
}
...
if(isset($_POST['btn_save']) == ' Save '){
	...
	foreach($HeadofAccArr as $key => $Value){
		...
		$InsertDetailsQuery = "insert into hoa_be set hoa_id = '$HoaId', fin_year = '$FinYear', hoa_no = '$HeadofAcc', pin_id  = '$PinId', 
		pin_no = '$PinNo', be_prop_amt  = '$BudValue', status  = 'P', userid  = '$UserId', active = '1'";
		$InsertDetailsSql 	= mysqli_query($dbConn, $InsertDetailsQuery);
	}
	if($InsertDetailsSql == true){
		$msg = "Created successfully.";
	}else{
		$msg = "Error : Not created. Please try again.";
	}
}
							...
							<form name="form" method="post" action="PartPaymentPageView.php">
								...
								<div class="div12">
									...
									<div class="div8 card-div-body">
										<div class="row divhead head-b" align="center">HOA Budget estimated value Entry</div>
										<div class="top-card">
											...
											<div class="top-card-container">
												...
												<div class="div12">
													...
													<div class="div3 cboxlabel" align="center">Financial Year</div>
													<div class="div3">
...
<script>
	...
	$("body").on("change", "#cmb_fin_year", function(event) {
		...
		$.ajax({
			...
			data: { 'selyear': selyear, 'page':'BEHOA' }, 
			dataType: 'json',
			success:function(data){ 	//alert(JSON.stringify(data));
				...
				$.each(HData, function(index, element) {	
					...
					CourseLoadStr += 	"<div class='div3'>";
					if(element.be_prop_amt == null){
						CourseLoadStr +=	"<input type='text' name='txt_bud_val[]' id='txt_bud_val' class='card-label-tbox-lg' value=''>";
					}else{
				...
				if(FlagDat == 1){
					...
					Btnappend += 	"<div class='buttonsection' style='display:inline-table'>";
					Btnappend += 	"<input type='submit' class='btn btn-info' data-type='submit' value=' Save ' name='btn_save' id='btn_save'/>";
					Btnappend += 	"</div>&nbsp;&nbsp;&nbsp;";
				}
