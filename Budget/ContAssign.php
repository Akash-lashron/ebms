...
include "common.php";
$PageName = $PTPart1.$PTIcon.'Contractor Assign';
checkUser();
$success = 0;
...
if(isset($_POST['btn_save'])){
	...
	$StaffQuery = "select * from contractor where active = 1";
	...
	if ($StaffQuerySql == true ) {
		if(mysqli_num_rows($StaffQuerySql) > 0){
			while($StaffRow = mysqli_fetch_array($StaffQuerySql)){
				$TotStaffListArr[$StaffRow['contid']] = $StaffRow['name_contractor'];
			}
		}
	}
	...
	foreach($CcnoArr as $Key => $Value){
		...
		if($SelStaffId != ""){
			if(($Value != 0)&&($Value != NULL)&&($Value != "")){
				$UpdateQuery = "UPDATE sheet SET contid='$SelStaffId', name_contractor='$SelStaffName' WHERE computer_code_no = '$Value'";
				$UpdateQuerySql = mysqli_query($dbConn,$UpdateQuery);
				$UpdateworkQuery = "UPDATE works SET contid='$SelStaffId', name_contractor='$SelStaffName' WHERE ccno = '$Value'";
	...
	if($UpdateQuerySql == true){
		...
		$msg = "Error...Contractor Details Not Saved..!!!";
	}
}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center"> Contractor Selection </div>
																...
																<table width="100%" align="center" class="dataTable table2excel mgtb-8">
																	<thead>
																		<tr>
																			...
																			<th valign="middle"> Select / Assign Contractor </th>
																			<th valign="middle"> Contractor Assigned Status </th>
																		</tr>
																	</thead>
																	<tbody>
																		<tr class='labeldisplay'>
																				...
																				if($LiveWorksResultSql == true){
																					if(mysqli_num_rows($LiveWorksResultSql)>0){
																						while($WrkList = mysqli_fetch_object($LiveWorksResultSql)){
																							...
																							if(($WrkList->name_contractor == '')||($WrkList->name_contractor == NULL)){
																								$ContNameVal 	= $WrkList->name_contractor_old;
																							}else{
																								$ContNameVal 	= $WrkList->name_contractor;
																							}
																							
																							$SelContVal 	= $WrkList->contid;
																							//$splitCreateDateTimeStamp = explode(" ",$CreateDateTime);
																							//$CreateDate = $splitCreateDateTimeStamp[0];
																							//echo $CreateDate;
																							if(($SelContVal == 0) || ($ContNameVal == "")){
																			?>
																			...
																			<td valign='middle' class='tdrow' align = 'justify'>
																				<?php echo $ContNameVal; ?>
																				<input type="hidden" name="txt_hid_eicname[]" id="txt_hid_eicname" value="<?php echo $ContNameVal; ?>">
																			</td>
																			<td align='center' valign='middle' class='tdrow' style="width:18%;">
																				<!-- <select name="" id="" class="tboxsmclass"> -->
																				<select name="cmb_assigned_eic[]" id="cmb_assigned_eic" class="tboxsmclass ContList" >
																					<option value="">--- Select ---</option>
																					<option value=""><?php echo $objBind->BindCont($SelContVal); ?></option>
																				</select>
																			</td>
																			<?php if(($SelContVal != 0) && ($ContNameVal != "")){ ?>
			...
			BootstrapDialog.show({
				...
				buttons: [{
					...
					action: function(dialog) {
						dialog.close();
						window.location.replace('ContAssign.php');
					}
				}]
			});
		}
