...
if(isset($_POST["confirm"]) == " CONFIRM "){
	...
	if(count($ItemDescArr)>0){
		...
		$InsertQuery1 	= "insert into partab_master set ref_no = '$RefNo', tr_id = '$TrId', globid = '$GlobID',  work_name = '$NameWork', partA_amount = '$TotAmt', created_date = NOW(), created_by = '$staffid', is_confirmed = 'Y', confirmed_by = '$staffid', confirmed_on = NOW()";
																		...
																		if($SheetName == $ExcelSheetName)
																		{
																			...
																			foreach ($Spreadsheet as $Key => $Row) { // loop used to get each row of the sheet
																				...
																				<tr class='labeldisplay'>
																					...
																					<td valign='middle' class='tdrow' align="right"><?php if(($ItemAmtFile != 0)&&($ItemAmtFile != NULL)&&($ItemAmtFile != '')){ echo IndianMoneyFormat($ItemAmtFile); } ?><input type="hidden" name="txt_item_amt[]" value="<?php echo $ItemAmtFile; ?>"></td>
																			   </tr>
																				<?php
