...
if(isset($_POST['submit']) == " Save "){
	...
	if($Exe > 0){
		$msg = "EMD Entry Successfully Saved..!!";
		UpdateWorkTransaction($GlobInTsID,0,0,"W","EMD details Updated by ".$_SESSION['staffname']."","");
	}else{
		$msg = "Error: EMD Entry Not Saved..!!";
		UpdateWorkTransaction($GlobInTsID,0,0,"W","EMD details Tried to Saved by ".$_SESSION['staffname']." but not Updated","");
	}
}
if(isset($_GET['id'])){   
	$TRId 	 = $_GET['id'];
...
$RowCount = 0; 
$FinaQuery = "SELECT tr_id FROM bidder_bid_master WHERE tr_id = '$TRId'";
												...
												<div class="card-body padding-1 ChartCard" id="CourseChart">
													<div class="divrowbox pt-2">
																...
																<div class="row">
																	...
																	<div class="div6">
																		<input type='hidden' readonly="" name='txt_full_emd_amt' id='txt_full_emd_amt' class="tboxsmclass" value="<?php if(isset($_GET['id']) != ""){ echo $EMdamtt; } ?>">
																		<input type='text' readonly="" name='txt_full_emd_amt_indform' id='txt_full_emd_amt_indform' class="tboxsmclass" value="<?php if(isset($_GET['id']) != ""){ echo IndianMoneyFormat($EMdamtt); } ?>">
																		<input type="hidden" name="txt_tender_id" id="txt_tender_id" maxlength="50" class="tboxsmclass" style="width:99%" value="<?php if(isset($_GET['id']) != ""){ echo $TRId; } ?>">
																		<input type="hidden" name="txt_emdmas_id" id="txt_emdmas_id" maxlength="50" class="tboxsmclass" style="width:99%" value="<?php if(isset($_GET['id']) != ""){ echo $Emdid; } ?>">
																	</div>
																</div>
...
<option value="">---- Select ----</option>
																	...
																	<tr id="<?php echo $Index; ?>">
																		...
																		<td align="center"><input type="text" placeholder="DD/MM/YYYY" class="tboxsmclass date EmdDt"  name="txt_date_pg[]" id="txt_date_pg_<?php echo $Index; ?>" data-index="<?php echo $Index; ?>" value="<?php echo dt_display($Result2->issue_dt);?>"></td>
																		...
																		<td>
																			<input type="button"  class="delete btn btn-info" name="emp_delete" id="emp_delete" data-id="<?php echo $Detailid; ?>" data-index="<?php echo $Index; ?>" value="DELETE" />
																		</td>  
																	</tr>
																	<?php
																		$Index++;
																...
																<table class="dataTable etable table-responsive " align="center" width="100%" id="emdtable2">
																	...
																	<tr id="<?php echo $DDIndex; ?>">
																		...
																		<td align="center"><input type="text" placeholder="DD/MM/YYYY" class="tboxsmclass date EmdDt ValDDDate"  name="txt_date_pg_DD[]" id="txt_date_pg_DD_<?php echo $DDIndex; ?>" data-DDindex="<?php echo $Index; ?>" value="<?php echo $DDdate;?>"></td>
																		<td align="center"><input type="text"  placeholder="DD/MM/YYYY" class="tboxsmclass expdate ExpDt ValDDDate" name="txt_expir_date_pg_DD[]" id="txt_expir_date_pg_DD_<?php echo $DDIndex; ?>" data-DDindex="<?php echo $Index; ?>" value="<?php echo $DDexdate;?>"></td>
																		<td align="center"><input type="text" style="text-align:right;" maxlength="12" class="tboxsmclass EmdAmt<?php echo $contid; ?>"  data-id="<?php echo $contid; ?>" data-DDindex="<?php echo $DDIndex; ?>"  onKeyPress="return isNumberWithTwoDecimal(event,this);" name="txt_part_amt_DD[]" id="txt_part_amt_DD_<?php echo $DDIndex; ?>" value="<?php  echo $AmountList?>"></td>
																		...
																		<td><input type="button" class="DDdelete btn btn-info" name="emp_DD_delete" id="emp_DD_delete" data-id="<?php echo $Detailid; ?>" data-DDindex="<?php echo $DDIndex; ?>" value="DELETE" /></td>
																	</tr>
																	<?php
																		$DDIndex++;
																				}
...
$( ".date" ).datepicker({  
	...
	$(document).ready(function(){ 
		...
		$("body").on("change","#cmd_instype_0", function(event){
		var type = $(this).val();
		var dd=[];
				$("input[name='cmd_instype[]']").each(function(){  
					dd.push($(this).val());
					});
				if(type==dd && dd=="BG"){
				BootstrapDialog.alert(" BG is already exists");
		...
		/*
		$("body").on("change","#cmb_bidder_DD_0", function(event){
		//var type = $(this).val();
		var type   	 = $("#cmb_bidder_DD_0 option:selected").text(); 
		var rowCount     = $('#emdtable1 tr').length; 
		var rowCount2     = $('#emdtable2 tr').length; 
		var dd=[];
				$("input[name='cmb_bidder_DD[]'],input[name='cmb_bidder[]']").each(function(){  
					dd.push($(this).val());
					});
				if(type!=dd && (rowCount>2 || rowCount2>2)){
				BootstrapDialog.alert(" Bidder is not same");
					event.preventDefault();
					event.returnValue = false;
					$(this).val('');
		}
		});
		$("body").on("change","#cmb_bidder_0", function(event){
			
		var type1 = $(this).val();
		var type   	 = $("#cmb_bidder_0 option:selected").text(); 
		var dd=[];
		var rowCount      = $('#emdtable1 tr').length; 
		var rowCount2     = $('#emdtable2 tr').length; 
		
				$("input[name='cmb_bidder[]'],input[name='cmb_bidder_DD[]']").each(function(){  
					dd.push($(this).val());
					});
		...
		$("body").on("click", "#emp_add", function(event){ 
			...
			var ContName1   	 = $("#cmb_bidder_0").val();
			...
			if(ContName1 == ''){
				BootstrapDialog.alert("Bidder's Name should not be empty");
				return false;
			}else if(InstType == 0){
		...
		$("body").on("click", "#emp_DD_add", function(event){ 
			...
			var ContName1   	 = $("#cmb_bidder_DD_0").val();
			...
			/*
				...
				BootstrapDialog.alert("DraweeBank should not be empty");
			 	return false;
			}*/
			else{
...
$("input[name='cmd_contid[]']").each(function(){  
				...
				var cl=ContArr.length; 
				var cl_dd=ContDDArr.length;
				var ct=0;
				if(cl<cl_dd){ ct=cl_dd; }else{ ct=cl; }
				...
				for (var i = 0; i < ct; i++) {
					...
					$(".EmAmt"+EmdContId).each(function(){
						var EmdAmt = $(this).val();  
						if(EmdAmt==null){ EmdAmt=0; }
						ContEmdTotAmt = parseFloat(ContEmdTotAmt) + parseFloat(EmdAmt); 
					});
					 $(".EmdAmt"+DDContId).each(function(){
					 	var DDAmt = $(this).val(); 
						if(DDAmt==null){ DDAmt=0; }
						ContDDTotAmt = parseFloat(ContDDTotAmt) + parseFloat(DDAmt); 
					 });
					//alert(ContDDTotAmt);
					var contamt = ContEmdTotAmt + ContDDTotAmt; 
...
//-----------
				...
				/*else if(rowCount <= 2 ) {
				 ...
				 else if((rowCount <= 2) && (rowCount2<=2) ) {
					BootstrapDialog.alert(" Please Add Atleast One EMD Detail..!!");
					event.preventDefault();
					event.returnValue = false;
				}
				else if((rowCount == 3) && (rowCount2==2) && (dd=="BG") ) {
					BootstrapDialog.alert(" Please Add one more EMD Detail for BG..!!");
					event.preventDefault();
					event.returnValue = false;
				}
				else if(ContErr == 1){
					BootstrapDialog.alert(" Total EMD Amount should be greater than or equal to "+EmdAmount+" for the bidder "+ErrContName);
				//	BootstrapDialog.alert(" Total EMD Amount should be greater than or equal to "+EmdAmount+" );
					event.preventDefault();
					event.returnValue = false;
				}else{
					event.preventDefault();
