...
if($_SESSION['isadmin'] == 1){
	$MasterQuery = "SELECT * FROM emd_master INNER JOIN emd_detail ON emd_master.emid = emd_detail.emid INNER JOIN tender_register ON emd_master.tr_id = tender_register.tr_id 
	INNER JOIN contractor ON emd_detail.contid = contractor.contid ORDER BY emd_master.tr_id ASC, emd_detail.contid ASC";
}else{
	$MasterQuery = "SELECT * FROM emd_master INNER JOIN emd_detail ON emd_master.emid = emd_detail.emid INNER JOIN tender_register ON emd_master.tr_id = tender_register.tr_id 
	INNER JOIN contractor ON emd_detail.contid = contractor.contid WHERE (tender_register.eic = '$staffid' OR tender_register.created_by = '$UserId') ORDER BY emd_master.tr_id ASC, emd_detail.contid ASC";
}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">&nbsp;EMD - Register</div>
												<div class="card-body padding-1 ChartCard" id="CourseChart">
													<div class="divrowbox pt-2">
														<div class="table-responsive dt-responsive ResultTable">
															<div class="table-responsive dt-responsive rtabdiv" id="table-stmt">
																<table id="example" class="display rtable mgtb-8 table2excel" style="width:100%">
																	<thead>
																		<tr>
																			<th rowspan="2" valign="middle">SNo.</th>
																			<th rowspan="2" valign="middle">Technical Sanction No.</th>
																			...
																			if($RowCount == 1){ foreach($EmDDataArr as $Emdkey => $EmdValue){ 
																				...
																				?>
																					<tr class='labeldisplay'>
																						...
																						<td rowspan="<?php echo $TrRowspan; ?>" valign='middle' class='tdrow' align = 'justify'><?php echo $TsNumArr[$EmdValue->tr_id]; ?></td>
																						...
																						<?php if($EmdValue->status == 'R'){ ?>
																							<td class='tdrow' align='right' valign='middle' nowrap='nowrap'><?php echo "Released"; ?>&nbsp;<i class="fa fa-check-circle-o" style="font-size:20px; color:#046929;"></i></td>
																						<?php }else{ ?>
																							<td class='tdrow' align='right' valign='middle' nowrap='nowrap'></i><?php echo "Not-Released"; ?>&nbsp;<i class="fa fa-times-circle" style="font-size:20px; color:#EA253C;"></i></td>
																						<?php } ?>
																					</tr>
																				<?php 
																					$x++; $y++;  $SNO++;
	...
	$("#exportToExcel").click(function(e){ 
		...
		if(table.length){ 
			$(table).table2excel({
				...
				filename: "EMD Register-" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
				fileext: ".xls",
				exclude_img: true,
				exclude_links: true,
				exclude_inputs: true
