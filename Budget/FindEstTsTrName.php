...
if($Page == "TS"){
	...
	if($SelectSql2 == true){
		if(mysqli_num_rows($SelectSql2)>0){
			$List2 = mysqli_fetch_assoc($SelectSql2);
			$OutputArr = $List2;
			$OutputArr['ExcelItemNoCol'] 	 = '';
			$OutputArr['ExcelItemDescCol'] = '';
			$OutputArr['ExcelItemQtyCol']  = '';
			$OutputArr['ExcelItemUnitCol'] = '';
			$OutputArr['ExcelItemRateCol'] = '';
			$OutputArr['ExcelItemAmtCol']  = '';
			if(isset($_SESSION['FinBidColArr'])){
				$FinBidColArr = $_SESSION['FinBidColArr'];
				if($EstTsTrId == $FinBidColArr['TR']){
					$OutputArr['ExcelItemNoCol'] 	 = $FinBidColArr[0];
					$OutputArr['ExcelItemDescCol'] = $FinBidColArr[1];
					$OutputArr['ExcelItemQtyCol']  = $FinBidColArr[2];
					$OutputArr['ExcelItemUnitCol'] = $FinBidColArr[3];
					$OutputArr['ExcelItemRateCol'] = $FinBidColArr[4];
					$OutputArr['ExcelItemAmtCol']  = $FinBidColArr[5];
				}
			}
		}
	}
	...
	/* Commented on 15-11-2023 
		...
		/*	Commented on 02/12/2024
		$SelectQuery7	= "SELECT a.staffid,a.staffname,a.staffcode,b.designationname,a.section_name FROM staff a INNER JOIN designation b ON (a.designationid = b.designationid)
		 WHERE a.staffid = '$EicID'";
		 */
		$SelectQuery7	= "SELECT a.staffid, a.staffname, a.staffcode, b.designationname, a.sectionid, c.section_name FROM staff a
			INNER JOIN designation b ON a.designationid = b.designationid
			LEFT JOIN section_name c ON a.sectionid = c.secid
			WHERE a.staffid = '$EicID'";
		$SelectQuery7Sql = mysqli_query($dbConn,$SelectQuery7);
		if(mysqli_num_rows($SelectQuery7Sql)>0){
			$List8 = mysqli_fetch_array($SelectQuery7Sql);
