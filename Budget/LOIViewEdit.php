...
include "common.php";
$PageName = $PTPart1.$PTIcon.'LOI View';
checkUser();
$success = 0;
...
if($_SESSION['isadmin'] == 1){
	...
	$result = "SELECT a.*,  b.tr_no, b.work_name, b.ccno, c.name_contractor FROM loi_entry a 
		INNER JOIN tender_register b ON (a.tr_id = b.tr_id) 
		INNER JOIN contractor c ON (a.contid = c.contid) WHERE 
		(b.eic = '".$_SESSION['sid']."' OR b.created_by = '".$_SESSION['userid']."') 
		ORDER BY a.loa_pg_id ASC";
}	
//echo $result;exit;	
...
if($SelectSql1 == true){
	if(mysqli_num_rows($SelectSql1)>0){
		while($List = mysqli_fetch_object($SelectSql1)){
			...
			/* commented on 27-10-2023
			$RebateAmt = $QuoteAmt * $RebatePerc / 100;
			$TotalQuote = round(($QuoteAmt - $RebateAmt),0);
			
			if($RebatePerc==0 || $RebatePerc==null){
			$TotalQuote =$List->quoted_amt_af_reb;	
			}
			...
			if($NegoRebatePerc==0 || $NegoRebatePerc==null || $NegoRebatePerc=="0.00"){
...
if($IsNego == 'Y'){
				$QuoteAmtArr[$TrId][$ContId]=  $List->quoted_amt_af_neg;
			}else{
				$QuoteAmtArr[$TrId][$ContId]=  $List->quoted_amt_af_reb;
			}			
		}
	}
}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">LOI - View</div>
																...
																<table width="100%" align="center" class="dataTable table2excel mgtb-8">
																	<thead>
																		<tr>
																			...
																			<th valign="middle" nowrap="nowrap">Tender No.</th>
																	...
																	<tbody>
																		<tr class='labeldisplay'>
																			...
																			<?php } ?><a data-url="LOIEntry?id=<?php echo $List->loa_pg_id; ?>" class=" BtnHref btn btn-info" name="View" id="View"><?php echo $BtnName; ?></a></td>
																		</tr>
																			<?php 
																			 		$SNO++; 
																				} 
