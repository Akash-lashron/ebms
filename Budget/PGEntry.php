...
if(isset($_POST['btn_save']) == " Save "){ 
		...
		//	if($Emdinstnumstr != null){
				...
				if(count($DDinstnumstr)>0){
					foreach($DDinstnumstr as $Key => $Value){
						...
						/*
						$insert_query1	= "insert into bg_fdr_details set master_id='$LOiID',globid='$GlobID', contid='$Contractorid', inst_purpose='PG', inst_branch_name='$TrimBranc', inst_type='DD',inst_serial_no='$TrimInstnum', inst_bank_name='$TrimBankname',
						inst_date='$Insertdate', inst_exp_date='$InsertExpdate',  inst_amt='$TrimAmount', userid='$UserId', inst_status='EIC', createdby='$staffid',  created_section='EIC',  createdon= NOW() ,active='1'"; 
						*/
						$insert_sql1 = mysqli_query($dbConn,$insert_query1);
	...
	if(isset($_GET['id'])){   
		...
		$result =  "SELECT a.*, b.*, c.tr_no, c.work_name, c.ccno, c.eic, d.name_contractor FROM loi_entry a 
						LEFT JOIN bg_fdr_details b ON (a.loa_pg_id = b.master_id) 
						LEFT JOIN tender_register c ON (a.tr_id = c.tr_id) 
						LEFT JOIN contractor d ON (a.contid = d.contid) 
						WHERE b.inst_purpose='PG' AND b.userid='$UserId' AND a.loa_pg_id='$LOIPGID'
																	   ...
																	   <div class="face-static">
																			...
																			<div class="card-body padding-1">
																				<table class="dataTable etable " align="center" width="100%" id="pgtable1">
																					...
																					<tr>
																					 ...
																					 <?php
																					$Index = 1;
																					$SelectQuery2 = "SELECT * FROM bg_fdr_details where master_id != 0 AND master_id='$LOIPGID' AND inst_type !='DD' AND inst_purpose='PG' order by master_id asc";
																			...
																			<table class="dataTable etable table-responsive " align="center" width="100%" id="emdtable2">
																				...
																				<!-- For Update Function -->
																					<?php
																						$DDIndex = 1;
																								$SelectQuery3 ="SELECT * FROM bg_fdr_details where master_id != 0 AND master_id='$LOIPGID' AND inst_type ='DD' AND inst_purpose='PG' order by master_id asc";
		...
		if(KillEvent == 0){
			var CheckVal = 0;
			   ...
			   /*
					...
					BootstrapDialog.alert(" Please Add one more PG Detail for BG..!!");
					event.preventDefault();
					event.returnValue = false;
				}*/
