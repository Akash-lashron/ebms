...
include "common.php";
$PageName = $PTPart1.$PTIcon.'PG Register';
...
$WorksCCNoDataArr = array();
$WorksWONoDataArr = array();
$WorksCCQuery = "SELECT globid,ccno,wo_no FROM works";
$WorksCCQuerySql = mysqli_query($dbConn,$WorksCCQuery);
if($WorksCCQuerySql == true){
	if(mysqli_num_rows($WorksCCQuerySql)>0){
		while($WorksList = mysqli_fetch_object($WorksCCQuerySql)){
			$WorksCCNoDataArr[$WorksList->globid] = $WorksList->ccno;
			$WorksWONoDataArr[$WorksList->globid] = $WorksList->wo_no;
		}
	}
}
...
if($_SESSION['isadmin'] == 1){
	/*$MasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, d.name_contractor FROM loi_entry a INNER JOIN bg_fdr_details b ON (a.loa_pg_id = b.master_id) 
	INNER JOIN tender_register c ON (a.tr_id = c.tr_id) INNER JOIN contractor d ON (a.contid = d.contid) WHERE b.inst_purpose='PG'";*/
	$MasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, c.ccno, c.eic, d.name_contractor FROM loi_entry a 
		...
		INNER JOIN contractor d ON (a.contid = d.contid) WHERE b.inst_purpose='PG'  
		ORDER BY a.tr_id ASC, a.contid ASC ,b.bfdid asc";
}else{
	/*$MasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, d.name_contractor FROM loi_entry a INNER JOIN bg_fdr_details b ON (a.loa_pg_id = b.master_id) 
	INNER JOIN tender_register c ON (a.tr_id = c.tr_id) INNER JOIN contractor d ON (a.contid = d.contid) WHERE c.eic = '$staffid' AND b.inst_purpose='PG'";*/
	$MasterQuery = "SELECT a.*, b.*, c.tr_no, c.work_name, c.ccno, c.eic, d.name_contractor FROM loi_entry a 
		INNER JOIN bg_fdr_details b ON (a.loa_pg_id = b.master_id) 
		INNER JOIN tender_register c ON (a.tr_id = c.tr_id) 
		INNER JOIN contractor d ON (a.contid = d.contid) WHERE b.inst_purpose='PG' AND 
		(c.eic = '".$_SESSION['sid']."' OR c.created_by = '".$_SESSION['userid']."')
		ORDER BY a.tr_id ASC, a.contid ASC ,b.bfdid asc";
}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">PG Register
															...
															<div class="table-responsive dt-responsive rtabdiv" id="table-stmt">
																...
																<table width="100%" align="center" class="dataTable table2excel mgtb-8">
																	<thead>
																		...
																		<tr>
																			<th valign="middle">Instrument<br>Type</th>
																	...
																	<tbody>
																		...
																		<tr class='labeldisplay'>
																			...
																			<?php if($PGValue->inst_status == 'R'){ ?>
																				<td rowspan= <?php echo $ContRowSpan ?> class='tdrow' align='right' valign='middle' nowrap='nowrap'><?php echo "Released"; ?>&nbsp;<i class="fa fa-check-circle-o" style="font-size:20px; color:#046929;"></i></td>
																			<?php }else{ ?>
																				<td rowspan= <?php echo $ContRowSpan ?> class='tdrow' align='right' valign='middle' nowrap='nowrap'></i><?php echo "Not-Released"; ?>&nbsp;<i class="fa fa-times-circle" style="font-size:20px; color:#EA253C;"></i></td>
																			<?php } ?>
																		</tr>
																		<?php 
																				$x++; $y++;  $SNO++;
	...
	$("#exportToExcel").click(function(e){ 
		...
		if(table.length){ 
			$(table).table2excel({
				...
				filename: "PG Register-" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
				fileext: ".xls",
				exclude_img: true,
				exclude_links: true,
				exclude_inputs: true
