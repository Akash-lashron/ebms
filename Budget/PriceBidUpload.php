...
if(isset($_POST["btn_upload"]) == "Upload File"){
	...
	$ColMappArr = array("TR"=>$TrId,0=>$ItmNoColmn,1=>$DescColmn,2=>$QtyColmn,3=>$UnitColmn,4=>$RateColmn,5=>$AmtColmn);
	$_SESSION['FinBidColArr'] = $ColMappArr;
...
if(isset($_POST["confirm"]) == " CONFIRM "){ 
	...
	$ProfitRebate = $_POST['cmb_rebate_profit'];
	$Execute = 0;
	...
	if(count($ItemDescArr)>0){
		...
		$update_query	= "UPDATE works SET work_status='FBU' WHERE globid = '$GlobID'";
		...
		createdby ='".$_SESSION['userid']."', createdon = NOW(), rebate_profit = '$ProfitRebate'";
															   ...
															   <!--	<div class="div12 dataTable" align="left">
																...
																<table width="100%" align="center" class="dataTable table2excel mgtb-8">
																	...
																	<tbody>
																					...
																					foreach ($Spreadsheet as $Key => $Row) { // loop used to get each row of the sheet
																						if(($Key >= $StartRow)&&($Key < $EndRow)){
																							if(trim($Row[$DescColmn]) != ''){
																								$ItemNo		= (string)$Row[$ItmNoColmn];
																								$ItemNo 		= trim($ItemNo);//trim($Row[$ItmNoColmn]); 
																								//echo $Row[$ItmNoColmn]; //exit;
																		...
																		<tr class='labeldisplay'>
																			<td align="center">&nbsp;</td>
																			<td align="right" valign="middle">REBATE / PROFIT&nbsp;</td>
																			<td align="center" colspan="2">
																				<select class="tboxclass" name="cmb_rebate_profit" id="cmb_rebate_profit" style="border:2px solid #7D2C9E; padding-left:1px; padding-right:1px;">
																					<option value="">--- Select ---</option>
																					<option value="RE">Rebate</option>
																					<option value="PR">Profit</option>
																				</select>
																			</td>
																			<td align="center">( % )&nbsp;</td>
																			<td align="right"><input type="number" min="0" max="99" name="txt_rebate_perc" id="txt_rebate_perc" class="tboxclass" value="0.00" style="border:2px solid #7D2C9E; text-align:right; font-weight:bold; padding-left:1px;; padding-right:1px;" maxlength="4"></td>
																			<input type="hidden" name="txt_hid_rebate_perc" id="txt_hid_rebate_perc" class="tboxclass" value="0.00" >
																		</tr>
																		
																		<tr class='labeldisplay'>
																			<td align="center">&nbsp;</td>
																			<td align="right" valign="middle"> REBATE / PROFIT VALUE ( &#8377; )&nbsp;</td>
																			<td align="center">&nbsp;</td>
																			<td align="center">&nbsp;</td>
																			<td align="center">&nbsp;</td>
																		...
																		<tr class='labeldisplay'>
																			<td align="center">&nbsp;</td>
																			<td align="right" valign="middle">TOTAL AMOUNT AFTER REBATE / PROFIT ( &#8377; )&nbsp;</td>
																			<td align="center">&nbsp;</td>
																			<td align="center">&nbsp;</td>
																			<td align="center">&nbsp;</td>
		...
		$(document).ready(function(){ 
			$("body").on("click","#confirm", function(event){
				if(KillEvent == 0){
					...
					var ProfRebate = $("#cmb_rebate_profit").val();
					var PercValid = 1;
					var RebValid = 1;
					
					if(ProfRebate == "PR"){
						var msg = 'Are you sure want to confirm with '+RebatePerc+' % Profit ?';
					}else if(ProfRebate == "RE"){
						var msg = 'Are you sure want to confirm with '+RebatePerc+' % Rebate ?';
					}else{
						var msg = 'Are you sure want to confirm with no Rebate / Profit ?';
					}
					
					if(RebatePerc < 0){
						PercValid = 0;
					}
					if((RebatePerc > 0)&&(ProfRebate == "")){
						RebValid = 0;
					}
					if((RebatePerc <= 0)&&(ProfRebate != "")){
						PercValid = 0;
					...
					if(TrId == ""){
						...
						BootstrapDialog.alert("Invalid Rebate / Profit Type. Unable to Save");
						event.preventDefault();
						event.returnValue = false;
					}else if(PercValid == 0){
						BootstrapDialog.alert("Invalid Rebate / Profit %. Unable to Save");
						event.preventDefault();
						event.returnValue = false;
					}else{
						event.preventDefault();
