			...
			if((TenNo != "")&&(Bidname != "")){
				$.ajax({
					...
					success:function(data){  //alert(data);	
						...
						//alert(TenData);
						//$.each(status, function(key, value) { //alert(1);
	   ...
	   $("body").on("click","#btn_upload", function(event){
			...
			if(ShortName.trim() == ""){
			    ...
			    $("#txt_end_row").val('');
					...
					event.preventDefault();
						BootstrapDialog.confirm('Tender No and Bidder Details Already Exists Are you sure want to Delete and replace ?', function(result){
							if(result) {
								KillEvent = 1;
