...
if($_SESSION['isadmin'] == 1){
	$MasterQuery = "SELECT a.*, b.name_contractor, c.* FROM sheet a INNER JOIN contractor b ON (a.contid = b.contid)  
	INNER JOIN bg_fdr_details c ON (a.sheet_id = c.master_id) WHERE c.inst_purpose='SD'";
}else{
	$MasterQuery = "SELECT a.*, b.name_contractor, c.* FROM sheet a INNER JOIN  contractor b ON (a.contid = b.contid)  
	INNER JOIN bg_fdr_details c ON (a.sheet_id = c.master_id) WHERE a.eic = '$staffid' AND c.inst_purpose='SD'";
}
									...
									<div class="div12">
										<div class="card cabox">
											<div class="face-static">
												<div class="card-header inkblue-card" align="center">SD Register </div>
	...
	$("#exportToExcel").click(function(e){ 
		...
		if(table.length){ 
			$(table).table2excel({
				...
				filename: "SD Register-" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
				fileext: ".xls",
				exclude_img: true,
				exclude_links: true,
				exclude_inputs: true
