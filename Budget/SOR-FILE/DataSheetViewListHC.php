...
if(isset($_GET['dsid'])){
	$DsId = $_GET['dsid'];
	$ParIdArr = array();
	$SelectParIdQuery = "SELECT id, type, par_id FROM (SELECT * FROM group_datasheet_hc ORDER BY par_id, id) products_sorted, (SELECT @pv := '$DsId') initialisation 
	...
	$PageTile = $GlobPageTitleArrHC[$DsId];
	$SelectQuery = "select * from datasheet_master_hc where par_id IN ($ParIdStr)";//= '$DsId'";
							...
							<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="color1 dataTable">
								...
								<?php $Slno = 1; if($RowCount == 1){ while($List = mysqli_fetch_object($SelectSql)){ ?>
									<tr>
										...
										<a data-url="DataSheetViewNewHC?refid=<?php echo $List->ref_id; ?>"><u><?php echo $List->type; ?></u></a>
										<?php }else{ ?>
										<a data-url="DataSheetViewNewHC?refid=<?php echo $List->ref_id; ?>"><u><?php echo $List->type; ?></u></a>
										<?php } ?>
										</td>
