...
if(isset($_POST['btn_generate']) == " Generate OTP "){
	...
	if($AdmEmailId != ""){
		...
		if(!$mail->send()) {
			...
			$InsertQuery = "insert into sor_acc set otp = '$OTP', access_type = 'PRCA', access_email = '$AdmEmailId', access_date = NOW(), staffid = ".$_SESSION['sid'];
	...
	$OTP 		= $_POST['txt_otp'];
	if($OTP != ""){
		$SelectQuery = "select * from sor_acc where access_type = 'PRCA' and otp = '$OTP'";
		$SelectSql 	 = mysqli_query($dbConn,$SelectQuery);
		if($SelectSql == true){
			if(mysqli_num_rows($SelectSql)>0){
				$DeleteQuery = "delete from sor_acc where access_type = 'PRCA' and staffid = ".$_SESSION['sid'];
				$DeleteSql 	 = mysqli_query($dbConn,$DeleteQuery);
				header("Location: PeriodicalRateConfirmPart1.php");
			}
		}
	}
	$DeleteQuery = "delete from sor_acc where access_type = 'PRCA' and staffid = ".$_SESSION['sid'];
...
$IsUpdated = 0;
$SelectDefMasQuery  = "select de_id from default_master_temp";
$SelectDefMasSql 	= mysqli_query($dbConn,$SelectDefMasQuery);
if($SelectDefMasSql == true){
	if(mysqli_num_rows($SelectDefMasSql)>0){
		$IsUpdated = 1;
	}
}
$SelectItemMasQuery = "select item_id from item_master_temp";
$SelectItemMasSql	= mysqli_query($dbConn,$SelectItemMasQuery);
if($SelectItemMasSql == true){
	if(mysqli_num_rows($SelectItemMasSql)>0){
		$IsUpdated = 1;
	}
}
?>
						...
						<blockquote class="bq1" style="overflow:auto">
							...
							<div class="row <?php if($ViewBox == 2){ echo "hide"; } ?>">
								...
								<div class="div6" align="center">
									<div class="innerdiv2">
										<div class="row divhead head-sb" align="center"> Periodical Rate Confirm - Access</div>
										...
										<?php }else{ ?>
											<div class="row">
												<div class="div12 cboxlabel" align="center">
												<span class="InstHead" style="color:#F0155E">Not yet changed any Item Rate / Taxes & Over Heads Percentage. So unable to Confirm.</span>
												</div>
											</div>
										<?php } ?> 
											<div class="row clearrow"></div>
									...
									<div class="innerdiv2">
										...
										<div class="row innerdiv group-div" align="center">
											...
											<div class="row">
												...
												<div class="div5">
													<input type="text" name="txt_otp" id="txt_otp" class="tboxclass" value="" autocomplete="off">
												</div>
											</div>
											<div class="row clearrow"></div>
											<div class="row">
