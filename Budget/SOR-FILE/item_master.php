...
if (isset($_POST['btn_save_single_x']))
{
	...
	{
			 //echo "txt_single_level4".'</br>';	

		$sql_max_level4="select max(item_id) as item_id from item_master where 
							item_id like '" . $_POST['cmb_single_level2'] . "%' and char_length(item_id) = '9'";
		...
		{
			$new_item_id_4=$_POST['cmb_single_level2'].'00001';
		}
		else
		{
		...
		$sql_insert_4="insert into item_master(item_id,make,item_code,item_desc,unit,price)
...
// End : Single
if (isset($_POST['btn_update_single_x']))
{
	...
	if( ($_POST['txt_single_level4']!='') && ($_POST['cmb_make']!='New') && ($_POST['cmb_single_level3']!='Select') && ($_POST['cmb_single_level3']!='New') && ($_POST['cmb_single_level2']!='New'))
	{
		...
		/*$sql_insert_4="insert into item_master(item_id,make,item_code,item_desc,unit,price)
						values('" . $new_item_id_4 . "' ,
							   ...
							   '" . $_POST['txt_unit'] . "',
							   '" . $_POST['txt_price'] . "')";*/
		$UpdateQuery = "update item_master set item_code = '".$_POST['txt_single_level3']."', item_desc = '".$_POST['txt_single_level4']."', unit = '".$_POST['txt_unit']."', price = '".$_POST['txt_price']."' where item_id = '$hid_item_id'";
		$UpdateSql=mysqli_query($dbConn,$UpdateQuery,$conn);
		//echo "1=".$sql_insert_4.'</br>';
		...
		if($UpdateSql!="")
		{
			...
			<script type="text/javascript" language="javascript">
				...
				window.location.href = 'ItemMasterView.php';
			</script>
			<?php
		}
	}
...
$rec_stored=false;
$sheet_no=-1;
/*if (isset($_POST['btn_view_single_x']))
{
if($_GET['item_id']!=""){
	$item_id=$_GET['item_id'];
	$selectitem_sql="select * from item_master where item_id like '07%' order by item_id";
	//echo $material_select;
	$rsitem_sql= mysqli_query($dbConn,$selectitem_sql);
	if(mysqli_num_rows($rsitem_sql)>0){
		$List=mysqli_fetch_object($rsitem_sql);
		$itemcode=$List->item_code;
		$itemdescription=$List->item_desc;
		$rate=$List->rate;
		$unit=$List->unit;
		}
		}
...
if($_GET['id']!=""){
	$item_id	=	$_GET['id'];
	$MainItem 	= substr($item_id,0,4);
	$SelectItemQuery = "select * from item_master where item_id = '$item_id'";
...
<table width="925" border="0" align="center" cellpadding="0" cellspacing="0" class="color1">
	...
	<tr style="display:">
		...
		<td width="790" class="label">
			<input type="radio" name="type" value="Single" checked="checked"  onClick="show_type()" style="display:none"/>&nbsp;&nbsp;&nbsp; 
			<input type="radio" name="type"  value="Multiple"onClick="show_type()" style="display:none"/>&nbsp;&nbsp;&nbsp; 
	  </td>
	</tr>

	
	<tr>
		<td border="0" cellpadding="0" cellspacing="0" colspan="4" align="center"> 
			<table style="display:" id="single" width="900" border="0" align="center" cellpadding="0" cellspacing="0" class="color3">
				...
				<tr style="display:none">
					...
					<td width="384" class="labelbold">Level 1</td>
					<td width="500">
						<?php
						$sql_single_level1="select * from item_master where item_id in ('07') order by item_id";
				...
				<tr>
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold">Main Item</td>
					<td width="500">
						<?php
						...
						?>
						<select class="text" style="width:780px;height:21px;" name="cmb_single_level2" ID="cmb_single_level2" onBlur="func_make()">
							<option value="Select">Select</option>
							<!--option value="New">New</option-->
							<?php
							while($row=mysqli_fetch_assoc($rs_single_level2))
							{
								if($MainItem == $row['item_id']){
									$sel =  'selected="selected"';
								}else{
									$sel = '';
								}
								?>
								<option value="<?php echo $row['item_id'];?>" <?php echo $sel; ?>><?php echo $row['item_desc'];?></option>
								<?php
							}
						?>
				  		</select>
				...
				<tr style="display:none" id="single_level2">
					<td width="41">&nbsp;</td>
					<td width="384" class="labelbold">&nbsp;</td>
					<td width="500"><input type="text" style="width:790px;" name="txt_single_level2" id="txt_single_level2" class="text" value="" /></td>
				</tr>
				...
				<tr style="display:none" >
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold" valign="top">Make</td>
					<td width="500">
				...
				<tr>
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold">Item Code</td>
					<td width="500">
						<!--select class="text" style="width:790px;height:21px;" name="cmb_single_level3" ID="cmb_single_level3" onBlur="show_single_new_level3()"-->
							...
							<!--option value="New">New</option-->
				  		<!--/select-->
						<input type="text" style="width:98%;" name="txt_single_level3" id="txt_single_level3" class="text" value="<?php if($_GET['id'] != ''){ echo $itemcode; } ?>" />
					</td>
				</tr>
				...
				<tr>
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold">Item Description</td>
					<td width="500"><input type="text" style="width:98%;" name="txt_single_level4" id="txt_single_level4" class="text" value="<?php if($_GET['id'] != ''){ echo $itemdescription; } ?>" /></td>
				</tr>
				<tr><td colspan="3">&nbsp;</td></tr>
				<tr>
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold">Unit</td>
					<td>
						<select name="txt_unit" id="txt_unit" class="text">
							<option value="Select">Select</option>
							...
							<option value="CU.Mtr">Cum</option>
							<option value="Tonne km">Tonne km</option>
							<option value="Tonne km">Tonne</option>
							...
							<option value="Eachset">Each Set</option>
							<option value="Sqmtr">Sqmtr</option>
							<option value='Bag'>Bag</option>-->	
							<?php echo $objBind->BindUnit($unit); ?>					
						</select>
				...
				<tr>
					<td width="41" class="label">&nbsp;</td>
					<td width="384" class="labelbold">Rate</td>
					<td width="500"><input type="text" style="width:98%;" name="txt_price" id="txt_price" class="text" value="<?php if($_GET['id'] != ''){ echo $rate; } ?>" /></td>
				</tr>
				<tr><td colspan="3">&nbsp;</td></tr>
				
				<tr><td colspan="3" class="labelcenter">
				<input type="hidden" name="hid_item_id" id="hid_item_id" value="<?php echo $_GET['id']; ?>">
				<?php if($_GET['id'] != ''){ ?>
				<input type="image" name="btn_update_single" id="btn_update_single" value="Save" src="Buttons/Update_Normal.png" onMouseOver="this.src='Buttons/Update_Over.png'" onMouseOut="this.src='Buttons/Update_Normal.png'" />
				<?php }else{ ?>
				<input type="image" name="btn_save_single" id="btn_save_single" value="Save" src="Buttons/Save_Normal.jpg" onMouseOver="this.src='Buttons/Save_Over.jpg'" onMouseOut="this.src='Buttons/Save_Normal.jpg'" />
				<?php } ?>
				<input type="image" name="btn_view_single" id="btn_view_single" value="View" src="Buttons/View_Normal.png" onMouseOver="this.src='Buttons/View_Over.png'" onMouseOut="this.src='Buttons/View_Normal.png'" />
				</td>
				</tr>
