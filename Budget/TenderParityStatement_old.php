...
if(isset($_POST['btnView'])){   
	...
	if(($sheet_id != 0)||($sheet_id != "")&&($sheet_id != NULL)){
		...
		$SelectMbookQuery = "SELECT * FROM schdule WHERE sheet_id = '$sheet_id' ORDER BY sch_id ASC"; 
		...
		if(($SelectMbookQuerySql == true)&&(mysqli_num_rows($SelectMbookQuerySql)>0)){ 
			while($MBList=mysqli_fetch_object($SelectMbookQuerySql)){ 
				$MBsheetid = $MBList->sheet_id;
				$MBsno = $MBList->sno;
				$subdivid 	= $MBList->subdiv_id;
				$mbquantity   = $MBList->total_quantity;
				//$DeptQuantity  = $MBList->quantity;
				$MBQtyArr[$subdivid] = $mbquantity;
				$MBItemNoArr[$subdivid] = $MBsno;
		...
		$SelectScheduleQuery = "SELECT * FROM measurementbook WHERE sheetid = '$sheet_id' AND (part_pay_flag = '0' OR part_pay_flag = '1') ORDER BY measurementbookid ASC";
																	...
																	<?php
																		...
																		*/
																				while($Result2 = mysqli_fetch_object($SelectScheduleQuerySql)){ 
																					$SheetID		  = $Result2->sheetid; 
																					//$ItemNo		  = $Result2->sno; 
																					$ItemDesc	  = $Result2->description; 
																					$ItemQty		  = $Result2->mbquantity; //echo "AA=".$ItemQty;
																					$ItemUnit	  = $Result2->per; 
																					$DeptItemRate = $Result2->rate; 
																					$subdiv_id	  = $Result2->subdivid; 
																					...
																					$ItemNo = $MBItemNoArr[$subdiv_id];
																					echo $ItemNo;
																					if(($ItemNo != '')||($ItemNo != NULL)){
																						if(($MBQty != '')||($MBQty != NULL)){
																							$MBQty = round(($MBQty),2);
																						}
																						//$MBQty =  (($MBQty != '')||($MBQty != NULL)) ? round(($MBQty),2)
																						//$MBQty = round(($MBQty),2);
																						//echo $DeptItemRate;exit;
																						...
																						echo '<td class="hideText" align="justify">'.$ItemDescDept.'</td>';
																						...
																						if(!empty($ContArr)){
																							foreach($ContArr as $ContractId){ 
																								...
																								//echo $ContractId."*".$ItemNo;
																								//echo $ItemQty."*".$ItemRate;
																			...
																			if(!empty($ContArr)){
																				foreach($ContArr as $ContractId){
																					...
																					if($DeptTotalAmount != 0){
																						...
																						$TotalVariatePerc = 0;//round(($TotalVariateAmt * 100 / $DeptTotalAmount),2);
																					}
																					echo '<td>&nbsp;</td>';
