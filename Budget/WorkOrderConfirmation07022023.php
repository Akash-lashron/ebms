...
if(isset($_POST['btn_save']) == ' Save '){
	...
	if($InQueryCon == 1){
		...
		if($Checkpt == 1){
			...
			$SelectQuery2 	= "SELECT globid,sheetid FROM works WHERE globid = '$GlobID'";
		...
		if($SheetCheckVal == 1){
			$UptSheetQuery	= "UPDATE sheet SET tr_id='$TrNo', globid ='$GlobID', work_name='$WrkName', short_name='$WrkShortName', work_order_no='$WrkOrderNo', 
			...
			userid = '$userid', created_date = NOW() WHERE globid = '$GlobID'";
			$UptSheetQuery_sql = mysqli_query($dbConn,$UptSheetQuery);
		}else if($SheetCheckVal == 0){ 
			$SheetQuery	= "INSERT INTO sheet SET tr_id = '$TrNo', globid = '$GlobID', work_name = '$WrkName', short_name = '$WrkShortName', 
			work_order_no = '$WrkOrderNo', agree_no = '$AggrementNo', agree_date = '$AggrementDt', computer_code_no = '$CompCodeNo', 
			...
			is_less_appl = '$LCESSAppRad', gst_inc_exc = '$GSTIncExRad',active = '1', userid = '$userid', created_date = NOW()";
			$SheetQuery_sql = mysqli_query($dbConn,$SheetQuery);
			$LastInsertId 	 = mysqli_insert_id($dbConn);
			$UpdateWorkQuery		= "UPDATE works SET sheetid='$LastInsertId' WHERE globid = '$GlobID'";
			$UpdateWorkQuery_sql = mysqli_query($dbConn,$UpdateWorkQuery);
		}
		...
		if($CheckPt == 1){//echo 1;exit;
			...
			agree_no='$AggrementNo', agree_date='$AggrementDt', computer_code_no='$CompCodeNo', work_order_date='$WrkOrderDate', work_order_cost='$WrkOrderAmt',
			work_commence_date='$WrkCommDate',hoaid='$HoaId', eic='$EnggId', work_duration='$WrkDuration', date_of_completion='$DOCompletion', cbdtid='$BankDetChkBox', sd_perc = '$SDper', contid='$ContID',
			...
			agree_no = '$AggrementNo', agree_date = '$AggrementDt', computer_code_no = '$CompCodeNo', work_order_date = '$WrkOrderDate', work_order_cost = '$WrkOrderAmt',
			work_commence_date = '$WrkCommDate',hoaid = '$HoaId', work_duration = '$WrkDuration', eic='$EnggId', date_of_completion = '$DOCompletion', sd_perc = '$SDper', cbdtid = '$BankDetChkBox', contid='$ContID',
			assigned_staff = '$EnggId',  name_contractor='$ContName', is_gst_appl = '$IsGSTRad', gst_perc_rate = '$GSTRateonWO', is_less_appl = '$LCESSAppRad', gst_inc_exc = '$GSTIncExRad',active = '1',
			...
			wo_amount='$WrkOrderAmt', wo_date='$WrkOrderDate', work_commence_date='$WrkCommDate', work_status='WO', work_duration='$WrkDuration', 
			sch_comp_date='$DOCompletion', is_gst_appl='$IsGSTRad', gst_perc_rate='$GSTRateonWO', is_less_appl='$LCESSAppRad', sd_perc = '$SDper',
	...
	if(isset($_POST['btn_confirm']) == ' Save '){
		...
		$UpdateWorkQuery	= "UPDATE sheet SET  rebate_percent ='$Rebate' WHERE globid = '$GlobID' and sheet_id  = '$SheetID'";
		$UpdateWorkQuery_sql = mysqli_query($dbConn,$UpdateWorkQuery);
...
<script type="text/javascript">
    ...
    <body class="page1" id="top" oncontextmenu="return false"onload="noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
        <!--==============================header=================================-->
        <form action="WorkOrder.php" method="post" enctype="multipart/form-data" name="phuploader">
																	...
																	if($RowCount == 1){ while($List = mysqli_fetch_object($SelectSql)){ ?>
																		<tr>
																			<td align="center"><?php echo $List->item_no; ?>
																					<input type="hidden" readonly name="txt_item_no[]" id="txt_item_no" class="tboxsmclass" value="<?php if($List->item_no != 0){ echo $List->item_no; } ?>" style="width:70px">
