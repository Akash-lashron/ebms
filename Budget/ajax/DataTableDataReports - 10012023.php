...
if(isset($_POST['code'])){
	...
	switch ($PageCode) {
		case "EXPMO":
			...
			$WorksArr = array(); 
			if($BudHoa == "ALL"){
				$SelectHoaQuery = "SELECT * FROM hoa_master WHERE fin_year = '$BudFinYear'"; 
			}else{
				$SelectHoaQuery = "SELECT * FROM hoa_master WHERE fin_year = '$BudFinYear' AND (old_hoa_no = '$BudHoa' OR new_hoa_no = '$BudHoa')"; 
			}
		...
		case "EXPPE":
			...
			for($i=$StartPeriodYr; $i<$EndPeriodYr; $i++){
				...
				if($BudHoa == "ALL"){
					$SelectHoaQuery = "SELECT * FROM hoa_master WHERE fin_year = '$FinYearTemp'"; 
				}else{
					$SelectHoaQuery = "SELECT * FROM hoa_master WHERE fin_year = '$FinYearTemp' AND (old_hoa_no = '$BudHoa' OR new_hoa_no = '$BudHoa')"; 
				}
				$SelectHoaSql = mysqli_query($dbConn, $SelectHoaQuery);
				if($SelectHoaSql == true){
					if(mysqli_num_rows($SelectHoaSql)>0){
