...
$PGdetailQuery 	= "select * from bg_fdr_details  where master_id = '$PGLOID' and  inst_status != 'R' AND inst_purpose = 'PG' order by contid ";	
...
if($SelectSql == true){
	if(mysqli_num_rows($SelectSql)>0){
		while($List = mysqli_fetch_array($SelectSql)){
			...
			$EctndDat     =      $List['inst_ext_date'];
			$EctndDat1     =      dt_display($EctndDat );
			$List['inst_date']               =  $Issueddt1;
			$List['inst_exp_date']           =  $validddt1;
			$List['inst_amt']                =  $Emdamount1;
			$List['inst_ext_date']           =  $EctndDat1;
