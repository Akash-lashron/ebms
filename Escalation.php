...
if ($sqlquery == true) 
{
	...
	$BaseIndexMonth   		= date('M-Y', strtotime($work_commence_date));
}
//echo $work_commence_date;
$Line = $Line + $LineIncr;
...
$title = '<table width="875" border="0"  cellpadding="2" cellspacing="2" align="center" bgcolor="#FFFFFF" style="border:none;" class="label">
			...
			<br/><br/>
			Escalation for 10CA ('.$TCAfir_month.' to '.$TCAlas_month.')&nbsp;&nbsp;
			<br/><br/>
			Base Index Month : '.$BaseIndexMonth.'
			<br/>&nbsp;
			</td></tr>
			</table>';
echo $title;
...
<?php
$TCATCABidStr = ""; $ItemCodeArr = array();
$select_bid_query_tca 	= "select distinct bid, esc_item_code from escalation_10ca_details where sheetid = '$sheetid' and quarter = '$quarter' and esc_rbn = '$esc_rbn'";
...
if($select_bid_sql_tca == true)
{
	if(mysql_num_rows($select_bid_sql_tca)>0)
	{
		while($BidListTCA = mysql_fetch_object($select_bid_sql_tca))
		{
			...
			$TCABidStr .= $bidTCA."*";
			$ItemCodeArr[$BidListTCA->bid] = $BidListTCA->esc_item_code;
		}
	}
}
...
if($ItemCode == "CsIo"){
	...
	$SelectQuery1 	= "select a.*, b.* from esc_consumption_10ca_site a inner join base_index b on (a.item_code = b.base_index_code) 
					  where a.sheetid = '$sheetid' and a.item_code = 'CsIo' and a.quarter = '$quarter' and b.active = 1";
	...
	if($SelectSql1 == true){
		if(mysql_num_rows($SelectSql1)>0){
			while($List = mysql_fetch_object($SelectSql1)){
				...
				$Decimal = 3;
				$TCAbase_index_str1 .= $List->bid."*@*".''."*@*".$List->base_index_item
	...
	//echo $base_index_str;exit;
	//$TCADataStr = $base_index_str."@@##@@".$month_count;
}else if($ItemCode == "SsIo"){
	$month_count = 0;
	$InvoiceYrArr = array(); $IndexArr = array();
	$SelectQuery1 	= "select a.*, b.* from esc_consumption_10ca_site a inner join base_index b on (a.item_code = b.base_index_code) 
					  where a.sheetid = '$sheetid' and a.item_code = 'SsIo' and a.quarter = '$quarter' and b.active = 1";
	$SelectSql1 	= mysql_query($SelectQuery1);
	//echo $SelectQuery1;exit;
	if($SelectSql1 == true){
		if(mysql_num_rows($SelectSql1)>0){
...
$title = '<table width="875" border="0"  cellpadding="2" cellspacing="2" align="center" bgcolor="#FFFFFF" style="border:none;" class="label">
			...
			Escalation MBook No.'.$esc_mbook.'
			<br/><br/>
			Escalation for 10CC ('.$fir_month.' to '.$las_month.')&nbsp;&nbsp;
			<br/><br/>
			Base Index Month : '.$BaseIndexMonth.'
			<br/>&nbsp;</td></tr>
			</table>';
...
<?php 
	...
	if(($total_esc_amt != "")&&($total_esc_amt != 0))
	{
		...
		$delete_mbook_query		= 	"delete from mymbook where sheetid = '$sheetid' and rbn = '$esc_rbn' and esc_id = '$esc_id' and 
									quarter = '$quarter' and mtype = 'E' and genlevel = 'escalation'";
		...
		$insert_mbook_query  	= 	"insert into mymbook set mbno = '$esc_mbook', startpage = '$start_page', endpage = '$end_page', sheetid = '$sheetid', 
									quarter = '$quarter', staffid = '$staffid', rbn = '$esc_rbn', esc_id = '$esc_id', active =1, mtype = 'E', genlevel = 'escalation', mbookorder = 1";
