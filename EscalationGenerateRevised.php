...
if ($_POST["submit"] == ' View ') 
{
	...
	$tcafrom_date 		= 	trim($_POST['txt_tcafrom_date']);
	$tcato_date 		= 	trim($_POST['txt_tcato_date']);
	...
	$curr_rbn 			= 	trim($_POST['txt_curr_rbn']);
	//echo $from_date;exit;
	$_SESSION['escal_sheetid'] 			= $sheetid;
	$_SESSION['escal_tca_from_date'] 	= $tcafrom_date;
	$_SESSION['escal_tca_to_date'] 		= $tcato_date;
	...
	$_SESSION['curr_rbn'] 				= $curr_rbn;
	header('Location: EscalationRevised.php');
} 
?>
     ...
     { 
		...
		function GetEscQuarterRBN()
     	{ 
			...
			document.getElementById("txt_curr_rbn").value = "";
            ...
            strURL = "find_RevisedEscQuarterRBN.php?sheetid=" + document.form.cmb_shortname.value+"&quarter="+document.form.cmb_quarter.value;
            xmlHttp.open('POST', strURL, true);
            xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xmlHttp.onreadystatechange = function ()
            {
                if (xmlHttp.readyState == 4)
                {
                    ...
                    else
                    {
						...
						document.getElementById("txt_curr_rbn").value = curr_rbn;
						/*var SplitQtrStr = QtrStr.split("*");
						document.form.cmb_quarter.length = 0;
						var optn = document.createElement("option");
						optn.value = "";
...
<script>
   $(function () {
        ...
        function DisplayPageDetails() {
            ...
            $.post("MBookNoServiceRevEsc.php", {currentmbook: currentmbooknovalue, currentbmookname: currentmbooknotext, sheetid: wordordernovalue, generatetype: generatetype, quarter: quarter, rbn: rbn, esc_id: esc_id, mtype: mtype}, function (data) {
       	...
       	<div class="container_12">
			...
			<!--<div align="right"><a href="View_Electricity_generate_Bill.php">View</a>&nbsp;&nbsp;&nbsp;</div>-->
                 <blockquote class="bq1">
                     <div class="title">Escalation Generate - Revised</div>
						...
						<input type="hidden" name="hid_staffid" id="hid_staffid" value="<?php echo $staffid; ?>">
                        	<table width="1078px" border="0" align="center" cellpadding="0" cellspacing="0" class="color1">
                                <tr><td width="16%">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
								<tr>
                                    <td>&nbsp;</td>
                                    <td class="label">Work Short Name</td> 
                                    <td>
										<select name="cmb_shortname" id="cmb_shortname" class="textboxdisplay" style="width:465px" onChange="workorderdetail();AllQuarter();func_GenerateMBno();">
								...
								<tr>
                                    ...
                                    <td class="label">
										<select name="cmb_quarter" id="cmb_quarter" style="width:215px;" class="textboxdisplay" onChange="GetEscQuarterRBN();">
											<option value="">-------------- Select --------------</option>
										</select>
										&emsp;&nbsp;
										<label class="label">&emsp;&emsp;RAB</label>&nbsp;&emsp;
										<input type="text" name='txt_curr_rbn' readonly="" id='txt_curr_rbn' class="textboxdisplay" style="width: 150px;">
										<input type="hidden" name='txt_rbn' readonly="" id='txt_rbn' class="textboxdisplay" style="width: 150px;">
