...
$curr_rbn    	= 	$_SESSION["curr_rbn"];

$start_page = $esc_mbookpage;
...
$tcafromdate 		= 	dt_format($esc_tcafrom_date);
$tcatodate 			= 	dt_format($esc_tcato_date);
$TCAMonthList 			= 	array();
...
<script type="text/javascript" language="javascript">
	...
	function goBack()
	{
		url = "EscalationGenerateRevised.php";
...
for($i = 0; $i<count($TCAexptca_data); $i+=14)
{
	...
	<tr style=" height:35px;">
		...
		<td align="right" valign="middle" nowrap="nowrap"><?= $TCAPrintqty_month_wise_mt; ?></td>
...
<?php
	...
	$update_mbook_query = "update escalation_10ca_details set esc_mbook = '$esc_mbook', esc_page = '$page' 
	where sheetid = '$sheetid' and esc_rbn = '$esc_rbn' and quarter = '$quarter' and esc_month = '$TCAPrintesc_month' and esc_id = '$esc_id'";
	//$update_mbook_sql = mysql_query($update_mbook_query);
	//echo $update_mbook_query."<br/>";
  }
}
...
?>
  	<tr style=" height:35px;" class="label">
		...
		<td colspan="9" align="right" valign="middle">10-CA Escalation Amount&nbsp;&nbsp;<i class='fa fa-inr' style='font-weight:normal; padding-top:5px;'></i>&nbsp;</td>
...
//$Line = $Line+6;
//$tablehead = $tablehead . "<table width='1087px' frame=''  bgcolor='#0A9CC5' border='1' cellpadding='3' cellspacing='3' align='center' style='color:#ffffff;' id='mbookdetail' class='label table1'>";
$tablehead = $tablehead . "<tr style='background-color:#EEEEEE;' class='label'>";
//$tablehead = $tablehead . "<td  align='center' class='labelsmall labelheadblue' width='12px' style='background-color:#0A9CC5;' rowspan='2'></td>";
$tablehead = $tablehead . "<td  align='center' class='' width='44px' rowspan='2'>Item No.</td>";
$tablehead = $tablehead . "<td  align='center' class='' width='130px' rowspan='2'>Description of work</td>";
$tablehead = $tablehead . "<td  align='center'  width='40px' rowspan='2'>Contents of Area</td>";
...
<?php
	...
	$select_esc_paid_query = "select rev_tcc_mbook, rev_tcc_mbpage, rev_esc_total_amt from escalation_revised where sheetid = '$sheetid' and quarter = '$quarter' ORDER BY rev_esc_id  DESC";
	...
	{
		$Esc_Abs_MBook = $PaidEsc_Abs_MBook;
		$Esc_Abs_MBPage = $PaidEsc_Abs_MBPage;
...
<table width='675' cellpadding='3' cellspacing='3' align='center' class='label table1 labelprint' bgcolor="#FFFFFF" id="table1">
	...
	<tr style="background-color:#DFDFDF">
		...
		<tr style='border-style:none;'><td colspan='3' align='center' style='border-style:none;'> page <?= $page; ?></td></tr>
</table>
<?php 
	$end_page = $page;
	...
	{
		$update_tcc_query = "update escalation set rev_tcc_amt = '$TCC_Total_Amt', rev_tcc_mbook = '$esc_mbook', rev_tcc_mbpage = '$page', 
							rev_tca_amt = '$TCA_Total_Amt', rev_tca_mbook = '$tca_mbook_no', rev_tca_mbpage = '$tca_mbook_page', rev_esc_total_amt = '$total_esc_amt', 
							rev_rbn = '$curr_rbn', rev_modifieddate = NOW() 
							where sheetid = '$sheetid' and rbn = '$esc_rbn' and esc_id = '$esc_id' and flag = 0";
							//echo $update_tcc_query;
		$update_tcc_sql = mysql_query($update_tcc_query);
		
		$delete_mbook_query		= 	"delete from mymbook where sheetid = '$sheetid' and rbn = '$curr_rbn' and esc_id = '$esc_id' and 
									quarter = '$quarter' and mtype = 'E' and genlevel = 'rev_escalation'";
									//echo $delete_mbook_query	;
		$delete_mbook_sql 		= 	mysql_query($delete_mbook_query);						
		
		$insert_mbook_query  	= 	"insert into mymbook set mbno = '$esc_mbook', startpage = '$start_page', endpage = '$end_page', sheetid = '$sheetid', 
									quarter = '$quarter', staffid = '$staffid', rbn = '$curr_rbn', esc_id = '$esc_id', active =1, mtype = 'E', genlevel = 'rev_escalation', mbookorder = 1";
