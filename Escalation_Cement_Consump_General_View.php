...
$escal_measure_query = 	"SELECT mbookheader.mbheaderid, DATE(mbookheader.date) as mdate, mbookheader.sheetid, 
							...
							WHERE schdule.measure_type != 's' AND schdule.measure_type != 'st' AND mbookheader.sheetid = '$sheetid' 
							AND (mbookheader.date BETWEEN '$fromdate' AND '$todate') 
							AND schdule.sheet_id = '$sheetid' AND schdule.tc_unit != '0' AND (schdule.item_flag = 'NI' OR schdule.escalation_flag = 'Y') AND schdule.mat_code = 'RMC' 
							AND NOT EXISTS 
...
if(isset($_POST['submit']) == " Save ")
{
	...
	if($date_wise_data != "")
	{
		...
		and ecm.esc_id='$esc_id' and ecm.esc_item_type = 'CEM' and ec.item_code = 'CIo' and ecm.item_code = 'CIo' and ecm.quarter = '$quarter'";
		...
		for($j=0; $j<$count1; $j++)
		{
			...
			$insert_master_query = "insert into esc_consumption_10ca_master set esc_id = '$esc_id', sheetid = '$sheetid',
									esc_rbn = '$esc_rbn', quarter = '$quarter', item_code = 'CIo', esc_cons_mbook = '$cc_mbookno', 
...
@media print 
{
	...
	<div class="content">  
		<div class="title">Escalation - Cement Consumption</div> 
