...
if ($_POST["submit"] == ' View ') 
{
	...
	$mbid 			= 	trim($_POST['currentmbookno']);
	//echo $from_date;exit;
	...
	$_SESSION['sc_mbook_no'] 		= $mbook;
	$_SESSION['sc_mbook_pageno'] 	= $mbookpage;
	$_SESSION['sc_mbook_id'] 		= $mbid;
	header('Location: Escalation_Steel_Site_Consump_View.php?varid=1');
} 
?>
     ...
     { 
		...
		/*function find_priceindex_period()
     	{ 
			document.getElementById("txt_from_date").value  = "";
			document.getElementById("txt_to_date").value  = "";
			document.getElementById("txt_quarter").value = "";
			document.getElementById("currentmbookno").value = "";
            ...
            strURL = "find_priceindex_period.php?sheetid=" + document.form.cmb_shortname.value+"&type=TCA&base_index_code=SIo";/// This is for Cement - So base_index_code is CIo;
            xmlHttp.open('POST', strURL, true);
            xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xmlHttp.onreadystatechange = function ()
            {
                if (xmlHttp.readyState == 4)
                {
                    ...
                    if (data != "")
                    {
						...
						var esc_id		 = name[9];
						//alert(pi_from_date)
						document.getElementById("txt_from_date").value  = pi_from_date;
						document.getElementById("txt_to_date").value  = pi_to_date;
						document.getElementById("txt_quarter").value  = quarter;
						document.getElementById("txt_rbn").value  = rbn;
						document.getElementById("txt_esc_id").value  = esc_id;
						//document.getElementById("txt_price_index_rate_m3"+bid).value  = pi_rate3;
        ...
        { 
            ...
            strURL = "find_generatembno.php?sheetid=" + document.form.cmb_shortname.value + "&mtype=SC";
            xmlHttp.open('POST', strURL, true);
            xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xmlHttp.onreadystatechange = function ()
            {
...
<!--==============================header=================================-->
    ...
    <!--==============================Content=================================-->
    <div class="content">
                     <div class="title">Escalation - Steel Consumption</div>
