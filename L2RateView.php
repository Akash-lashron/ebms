...
$schdulesql ="SELECT DISTINCT sno,sch_id, subdiv_id, sheet_id,  description, total_quantity, rate, per, decimal_placed, total_amt, subdiv_id, l2_rate FROM  schdule   where sheet_id= '$sheetid' AND  sno != '0' ";
$schdule=mysql_query($schdulesql);
 ...
 if(isset($_POST['update']))
 {
	...
	for($i=0; $i<$cnt; $i++)
	{
		...
		$update_decimal_query = "update schdule set l2_rate = '$result[1]' WHERE sch_id = '$result[0]' AND sheet_id = '$sheetid'";
		$update_decimal_sql = mysql_query($update_decimal_query);
		if($update_decimal_sql != true){ $temp++; }
	}
	header('Location: L2RateView.php?sheetid='.$sheetid.'&temp='.$temp); 
 }
?>
...
<SCRIPT type="text/javascript">
    ...
    <body class="page1" id="top" oncontextmenu="return false"onload="noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="phuploader">
            ...
            <!--==============================Content=================================-->
            <div class="content"> 
                <div class="title">Agreement - L2 Rate Assign</div>
								...
								<table class="table-bordered table1 labeldisplay" align="center" id="dataTable">
									<thead>
										<tr>
											...
											<th>Qty</th>
											<th>Unit</th>
											<th>L2 Rate  <i class='fa fa-inr' style=' width:4px; height:5px; padding-top:5px'></i></th>
										</tr>
									</thead>
									<tbody>
                                  	<?php 
								 	...
								 	?>
										<tr>
											...
											<td align="center"><?php echo $List->total_quantity; ?></td>
											<td align="center"><?php if(($List->per != "")&&($List->per != '0')){ echo $List->per; } ?></td>
											<td align="center">
											<?php if(($List->per != "")&&($List->per != '0')){ ?>
												<input type="text" class="textboxdisplay textboxstyle" style="color:#003399; width:111px" name="txt_decimal_placed" id="txt_decimal_placed<?php echo $divid_incr; ?>" value="<?php echo $List->l2_rate; ?>"  onkeypress="return IsAlphaNumeric(event);" onBlur="get_decimal_val(<?php echo $x1; ?>,<?php echo $List->sch_id; ?>,this);">
												<input type="hidden" name="hide_result[]" id="hide_result<?php echo $x1; ?>" value="<?php echo $List->sch_id."@".$List->l2_rate; ?>" >
