...
if(isset($_POST['Submit'])){
	...
	$GenCheck  = $_POST['mbookG'];
	$StlCheck  = $_POST['mbookS'];
	$AbsCheck  = $_POST['mbookA'];
	$EscCheck  = $_POST['mbookE'];
	...
	if(count($GenCheck) > 0){
		...
		foreach($GenCheck as $Value){
			...
			if($Mbno != ''){
				$InsertQuery = "insert into mbookallotment set mballotmentdate = '$mbookdate', sheetid='$sheetid', allotmentid='$MbId', mbno = '$Mbno',staffid='$staffname',mbpage = 0, mbtotalpages=100, flag='1', mbookmode = '$MbMode', active='1', userid='$userid'";
	...
	if(count($StlCheck) > 0){
		...
		foreach($StlCheck as $Value){
			...
			$Mbno 		= $ExpSTLStr[1];
			if($Mbno != ''){
				$InsertQuery = "insert into mbookallotment set mballotmentdate = '$mbookdate', sheetid='$sheetid', allotmentid='$MbId', mbno = '$Mbno',staffid='$staffname',mbpage = 0, mbtotalpages=100, flag='1', active='1', userid='$userid'";
				$InsertSql	 = mysql_query($InsertQuery);
	...
	if($Error == 0){
		$msg = "MBook Alloted Sucessfully for Staff..!!";
		$success = 1;
		/*if($TempG2 > 0){
			UpdateWorkTransaction($sheetid,0,"W","General MBook issued to staff","");
		}
		if($TempS2 > 0){
			UpdateWorkTransaction($sheetid,0,"W","Steel MBook issued to staff","");
		}
		if($TempA2 > 0){
			UpdateWorkTransaction($sheetid,0,"W","Abstract MBook issued to staff","");
		}
		if($TempE2 > 0){
			UpdateWorkTransaction($sheetid,0,"W","Escalation MBook issued to staff","");
		}*/
	}else{
...
if(($_GET['sheetid'] != '')&&($_GET['staffid'] != '')){
	...
	$GMBModeArr = array(); $SMBModeArr = array(); $AMBModeArr = array(); $EMBModeArr = array();
	$SelectQuery =  "SELECT agreementmbookallotment.mbno, agreementmbookallotment.mbooktype, agreementmbookallotment.allotmentid, agreementmbookallotment.mbookmode 
					FROM agreementmbookallotment 
			   		WHERE NOT EXISTS ( SELECT allotmentid FROM mbookallotment WHERE agreementmbookallotment.allotmentid = mbookallotment.allotmentid)
 			   		AND agreementmbookallotment.active = 1 AND agreementmbookallotment.sheetid ='".$_GET['sheetid']."' ORDER BY agreementmbookallotment.mbno";
		...
		if(mysql_num_rows($SelectSql)>0){
			while($List = mysql_fetch_object($SelectSql)){
				if($List->mbooktype == 'G'){
					...
					$GMBModeArr[$List->allotmentid] = $List->mbookmode;
				}elseif($List->mbooktype == 'S'){
					$SMBArr[$List->allotmentid] = $List->mbno;
					$SMBModeArr[$List->allotmentid] = $List->mbookmode;
				}elseif($List->mbooktype == 'A'){
					$AMBArr[$List->allotmentid] = $List->mbno;
					$AMBModeArr[$List->allotmentid] = $List->mbookmode;
				}elseif($List->mbooktype == 'E'){
					$EMBArr[$List->allotmentid] = $List->mbno;
					$EMBModeArr[$List->allotmentid] = $List->mbookmode;
				}
			}
		}
	}
        ...
        <div class="content">
            ...
            <div class="container_12">
                <div class="grid_12">
					...
					<div align="right"><a href="MBookAllotmentEdit.php?edit=new">Over All View</a></div>
												...
												<div class="div8">
													<select id="staffname" name="staffname" class="tboxclass">
														...
														<?php if(($_GET['staffid'] != '')&&($_GET['sheetid'] != '')){
															echo $objBind->BindWorkStaffList($_GET['staffid'],$_GET['sheetid']);
														} ?>
													</select>
												</div>
											</div>
															...
															if($_GET['sheetid'] != ''){
																if(isset($GMBArr)){
																	foreach($GMBArr as $Key => $Value){
																		$GMBMode = $GMBModeArr[$Key];
																		$MbValueStr = $Key."*@*".$Value."*@*".$GMBMode;
																		echo '<option value="'.$MbValueStr.'">'.$Value.'</option>';
																	}
																}
															} 
															?>
															...
															if($_GET['sheetid'] != ''){
																if(isset($SMBArr)){
																	foreach($SMBArr as $Key => $Value){
																		$SMBMode = $SMBModeArr[$Key];
																		$MbValueStr = $Key."*@*".$Value."*@*".$SMBMode;
																		echo '<option value="'.$MbValueStr.'">'.$Value.'</option>';
																	}
																}
															} 
															...
															if($_GET['sheetid'] != ''){
																if(isset($AMBArr)){
																	foreach($AMBArr as $Key => $Value){
																		$AMBMode = $AMBModeArr[$Key];
																		$MbValueStr = $Key."*@*".$Value."*@*".$AMBMode;
																		echo '<option value="'.$MbValueStr.'">'.$Value.'</option>';
																	}
																}
															} 
															...
															if($_GET['sheetid'] != ''){
																if(isset($EMBArr)){
																	foreach($EMBArr as $Key => $Value){
																		$EMBMode = $EMBModeArr[$Key];
																		$MbValueStr = $Key."*@*".$Value."*@*".$EMBMode;
																		echo '<option value="'.$MbValueStr.'">'.$Value.'</option>';
																	}
																}
															} 
		...
		$(document).ready(function() {
			...
			$.fn.Validation = function(event) { 
				...
				$(".mbno").each(function(index){
					var size = $("option:selected", this).size();
					if(size > 0){
						ch++;
					}
				});
				if($("#workorderno").val() == ''){
					...
					var AlertMsg = "Please Select Staff Name";
					BootstrapDialog.alert(AlertMsg);
					event.preventDefault();
					event.returnValue = false;
				}else if(ch == 0){
					...
					success: function (data) {   //alert(data);
						if(data != null){
							...
							$.each(GMB, function(index, element) {
								var MBNoValue = element.allotmentid+"*@*"+element.mbno+"*@*"+element.mbookmode;
