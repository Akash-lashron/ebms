...
if($_POST["btn_next"] == "Next") 
{
	...
	//echo $fromdate;exit;
	header('Location: SteelMBook_Staff_Wise.php?varid=1');
} 
...
if(isset($_SESSION['sheet_id']) && !empty($_SESSION['sheet_id']) && isset($_SESSION['rbn']) && !empty($_SESSION['rbn'])){
	...
	$MCount2 = 0;
	$select_meas_query2	= "select COUNT(mbgenerateid) as meas_count2 from mbookgenerate_staff where sheetid = '$sheetid' and rbn = '$rbn' and flag = 2 and staffid =".$_SESSION['sid'];
	...
	//echo $skip;echo "<br/>";echo $MCount1;echo "<br/>"; echo $MCount2; echo "<br/>";exit;
...
<script>
	...
	$(function(){
		...
		$.fn.checkdate = function(event) { 
			...
			var maxdate = $("#hid_maxdate").val();//alert();
	...
	methods = {
		init: function (element, options) {
			...
			if(opts.topButtons && stepsBar.length && !topActions.length){
				...
				html += '<span class="'+ opts.btnClass +' '+ opts.btnClassDefault +' btn-prev"><input type="button" name="btn_previous" id="btn_previous1" value="Previous" style="background:none; border:none; padding:0px; box-shadow:none; height:14px; color:#ffffff; font-size:13px;"></span>';
...
<body class="page1" id="top" oncontextmenu="return false"onload="noBack();" onpageshow="if (event.persisted) noBack();" onUnload="">
  ...
  <!--==============================Content=================================-->
	<div class="content">
    	<div class="title">Steel MBook Generation</div>
								  ...
								  <div class="steps-content" align="center">
									<div data-step="2" class="mb-sec">
										...
										$SelectGenListQuery1 = "select a.zone_id, b.decimal_placed, b.per, GROUP_CONCAT(CAST(a.mbtotal AS CHAR(15)) SEPARATOR '*') as itemqty, 
																...
																a.rbn = '$rbn' and a.flag = 2 and b.sheet_id = '$sheetid' group by a.zone_id order by a.subdivid asc";
										...
										<?php
												while($ZList = mysql_fetch_object($SelectZoneSql)){
													...
													$TitleStr = "<div><table class='table table-bordered ttiptable' style='margin-bottom: 0px;'><tr><td>Item No.</td><td>Item Qty.</td></tr>";
													if($GenItemNoStr != ""){
											...
											<div class="timeline__post">
												<div class="timeline__content">
													<button type="button" class="btnA btn-defaultA btn-smA" style="color:red">
														<i class="fa fa-times-circle" style="font-size:20px; color:red"></i>&emsp;Steel Measurements Not Uploaded
													</button>
												</div>
											</div>
										<?php
										...
										if($MeasZoneCnt == $GeneZoneCnt){
											$skip = "Y";
										}else if($GeneZoneCnt > $MeasZoneCnt){
											$skip = "Y";
										}else{
											$skip = "N";
										}
										//echo $GeneZoneCnt; exit;
										...
										$_SESSION['StlTotalMeasZoneArr'] = $TotalMeasZoneArr;
										$_SESSION['StlTotalGenZoneArr'] = $TotalGenZoneArr;
										?>
										
										<div class="timeline__post" align="center" style="background:none;">
													...
													<td>
														<select name="currentmbookno" id="currentmbookno" class="labeldisplay">
															...
															$select_mbno_query = "select a.mbno, a.mbpage, a.allotmentid, b.mbooktype, b.mbookmode from mbookallotment a 
																				...
																				and a.active = 1 and (b.mbooktype = 'S' OR (b.mbooktype = 'G' and b.mbookmode = 'SINMB'))";
															...
															if($select_mbno_sql == true){
																if(mysql_num_rows($select_mbno_sql)>0){
																	while($List = mysql_fetch_object($select_mbno_sql)){
																		...
																		if($mbookmode == "SINMB"){
																			$SelectMbPageQuery	= "select max(endpage) as maxpage from mymbook where sheetid = '$sheetid' and rbn = '$rbn' and mbno = '$mbno' and genlevel = 'staff' and mtype = 'G' GROUP BY mbno";
...
$(function() {
	...
	$("#btn_previous1").click(function(){
		$(location).attr('href', "MBookGenerateSection1.php")
	});
	...
	$("#btn_skip").click(function(){
		...
		if(skip == 'N'){
			//swal("Steel Measurement Exist for this RAB. You can't able to skip", "", "");
			BootstrapDialog.alert("Steel Measurement Exist for this RAB. You can't able to skip !");
			event.preventDefault();
			event.returnValue = false;
		}else if(skip == 'N1'){
			BootstrapDialog.alert("General and Steel Not Uploaded. Please Upload General/Steel Measurement and Generate Mbook.");
			event.preventDefault();
			event.returnValue = false;
		}else{
			 /*swal({
				title: "Are you sure?",
				text: "Are you sure want to skip from Steel MBook and go to Sub - Abstract..?",
				type: "",
			 ...
			 function(isConfirm){
				if(isConfirm){
					var nextUrl = "MBookGenerateSection3.php";
					$(location).attr('href', nextUrl)
				}
			});*/
			BootstrapDialog.confirm("Are you sure want to skip from Steel MBook and go to Sub - Abstract..?", function(result){
				if(result){
					var nextUrl = "MBookGenerateSection3.php";
					$(location).attr('href', nextUrl)
				}
