...
if(isset($_POST["txt_test"]))
{
	...
	else
	{
		...
		/*if($mtype == "G")
		{
			header('Location: MBook_Print_staff_wise_Accounts.php?workno='.$sheetid);
		}
		if($mtype == "S")
		{
			header('Location: SteelMBook_Print_staff_wise_Accounts.php?workno='.$sheetid);
		}
		if($mtype == "SABS")
		{
			header('Location: GeneralMBook_Composite_Print_Accounts.php?workno='.$sheetid);
		}
		if($mtype == "ABS")
		{
			header('Location: AbstMBook_Print_Common_Accounts.php?workno='.$sheetid);
		}*/
	}
	//echo $dataStr;
}
                        ...
                        <div class="container">
							...
							if($sheet_count == 1)
							{
								while($SheetList = mysql_fetch_object($select_sheet_sql))
								{
									...
									$select_acc_mb_query = "select * from send_accounts_and_civil where sheetid = '$sheetid' and rbn = '$rbn' and level>='$staff_levelid' ORDER BY mbookno ASC";
							...
							echo '<table width="100%" align="center" class="acctext table1">';
									...
									echo '</tr>';
											...
											while($AccMBList = mysql_fetch_object($select_acc_mb_sql))
											{
												...
												$acc_staff_L1 = $AccMBList->acc_staffid_L1;
												$acc_staff_L2 = $AccMBList->acc_staffid_L2;
												
												$acc_staff_name_L1_str 	= getstafflevel($acc_staff_L1);
												$exp_L1_Str 			= explode("@#*#@",$acc_staff_name_L1_str);
												$acc_staff_name_L1 		= $exp_L1_Str[2];
												if($acc_staff_L2 != 0)
												{
													$acc_staff_name_L2_str 	= getstafflevel($acc_staff_L2);
													$exp_L2_Str 			= explode("@#*#@",$acc_staff_name_L2_str);
													$acc_staff_name_L2 		= $exp_L2_Str[2];
												}
												else
												{
													$acc_staff_name_L2 = "";
												...
												if($AccMBList->genlevel == 'staff')
												{
													...
													{
														if($acc_staff_name_L2 != "")
														{
															$level_2_status = "<br/> <i class='fa fa-check' style='color:#14AF52; font-size:19px;' aria-hidden='true'></i> Verified by ".$acc_staff_name_L2;
														}
														else
														{
															$level_2_status = "";
														}
														$status = " <i class='fa fa-check' style='color:#14AF52; font-size:19px;' aria-hidden='true'></i> Verified by ".$acc_staff_name_L1.$level_2_status;
													}
													$link = "ViewMBook(this)";
													$title = "";
													$tooltipClass = "";
