...
$select_itemlist_sql 	= 	"select subdivision.subdiv_id, subdivision.subdiv_name, subdivision.div_id, schdule.per, schdule.measure_type from subdivision INNER JOIN schdule ON (schdule.subdiv_id = subdivision.subdiv_id) where subdivision.sheet_id = '$sheet_id' and subdivision.active = '1'";
...
{				
	if(in_array($mitemno, $ItemNoList))
	{
		...
		//$mbookdetail_sql_1		= 	"update mbookdetail_temp set subdivid = '$subdivid', subdiv_name = '$mitemno', mbdetail_flag = '$errorflag' where mbheaderid  = '$mbheaderid'";
...
if(($mitemno != "") && ($description != "") && ($mdate != ""))
{
	if($description != "")
	{
		...
		$type = $TypeList[$mitemno];
		$errorflag 				 =  $mitemno."@@@".$deflag."@@@".$ieflag."@@@".""."@@@".$dteflag;
		$mbookdetail_sql 	= 	"update mbookdetail_temp set subdivid = '$subdivid', subdiv_name = '$mitemno', descwork = '$description', measurement_no = '$number', measurement_l = '$length', measurement_b = '$breadth', measurement_d = '$depth', measurement_contentarea = '$contentarea', measure_type = '$type', remarks = '$itemunit', mbdetail_flag = '$errorflag', entry_date = NOW() where mbdetail_id  = '$mbdetailid'";
		$mbookdetail_query 	= 	mysql_query($mbookdetail_sql);
		
		$mbookheader_sql 	= 	"update mbookheader_temp set date = '$mdate', divid = '$divid', subdivid = '$subdivid', subdiv_name = '$mitemno', measure_type = '$type' where mbheaderid  = '$mbheaderid'";
