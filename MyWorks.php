...
$SelDisciplineQuerySql = mysql_query($SelDisciplineQuery);
if($SelDisciplineQuerySql == true){
	if(mysql_num_rows($SelDisciplineQuerySql)>0){
		while($DispList = mysql_fetch_object($SelDisciplineQuerySql)){
...
$SelectStaffListSql = mysql_query($SelectStaffListQuery);
if($SelectStaffListSql == true){
	if(mysql_num_rows($SelectStaffListSql)>0){
		while($AllStaffList = mysql_fetch_object($SelectStaffListSql)){
															...
															<div class="table-responsive dt-responsive rtabdiv" id="table-stmt">
																...
																<div align="center">
																	...
																	<div class="div12 no-padding-lr" align="center">
																		...
																		<div class="col-status" data-id='L'><div class="well well-sm well-A <?php if($View == "L"){ ?> active <?php } ?>"><span class="rlable-pink">Live Works</span></div></div>
																		<div class="col-status" data-id='C'><div class="well well-sm well-A <?php if($View == "C"){ ?> active <?php } ?>"><span class="rlable-pink">Completed Works</span></div></div>
																	</div>
																</div>
																		...
																		<dl>
																			<dt>
																				<a href="#accordion<?php echo $sheetid; ?>" id="sheet-<?php echo $sheetid; ?>" aria-expanded="false" aria-controls="accordion<?php echo $sheetid; ?>" class="accordion-title accordionTitle js-accordionTrigger blue-bg <?php if($x == 1){ ?> is-collapsed is-expanded <?php } ?>">
																					...
																					&nbsp;&nbsp;<font id="BillStatus" data-id="<?php echo $sheetid; ?>" style="color:#ffffff; font-weight:bold; background:#10478A; border-radius:7px;  padding:1px 6px 1px 6px; display:inline-block; line-height: 17px; margin-top:4px;">RAB Generate & Bill Status</font>
																				</a>
																			</dt>
...
$(document).ready(function(){ 
	...
	$('.col-status').click(function(event){
		...
		if((id != undefined)&&(id != '')){
			$(location).attr('href', 'MyWorks.php?list='+id);
		}
	});
