	...
	if($ResultRbn != ""){
		...
		foreach($ExpResRbn as $RabValue){
			...
			if($CurrRbn == $rbn){
				...
				/*foreach($CurrParDpmMbIdArr as $K => $V){ 
					$TESTING1 = $_POST['ChiDpmAddQty'.$CurrRbn.'PID'.$V];
					$TESTING2 = $_POST['ChiDpmAddPerc'.$CurrRbn.'PID'.$V];
					$TESTING3 = $_POST['ChiDpmAddAmount'.$CurrRbn.'PID'.$V];
					echo $V."  ";
					print_r($TESTING1);print_r($TESTING2);print_r($TESTING3);
					echo "<br/>";
				}*/
				
				//print_r($CurrParDpmMbIdArr); print_r($CurrChiParDpmMbIdArr); echo "dd<br/>";
				//exit;
				
				//$ChildQtyRow = count($CurrChiDpmQtyArr);
				
				//$DeleteSlmPartPayQuery = "DELETE FROM measurementbook_temp WHERE sheetid = '$sheetid' AND rbn = '$rbn' AND subdivid = '$subdivid' AND part_pay_flag != '0' AND part_pay_flag != '1'";
				//$DeleteSlmPartPaySql = mysql_query($DeleteSlmPartPayQuery);
				...
				//else if($ChildQtyRow > 0){ 
					...
					//print_r($RunnParMbIdArr);
					/*foreach($CurrChiDpmQtyArr as $ChiDpmQtyKey => $ChiDpmQtyValue){
						$CurrChiDpmQtyValue 		= $ChiDpmQtyValue;
						$CurrChiDpmPercValue 		= $CurrChiDpmPercArr[$ChiDpmQtyKey];
						$CurrChiDpmAmtValue 		= $CurrChiDpmAmtArr[$ChiDpmQtyKey];
						$CurrChiParDpmMbId 			= $CurrChiParDpmMbIdArr[$ChiDpmQtyKey];
						$CurrRunnParMbId 			= $RunnParMbIdArr[$CurrChiParDpmMbId];
						if($CurrChiDpmPercValue == 0){ $rpmbid = $CurrParDpmMbIdValue; }else{ $rpmbid = $RunnParMbId; } 
						//echo "HI".$rpmbid."<br/>";
						//$InsertPPayQtySplitQuery 	= "insert into pp_qty_splt set mbid = '$CurrRunnParMbId', sheetid = '$sheetid', rbn = '$rbn', subdivid = '$subdivid', qty = '$CurrChiDpmQtyValue', percent = '$CurrChiDpmPercValue', rate = '$rate', gpmbid = '$CurrChiParDpmMbId', rpmbid = '$rpmbid', gppayid = '$gppayid', rppayid = '$rppayid', createddate = NOW(), staffid = '$staffid', userid = '$userid' ";
						$InsertPPayQtySplitSql 		= mysql_query($InsertPPayQtySplitQuery);
					}*/
				//}
			...
			/*$MbDPercArr = $_POST['txt_curr_perc'.$CurrRbn];
			$MBDIdArr = $_POST['txt_mbd_id'.$CurrRbn];
			foreach($MbDPercArr as $MbDPercKey => $MbDPercValue){
				if(($MbDPercValue != "")&&($MbDPercValue > 0)){
					$MBDId = $MBDIdArr[$MbDPercKey];
					$UpdatePercQuery 	= "update mbookdetail set curr_paid_perc = '$MbDPercValue', curr_paid_rbn = '$rbn' where mbdetail_id  = '$MBDId'";
					$UpdatePercSql 		= mysql_query($UpdatePercQuery);
					//echo $UpdatePercQuery."<br/>";
					//echo $UpdatePercQuery."<br/>";
				}
...
<form style="height:100%;" method="post" action="">
                            ...
                            <div class="header-top-wraper">
                                <div class="row">
                                            ...
                                            <ul class="nav navbar-nav mai-top-nav col-md-12 no-padding padding-l">
                                                <li class="nav-item col-md-12 no-padding-lr padding-l">
													<a class="nav-link" style="padding:14px 5px 20px 5px;">
														<!--Item No. -->
														<span class="badge col-md-11 desc-title">
									...
									<!--<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
										...
										<div class="col-sm-5">1200.000</div>
										<div class="text-sm text-sm-box text-white desc-text"><?php echo $Description; ?></div>
									</div>-->
            ...
            <div class="container-fluid">
                <div class="row">
								...
								<!--<div class="container col-md-12">-->
									<div class="pricing card-deck flex-column flex-md-row mb-3">
										<div class="card card-pricing text-center px-3 mb-4">
											<span class="center-header head-color1">Since Last Measurement</span>
											<div class="card-body pt-0">
												<table class="table mtab SlmTab" id="SLMaddrTab<?php echo $rbn; ?>">
													...
													<tbody>
														<?php if($SlmRowCnt2 == 1){ $SlmPaidSlno = 1; foreach($SLMQtyArr as $QtyKey=>$QtyValue){ ?>
														<tr id='SlmAddR0' class="SLMROW<?php echo $rbn; ?>">
															...
															<td><input type="text" name='ParSlmAddQty<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r ResCls SlmQty" data-rbn="<?php echo $rbn; ?>" value="<?php echo round($QtyValue,$Decimal); ?>"/></td>
															...
															<td><input type="text" name='ParSlmAddPerc<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r ResCls SlmPerc"  data-rbn="<?php echo $rbn; ?>" value="100"/></td>
															<td><input type="text" name='ParSlmAddAmount<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r SlmAmt ronly" readonly="" value="<?php echo round(($QtyValue*$ItemRate),2); ?>"/></td>
															<td>
									...
									<div class="pricing card-deck flex-column flex-md-row mb-3">
										<div class="card card-pricing text-center px-3 mb-4">
											<span class="center-header">Deduct Previous Measurement</span>
											<div class="card-body pt-0">
												<table class="table mtab SlmTab" id="DPMaddrTab<?php echo $rbn; ?>">
													<thead>
														<tr>
															<td class="text-center" width="40px">RAB</td>
													...
													<tbody>
														<?php 
														$DistPPParIdArr = array();
														$SelectDistParIdQuery = "select gpmbid, rpmbid from pp_qty_splt where sheetid = '$sheetid' and rbn < '$rbn' and subdivid = '$subdivid' group by gpmbid, rpmbid";
														$SelectDistParIdSql = mysql_query($SelectDistParIdQuery);
														if($SelectDistParIdSql == true){
															if(mysql_num_rows($SelectDistParIdSql)>0){
																while($DistPPParIdList = mysql_fetch_object($SelectDistParIdSql)){
																	if(in_array($DistPPParIdList->gpmbid,$DistPPParIdArr)){
																		// Already Exist
																	}else{
																		array_push($DistPPParIdArr,$DistPPParIdList->gpmbid);
																	}
																	//if(in_array($DistPPParIdList->rpmbid,$DistPPParIdArr)){
																		// Already Exist
																	//}else{
																		//array_push($DistPPParIdArr,$DistPPParIdList->rpmbid);
																	//}
																}
															}
														}
														//print_r($DistPPParIdArr);
														if($DpmRowCnt2 == 1){ $DpmPaidSlno = 1; while($DpmPaidList = mysql_fetch_object($SelectDpmPaidSql)){ 
															...
															if($DpmPaidList->qty_split == 'Y'){
																$RowDisp = 0;
															}else{
																$RowDisp = 1;
															}//// HERE '$RowDisp' INDICATES THAT IF IT IS '0' - THEN IT IS CHILD ROW SO NO NEED TO DISPLAY HERE
															if($RowDisp == 1){
																if(in_array($DpmPaidList->measurementbookid,$DistPPParIdArr)){
																	$PPayExist = 1;
																	$CheckPPayQtySplitIdStr = $DpmPaidList->part_pay_flag;
																	$ExpCheckPPayQtySplitIdStr = explode("*",$CheckPPayQtySplitIdStr);
																	$CheckPPayQtySplitId = $ExpCheckPPayQtySplitIdStr[2];
																	$SelectQtySplitQuery = "select * from (select * from pp_qty_splt order by gpmbid, rpmbid) pp_qty_splt_abc, (select @pv := '$DpmPaidList->measurementbookid') initialisation where find_in_set(gpmbid, @pv) and length(@pv := concat(@pv, ',', rpmbid)) and rbn < '$rbn'";
																	//echo $SelectQtySplitQuery;
																...
																if($PPayExist == 1){ $RonlyClass = " ronly"; }else{ $RonlyClass = ""; }
														?>
														...
														<tr id='DpmAddR<?php echo $DpmPaidSlno; ?>' class="DpmAddR<?php echo $DpmPaidSlno; ?> DPMROW<?php echo $DpmPaidList->rbn; ?> DpmPId<?php echo $DpmPaidList->measurementbookid; ?>">
															...
															<td><input type="text" name='ParDpmAddPerc<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ResCls DpmPerc <?php echo $RonlyClass; ?>" data-rbn="<?php echo $DpmPaidList->rbn; ?>" value=""/></td>
															<td><input type="text" name='ParDpmAddAmount<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmAmt" readonly="" value="<?php echo "0.00";//$DpmPaidList->measurementbookid; ?>"/></td>
															<td>
																<?php if($PPayExist == 0){ ?>
																<i class="fa faAdd DpmAddRow" data-id="<?php echo $rbn; ?>" data-rowClass="<?php echo $DpmPaidSlno; ?>" data-rbn="<?php echo $DpmPaidList->rbn; ?>" data-paidPerc='<?php echo $DpmPaidList->pay_percent; ?>' data-mbid="<?php echo $DpmPaidList->measurementbookid; ?>">&#xf055;</i>
														...
														<?php }else{ ?>
														<tr id='' class="">
															<td><input type="text" name='ParDpmRAB' class="form-control small-tbox tbox-c ronly" readonly="" value="<?php echo $DpmPaidList->rbn; ?>"/></td>
															<td><input type="text" name='ParDpmAddQty' class="form-control small-tbox tbox-r ronly" readonly="" value="<?php echo $DpmPaidList->mbtotal; ?>"/></td>
															<td><input type="text" name='ParDpmAddRate' class="form-control small-tbox tbox-r ronly" readonly="" value="<?php echo $ItemRate; ?>"/></td>
															<td><input type="text" name='ParDpmPaidPerc' class="form-control small-tbox tbox-r ronly" value="<?php echo $DpmPaidList->pay_percent; ?>"/></td>
															<td><input type="text" name='ParDpmAddPerc' class="form-control small-tbox tbox-r ronly" data-rbn="<?php echo $DpmPaidList->rbn; ?>" value=""/></td>
															<td><input type="text" name='ParDpmAddAmount' class="form-control small-tbox tbox-r ronly" readonly="" value=""/></td>
															<td></td>
														</tr>
														<?php } ?>
														<?php 
															if($PPayExist == 1){ while($PPRowList = mysql_fetch_object($SelectQtySplitSql)){ 
																$TotalPercentpaid = $ToBeAddPercent + $PPRowList->percent; 
																if($PPRowList->percent == 0){
																	$PPayPerId = $PPRowList->gpmbid;
																}else{
																	$PPayPerId = $PPRowList->rpmbid;
																}
														?>
														<tr id='DpmAddR<?php echo $DpmPaidSlno; ?>' class="DpmAddR<?php echo $DpmPaidSlno; ?> DPMROW<?php echo $DpmPaidList->rbn; ?> DpmPId<?php echo $PPayPerId; ?>">
															<td><input type="text" name='ParDpmRAB<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-c ronly" readonly="" value="<?php echo $DpmPaidList->rbn; ?>"/></td>
															<td><input type="text" name='ParDpmAddQty<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ronly ResCls DpmQty DpmQty<?php echo $DpmPaidList->rbn; ?>" readonly="" data-rbn="<?php echo $DpmPaidList->rbn; ?>" value="<?php echo $PPRowList->qty; ?>"/></td>
															<td><input type="text" name='ParDpmAddRate<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmRate" readonly="" value="<?php echo $ItemRate; ?>"/></td>
															<td><input type="text" name='ParDpmPaidPerc<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmPaidPerc" value="<?php echo $TotalPercentpaid; ?>"/></td>
															<td><input type="text" name='ParDpmAddPerc<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ResCls DpmPerc" data-rbn="<?php echo $DpmPaidList->rbn; ?>" value="SS"/></td>
															<td><input type="text" name='ParDpmAddAmount<?php echo $DpmPaidList->rbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmAmt" readonly="" value="<?php echo "0.00";//$PPayPerId; ?>"/></td>
															<td>
																<i class="fa faAdd DpmAddRow" data-id="<?php echo $rbn; ?>" data-rowClass="<?php echo $DpmPaidSlno; ?>" data-rbn="<?php echo $DpmPaidList->rbn; ?>" data-paidPerc='<?php echo $TotalPercentpaid; ?>' data-mbid="<?php echo $PPayPerId; ?>">&#xf055;</i>
																<input type="hidden" name="ParDpmMbId<?php echo $DpmPaidList->rbn; ?>[]" value="<?php echo $PPayPerId; ?>">
																<input type="hidden" name="ParDpmMbIdList<?php echo $DpmPaidList->rbn; ?>[]">
															</td>
														</tr>
														<?php } } ?>
														...
														<tr id='DpmAddR1' style="border:1px solid #E9E9E9; color:#999999">
															<td colspan="7"> No Measurements Found </td>
														</tr>
														<?php } ?>
													</tbody>
												</table>
                    ...
                    <div class="col-md-8 col-md-8 col-sm-8 col-xs-12">
                        <div class="hpanel">
                            <div class="panel-body no-padding">
                                <div class="row">
                                    <div class="col-md-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="btn-group ib-btn-gp active-hook mail-btn-sd mg-b-15">
											...
											<?php $BDtSlno = 1; if($RABCount > 0){ foreach($RABArr as $RABKey=>$RABValue){ if($BDtSlno == 1){ $BDtFirstRAB = $RABValue; $BDtClass = ' active'; }else{ $BDtClass = ''; }  $BDtSlno++; ?>
											<button type="button" class="btn btn-default btn-sm BalanceBtn<?= $BDtClass; ?>" id="BalanceBtn<?= $RABValue; ?>" data-id="<?= $RABValue; ?>"><?= $RABValue; ?></button>
											<?php } } ?>
                                        </div>
                                    </div>
                                </div>
								<?php 
								if($RABCount > 0){ foreach($RABArr as $RABKey=>$RABValue){
								$DetRbn = $RABValue; $fromdate = $RABDateArr[$DetRbn][0]; $todate = $RABDateArr[$DetRbn][1]; $PrevDate = "";
								...
								//$SelectDetailsQuery = "select a.date, b.mbdetail_id, b.descwork, b.measurement_contentarea, b.prev_parent_id, b.prev_paid_perc from mbookheader a inner join mbookdetail b on (b.mbheaderid = a.mbheaderid) where a.sheetid = '$sheetid' and a.subdivid = '$subdivid' and a.date >= '$fromdate' and a.date <= '$todate' order by a.date asc, b.mbheaderid asc, b.mbdetail_id asc limit 10";
					...
					$SelectDetailsQuery = "select a.date, b.mbdetail_id, b.descwork, b.measurement_contentarea, b.prev_parent_id, b.prev_paid_perc from mbookheader a inner join mbookdetail b on (b.mbheaderid = a.mbheaderid) where a.sheetid = '$sheetid' and a.subdivid = '$subdivid' and a.date >= '$fromdate' and a.date <= '$todate' order by a.date asc, b.mbheaderid asc, b.mbdetail_id asc";
								...
								//$SelectDetailsQuery = "select b.mbdetail_id, b.descwork, b.measurement_contentarea, b.prev_parent_id, b.prev_paid_perc from mbookdetail b where b.subdivid = '$subdivid'";//and a.date >= '$fromdate' and a.date <= '$todate' order by a.date asc, b.mbheaderid asc, b.mbdetail_id asc";
								...
								?>
                                <div class="ib-tb BalanceDetails" id="BalanceDetails<?= $RABValue; ?>">
                                    <!--<table class="table table-hover table-mailbox">-->
									<table class="table table-bordered dettab table-hover example" id="BalTable<?= $RABValue; ?>">
										<thead>
											<tr>
                                        ...
                                        <tbody>
										<?php $PrevParIdArr = array(); while($DetList = mysql_fetch_object($SelectDetailsSql)){ 
											...
											if(in_array($prev_parent_id , $PrevParIdArr)){
												// Already Exist so no need
											}else{
												//if($prev_parent_id != 0){
													array_push($PrevParIdArr,$prev_parent_id);
												//}
											}
										?>
                                            <tr class="unread">
                                                ...
                                                <td align="right"><?php echo $TotalpaidPerc;//."*".$prev_parent_id; ?></td>
												<td nowrap="nowrap" width="40px" class="PercBox" data-id="<?php echo $DetList->mbdetail_id; ?>">
													<input type="text" name="txt_curr_perc<?php echo $RABValue; ?>[]"  data-type='S' data-rbn="<?php echo $RABValue; ?>" id="txt_curr_perc<?php echo $DetList->mbdetail_id; ?>" data-id="<?php echo $DetList->mbdetail_id; ?>" data-qty="<?php echo $DetList->measurement_contentarea; ?>" data-prev_par_id = "<?php echo $prev_parent_id; ?>" class="form-control small-tbox tbox-r FixCurrPerc<?php echo $RABValue; ?> fixAll PreParId<?php echo $prev_parent_id; ?>" value="">
												</td>
												<td align="center">
												<i class="fa fa-check-circle fa-default SRowCheck" id="SRow<?php echo $DetList->mbdetail_id; ?>" style="font-size:24px"></i>
												</td>
                                            </tr>
									<?php $slno++; } ?>
                                        </tbody>
							...
							<input type="hidden" name="txt_rbn" id="txt_rbn" value="<?php echo $_GET['rbn']; ?>">
							...
							<input type="hidden" name="txt_result" id="txt_result" value="">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <?php include("JSLibrary.php"); ?>
        ...
        <div class="fixed-bottom" style="text-align:center">
			...
			<div class="col-md-3"><button type="button" name="PayableAmt" id="btn_info" class="btn btn-success col-md-12" disabled="disabled" style="opacity:1; text-align:left">Total Amount : &nbsp; &#8377 <span class="badge" style="color:#000000; font-size:13px;" id="TotalPayableAmt"> <?php echo $SLMTotalAmt; ?> </span> </button></div>
	...
	$(document).ready(function(){
		...
		$(".SlmAddRow").click(function(){ 
			...
			var Rate 	 = Number($('#txt_rate').val());
				Rate = Rate.toFixed(2);
			$('#SLMaddrTab'+dataid+' tr:last').after("<tr class='SLMROW"+rbn+"'><td><input type='text' name='ChiSlmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiSlmAddQty"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmQty' data-rbn='"+rbn+"' /></td><td><input name='ChiSlmAddRate"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiSlmAddPerc"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmPerc' data-rbn='"+rbn+"'></td><td><input name='ChiSlmAddAmount"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmAmt' readonly=''></td><td><i class='fa faDel SlmDelRow' data-rbn='"+rbn+"'>&#xf057;</i></td></tr>");
	  		i++;
			$(this).closest("tr").find(".SlmPerc").prop("readonly", true);
			$(this).closest("tr").find(".SlmPerc").addClass("ronly");
			$(this).closest("tr").find(".SlmPerc").val("");
			$(this).closest("tr").find(".SlmQty").prop("readonly", true);
			$(this).closest("tr").find(".SlmQty").addClass("ronly");
			$(this).closest("tr").find(".SlmAmt").val("");
		...
		$(".DpmAddRow").click(function(){ 
			...
			var rbn 		= $(this).attr("data-rbn");
			var PaidPerc 	= $(this).attr("data-paidPerc");
			var Rate 	 	= Number($('#txt_rate').val());
				Rate 		= Rate.toFixed(2);
			$('.DpmAddR'+rowClass).after("<tr class='DPMROW"+rbn+"'><td><input type='text' name='ChiDpmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiDpmAddQty"+rbn+"PID"+mbid+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmQty DpmQty"+rbn+"' data-rbn='"+rbn+"' /></td><td><input name='ChiDpmAddRate"+rbn+"PID"+mbid+"[]' type='text' class='form-control input-md small-tbox ronly tbox-r DpmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiDpmPaidPerc"+rbn+"PID"+mbid+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly' readonly='' value='"+PaidPerc+"'></td><td><input name='ChiDpmAddPerc"+rbn+"PID"+mbid+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmPerc' data-rbn='"+rbn+"'></td><td><input name='ChiDpmAddAmount"+rbn+"PID"+mbid+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly DpmAmt' value='0.00' readonly=''></td><td><i class='fa faDel DpmDelRow DpmDelRow"+rbn+"' data-rbn='"+rbn+"'>&#xf057;</i><input type='hidden' name='ChiParDpmMbId"+rbn+"PID"+mbid+"[]' value='"+mbid+"'><input type='hidden' name='ChiParDpmMbIdList"+rbn+"PID"+mbid+"[]' value=''></td></tr>");
	  		i++;
			$(this).closest("tr").find(".DpmPerc").prop("readonly", true);
			$(this).closest("tr").find(".DpmPerc").addClass("ronly");
		});
		...
		$('body').on('change','.SlmQty',function(){
			...
			var CurrAmt = Number(CurrQty)*Number(Rate)*Number(CurrPerc)/100;
				CurrAmt = CurrAmt.toFixed(Decimal);
		...
		$('body').on('click','.SlmDelRow',function(){
			var rbn = $(this).attr('data-rbn');
			$(this).parents("tr").remove();
			var ParChilRowCnt = $('.SLMROW'+rbn).length;
			if(ParChilRowCnt == 1){
				$('.SLMROW'+rbn).find(".SlmPerc").prop("readonly", false);
				$('.SLMROW'+rbn).find(".SlmPerc").removeClass("ronly");
				$('.SLMROW'+rbn).find(".SlmQty").prop("readonly", false);
				$('.SLMROW'+rbn).find(".SlmQty").removeClass("ronly");
			}
			var Qty = $(this).val();
			var TotalSlmQty = 0;
			var Decimal = Number($('#txt_decimal').val());
			...
			$(".CArea"+rbn).each(function() {
				...
				CAreaQtyTotal = Number(CAreaQtyTotal)+Number(qty);
				if(CAreaQtyTotal != 0){ CAreaQtyTotal = CAreaQtyTotal.toFixed(Decimal); }
				if(Number(CAreaQtyTotal) <= Number(TotalSlmQty)){
					$("#SRow"+mbdid).addClass("fa-active");
					$("#SRow"+mbdid).removeClass("fa-default");
					TotalRowSel++;
				}else{
		...
		$('body').on('click','.DpmDelRow',function(){
			var rbn = $(this).attr('data-rbn');
			$(this).parents("tr").remove();
			var ParChilRowCnt = $('.DPMROW'+rbn).length;
			if(ParChilRowCnt == 1){
				$('.DPMROW'+rbn).find(".DpmPerc").prop("readonly", false);
				$('.DPMROW'+rbn).find(".DpmPerc").removeClass("ronly");
			}
			var Qty = $(this).val();
			var TotalDpmQty = 0;
			var Decimal = Number($('#txt_decimal').val());
		...
		$('body').on('change','.SlmPerc',function(){
			var Decimal = Number($('#txt_decimal').val());
			var Rate = Number($('#txt_rate').val());
			var CurrQty = $(this).closest('tr').find('.SlmQty').val();
			var CurrPerc = $(this).val();
			var CurrAmt = Number(CurrQty)*Number(Rate)*Number(CurrPerc)/100;
				CurrAmt = CurrAmt.toFixed(Decimal);
			$(this).closest('tr').find('.SlmAmt').val(CurrAmt);
			CalculateTotalAmounts();
			...
			$(".CArea"+rbn).each(function() {
				...
				CAreaQtyTotal = Number(CAreaQtyTotal)+Number(qty);
				if(CAreaQtyTotal != 0){ CAreaQtyTotal = CAreaQtyTotal.toFixed(Decimal); }
				if((CAreaQtyTotal <= TotalSlmQty)&&(CurrPerc > 0)){
					$("#SRow"+mbdid).addClass("fa-active");
					$("#SRow"+mbdid).removeClass("fa-default");
					TotalRowSel++;
				}else{
		...
		$('body').on('change','.DpmPerc',function(){
			var Decimal = Number($('#txt_decimal').val());
			var Rate = Number($('#txt_rate').val());
			var CurrQty = $(this).closest('tr').find('.DpmQty').val();
			var CurrPerc = $(this).val();
			var CurrAmt = Number(CurrQty)*Number(Rate)*Number(CurrPerc)/100;
				CurrAmt = CurrAmt.toFixed(Decimal);
			$(this).closest('tr').find('.DpmAmt').val(CurrAmt);
			...
			$(".CArea"+rbn).each(function() {
				...
				if(CAreaQtyTotal != 0){ CAreaQtyTotal = CAreaQtyTotal.toFixed(Decimal); }
				//alert(qty+" == "+CAreaQtyTotal+" == "+TotalDpmQty);
				if((Number(CAreaQtyTotal) <= Number(TotalDpmQty))&&(CurrPerc > 0)){ 
					$("#SRow"+mbdid).addClass("fa-active");
					$("#SRow"+mbdid).removeClass("fa-default");
					TotalRowSel++;
				}else{
		...
		$('body').on('change','.ResCls',function(){
			var rbn = $(this).attr('data-rbn');
			$(".BalanceDetails").each(function() {
				$(this).hide();
			});
		...
		/*$(".fixAll").change(function(event){
			...
			if(TotalQty>0){
				TotalQty = TotalQty.toFixed(Decimal);
			}
			//alert(MbDetIdStr);
			...
			var Rate 	 = Number($('#txt_rate').val());
				Rate = Rate.toFixed(2);
			var SlmAmt = Number(TotalQty)*Number(Rate);
				SlmAmt = SlmAmt.toFixed(2);
			$('#SLMaddrTab'+dataid+' tr:last').after("<tr class='SLMROW"+rbn+"'><td><input type='text' name='ChiSlmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiSlmAddQty"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmQty' data-rbn='"+rbn+"' value='"+TotalQty+"' /></td><td><input name='ChiSlmAddRate"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiSlmAddPerc"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmPerc' data-rbn='"+rbn+"' value='"+FixPerc+"'></td><td><input name='ChiSlmAddAmount"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmAmt' readonly='' value='"+SlmAmt+"'></td><td><i class='fa faDel SlmDelRow' data-rbn='"+rbn+"'>&#xf057;</i></td></tr>");
	  		i++;
			CalculateTotalAmounts();
			...
			if(CurrRbn == rbn){
				...
				RowArr.forEach(function(elementQty, keyPercent) {
					...
					var SlmAmt = Number(elementQty)*Number(Rate)*Number(keyPercent)/100;
						SlmAmt = SlmAmt.toFixed(2);
					$('#SLMaddrTab'+dataid+' tr:last').after("<tr class='SLMROW"+rbn+"'><td><input type='text' name='ChiSlmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiSlmAddQty"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmQty' data-rbn='"+rbn+"' value='"+elementQty+"' /></td><td><input name='ChiSlmAddRate"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiSlmAddPerc"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls SlmPerc' data-rbn='"+rbn+"' value='"+keyPercent+"'></td><td><input name='ChiSlmAddAmount"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly SlmAmt' readonly='' value='"+SlmAmt+"'></td><td><i class='fa faDel SlmDelRow' data-rbn='"+rbn+"'>&#xf057;</i></td></tr>");
				});
			}else{
				var sed = 0; var RowArr = []; 
				$('#BalTable'+rbn).DataTable().column( 3 ).data().each( function ( value, index ) { //alert()
				...
				//alert();//alert(RowArrMulti.toString());
				$(".DpmDelRow"+rbn).each(function(){
					$(this).parents("tr").remove();
				});
				var ParChilRowCnt = $('.DPMROW'+rbn).length;
				$('.DPMROW'+rbn).find(".DpmPerc").prop("readonly", true);
				$('.DPMROW'+rbn).find(".DpmPerc").addClass("ronly");
				$('.DPMROW'+rbn).find(".DpmQty").prop("readonly", true);
				$('.DPMROW'+rbn).find(".DpmQty").addClass("ronly");
				$('.DPMROW'+rbn).find(".DpmPerc").val("");
				$('.DPMROW'+rbn).find(".DpmAmt").val("");
				var dataid = rbn;//$(this).attr("data-id");
				var rbn = rbn;//$(this).attr("data-id");
				var Rate = Number($('#txt_rate').val());
					Rate = Rate.toFixed(2);
				var PaidPerc = $('.DPMROW'+rbn).find(".DpmPaidPerc").val(); 
				var ParMbId = $('.DPMROW'+rbn).find(".DpmAddRow").attr('data-mbid');
				RowArr.forEach(function(elementQty, keyPercent) {
					if(ItemType == 'st'){
						elementQty = (elementQty / 1000);
						elementQty = elementQty.toFixed(Decimal);
					...
					var SlmAmt = Number(elementQty)*Number(Rate)*Number(keyPercent)/100;
						SlmAmt = SlmAmt.toFixed(2); //alert(elementQty);
					$('.DPMROW'+rbn).closest( "tr" ).after("<tr class='LDPMROW"+rbn+"'><td><input type='text' name='ChiDpmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiDpmAddQty"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmQty DpmQty"+rbn+"' data-rbn='"+rbn+"' value='"+elementQty+"' /></td><td><input name='ChiDpmAddRate"+rbn+"[]' type='text' class='form-control input-md small-tbox ronly tbox-r DpmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiDpmPaidPerc"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly ResCls' readonly='' value='"+PaidPerc+"'></td><td><input name='ChiDpmAddPerc"+rbn+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmPerc' data-rbn='"+rbn+"' value='"+keyPercent+"'></td><td><input name='ChiDpmAddAmount[]"+rbn+"' type='text' class='form-control input-md small-tbox tbox-r ronly DpmAmt' value='0.00' readonly='' value='"+SlmAmt+"'></td><td><i class='fa faDel DpmDelRow DpmDelRow"+rbn+"' data-rbn='"+rbn+"'>&#xf057;</i><input type='hidden' name='ChiParDpmMbId"+rbn+"[]' value='"+ParMbId+"'></td></tr>");
				});
			}
		...
		$(".fixAll").change(function(event){
			...
			if(CurrRbn == rbn){ //alert(4);
				...
				/*PrevParIdSpit.forEach(function(ParIdVal, ParIdKey) { 
					var CurrParIdVal = ParIdVal;
					$(".PreParId"+CurrParIdVal).each(function(){ 
						var CurrRowQty = $(this).attr('data-qty');
						var CurrRowPerc = $(this).val();
						alert(CurrRowQty+" = "+CurrRowPerc);
					});
				});*/
				...
				PrevParIdSpit.forEach(function(ParIdVal, ParIdKey) { 
					...
					var PercArrLen = RowArr.length; //alert(122);
