...
if((isset($_POST['btn_save']) == " Save ")||(isset($_POST['BalanceBtn']) != "")){ 
	...
	$rbn 		= $_POST['txt_rbn'];
	$rate 		= $_POST['txt_rate'];
	$ResultRbn  = $_POST['txt_result_2'];//$_POST['txt_result'];
...
if(($subdivid != '')&&($sheetid != '')&&($rbn != '')&&($PPrbn != '')){
	...
	$SLMQtyArr = array(); $SLMItemQty = 0; $SLMPercArr =array(); $SLMItemRowCnt = 0; $SLMParIDArr =array();
	$SelectSlmPaidQuery = "select * from measurementbook_temp where sheetid = '$sheetid' and subdivid = '$subdivid' and part_pay_flag != 'DMY'";
	$SelectSlmPaidSql 	= mysql_query($SelectSlmPaidQuery);
	if($SelectSlmPaidSql == true){
		if(mysql_num_rows($SelectSlmPaidSql)>0){
			$SlmRowCnt2 = 1;
			while($SlmQtyList = mysql_fetch_object($SelectSlmPaidSql)){
				if(($SlmQtyList->part_pay_flag == 0)||($SlmQtyList->part_pay_flag == 1)){
					$SLMPercentage = number_format($SlmQtyList->pay_percent, 2, '.', '');
					//echo $SlmQtyList->pay_percent;exit;
					array_push($SLMParIDArr,$SlmQtyList->measurementbookid);
					array_push($SLMQtyArr,$SlmQtyList->mbtotal);
					array_push($SLMPercArr,$SLMPercentage);
					$SLMItemQty = $SLMItemQty + $SlmQtyList->mbtotal;
					$SLMItemRowCnt++;
				}
			}
...
<form style="height:100%;" method="post" action="PartPayment.php">
													...
													<thead>
														<tr>
															...
															<th nowrap="nowrap" style="padding: 4px 4px;" align="center">
																...
																<div>
																<input type="text" name="fixAll<?php echo $RABValue; ?>[]" data-id="<?php echo $RABValue; ?>" class="form-control small-tbox tbox-r fixAll" data-type='A' onKeyPress="return isPercentageValue(event,this);">
																</div>
															</th>
									...
									<table class="table mtab SlmTab TabF" id="SLMaddrTab<?php echo $rbn; ?>" style="margin-bottom:6px">
										...
										<tbody>
											...
											<tr id='SlmAddR0' class="SLMROW<?php echo $rbn; ?>">
												...
												<td><input type="text" name='ParSlmAddQty' class="form-control small-tbox tbox-r ronly ParSlmAddQty" readonly="" data-rbn="<?php echo $rbn; ?>" value="<?php echo $MBGList->mbtotal; ?>"/></td>
											...
											//echo $SLMParIDArr[$QtyKey]."<br/>";
											//echo $SLMPercArr[$QtyKey]."<br/>";
											//echo $CurrParIdArr[$SLMParIDArr[$QtyKey]][$SLMPercArr[$QtyKey]]."<br/>";
											?>
											<tr id='SlmAddR0' class="SLMROW<?php echo $rbn; ?>">
												<td><input type="text" name='ChiSlmRAB<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-c ronly" readonly="" value="<?php echo $rbn; ?>"/></td>
												<td><input type="text" name='ChiSlmAddQty<?php echo $rbn; ?>[]' class="form-control small-tbox ResCls tbox-r SlmQty" data-rbn="<?php echo $rbn; ?>" value="<?php echo round($QtyValue,$Decimal); ?>" onKeyPress="return isDecimalValue(event,this);"/></td>
												<td><input type="text" name='ChiSlmAddRate<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r SlmRate ronly" readonly="" value="<?php echo $ItemRate; ?>"/></td>
												<td><input type="text" name='ChiSlmAddPerc<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r ResCls SlmPerc" readonly='' data-rbn="<?php echo $rbn; ?>" value="<?php echo $SLMPercArr[$QtyKey]; ?>"/></td>
												<td>
													<input type="text" name='ChiSlmAddAmount<?php echo $rbn; ?>[]' class="form-control small-tbox tbox-r SlmAmt ronly" readonly="" value="<?php echo $SLMItemAmount; ?>"/>
													<!--<i class="fa faAdd SlmAddRow" data-id="<?php echo $rbn; ?>" data-rbn="<?php echo $rbn; ?>">&#xf055;</i>-->
													<input type="hidden" name="ChiSlmMbIdList<?php echo $rbn; ?>[]" value="<?php echo $CurrParIdArr[$SLMParIDArr[$QtyKey]][$SLMPercArr[$QtyKey]]; ?>">
												</td>
										...
										if($SelectGrParDetailSql == true){
											if(mysql_num_rows($SelectGrParDetailSql)>0){
												while($GrParDtList = mysql_fetch_object($SelectGrParDetailSql)){
													...
													if($PPrbn == $GrParRbn){ $ActiveRABClass = "ActiveRAB"; $ActCellClass = ""; }else{ $ActiveRABClass = ""; $ActCellClass = "ronly"; }
													...
													?>
														...
														<tr id='DpmAddR<?php echo $DpmPaidSlno; ?>' class="DpmAddR<?php echo $DpmPaidSlno; ?> DPMROW<?php echo $GrParRbn; ?> DpmPId<?php echo $DistGrParId; ?> <?php echo $ActiveRABClass; ?>">
															<td><input type="text" name='ParDpmRAB<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-c ronly" readonly="" value="<?php echo $GrParRbn; ?>"/></td>
															<td><input type="text" name='ParDpmAddQty<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly ResCls DpmParQty DpmQty DpmQty<?php echo $GrParRbn; ?>" readonly="" data-parid="<?php echo $DistGrParId; ?>" data-rbn="<?php echo $GrParRbn; ?>" value="<?php echo $GrParQty; ?>"/></td>
															<td><input type="text" name='ParDpmAddRate<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmRate" readonly="" value="<?php echo $ItemRate; ?>"/></td>
															<td><input type="text" name='ParDpmAddPerc<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ResCls DpmPerc <?php echo $RonlyClass; ?>" readonly='' data-rbn="<?php echo $GrParRbn; ?>" value="<?php echo $GrParPercent; ?>"/></td>
															<td>
																<input type="text" name='ParDpmAddAmount<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmAmt" readonly="" value="<?php //echo $CurrPPayAmt;//"0.00";//$DpmPaidList->measurementbookid; ?>"/>
																<input type="hidden" name="ParDpmMbId<?php echo $GrParRbn; ?>[]" value="<?php echo $DistGrParId; ?>">
																<input type="hidden" name="ParDpmMbIdList<?php echo $GrParRbn; ?>[]">
																<input type="hidden" class="DpmAddRow" data-mbid="<?php echo $DistGrParId; ?>">
															</td>
														</tr>
														<?php 
														$CurrPPayCount = 0;
														...
														if($CurrPPayCount == 1){ 
															while($CurrPPayList = mysql_fetch_object($SelectCurrPPaySql)){ 
																$CurrPPayAmt = round(($CurrPPayList->qty * $CurrPPayList->percent * $ItemRate / 100),2); 
																if(in_array($GrParRbn,$ResRbnArr)){
																	// Already Exist so no need
																}else{
																	array_push($ResRbnArr,$GrParRbn);
																}
																$GPMBId = $CurrPPayList->gpmbid;
																$RPMBId = $CurrPPayList->rpmbid;
														?>
														<tr class='DpmAddR<?php echo $GrParRbn; ?>'>
															...
															<td><input name='ChiDpmAddQty<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' type='text' class='form-control input-md small-tbox tbox-rb <?php echo $ActCellClass; ?> ResCls DpmQty DpmChiQty<?php echo $GPMBId; ?> DpmQty<?php echo $GrParRbn; ?>' data-rbn='<?php echo $GrParRbn; ?>' value='<?php echo $CurrPPayList->qty; ?>' onKeyPress="return isDecimalValue(event,this);"/></td>
															<td><input name='ChiDpmAddRate<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' type='text' class='form-control input-md small-tbox ronly tbox-r DpmRate' readonly='' value='<?php echo $ItemRate; ?>' /> </td>
															<td><input name='ChiDpmAddPerc<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmPerc' readonly='' data-rbn='<?php echo $GrParRbn; ?>' value='<?php echo $CurrPPayList->percent; ?>'></td>
															<td><input name='ChiDpmAddAmount<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' type='text' class='form-control input-md small-tbox tbox-r ronly DpmAmt' readonly='' value='<?php echo $CurrPPayAmt; ?>'></td>
															<td>
																<i class='fa faOk DpmDelRow DpmDelRow<?php echo $GrParRbn; ?>' data-rbn='<?php echo $GrParRbn; ?>'>&#xf058;</i>
																<input type='hidden' name='ChiParDpmMbId<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' value='<?php echo $GPMBId; ?>'>
																<input type='hidden' name='ChiParDpmMbIdList<?php echo $GrParRbn; ?>PID<?php echo $GPMBId; ?>[]' value='<?php echo $CurrParIdArr[$RPMBId][$CurrPPayList->percent]; ?>'>
															</td>
														</tr>
														<?php
															} 
														...
														foreach($ONLYChildIDArr as $OCKey => $OCValue){
															...
															?>
															<tr id='DpmAddR<?php echo $DpmPaidSlno; ?>' class="DpmAddR<?php echo $DpmPaidSlno; ?> DPMROW<?php echo $GrParRbn; ?> DpmPId<?php echo $OCValue; ?> <?php echo $ActiveRABClass; ?>">
																<td><input type="text" name='ParDpmRAB<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-c ronly" readonly="" value="<?php echo $GrParRbn; ?>"/></td>
																<td><input type="text" name='ParDpmAddQty<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly ResCls DpmQty DpmParQty DpmQty<?php echo $GrParRbn; ?>" data-parid="<?php echo $OCValue; ?>" readonly="" data-rbn="<?php echo $GrParRbn; ?>" value="<?php echo $QtyArr[$OCValue]; ?>"/></td>
																<td><input type="text" name='ParDpmAddRate<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmRate" readonly="" value="<?php echo $ItemRate; ?>"/></td>
																<td><input type="text" name='ParDpmAddPerc<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ResCls DpmPerc <?php echo $RonlyClass; ?>" readonly='' data-rbn="<?php echo $GrParRbn; ?>" value="<?php echo $PercArr[$OCValue]; ?>"/></td>
																<td>
																	<input type="text" name='ParDpmAddAmount<?php echo $GrParRbn; ?>[]' class="form-control small-tbox tbox-r ronly DpmAmt" readonly="" value="<?php //echo $CurrPPayAmt;//"0.00";//$DpmPaidList->measurementbookid; ?>"/>
																	<input type="hidden" name="ParDpmMbId<?php echo $GrParRbn; ?>[]" value="<?php echo $OCValue; ?>">
																	<input type="hidden" name="ParDpmMbIdList<?php echo $GrParRbn; ?>[]">
																	<input type="hidden" class="DpmAddRow" data-mbid="<?php echo $OCValue; ?>">
																</td>
															</tr>
															<?php
															$CurrPPayCount2 = 0;
															$SelectCurrPPayQuery2 = "select * from pp_qty_splt where sheetid = '$sheetid' and subdivid = '$subdivid' and gr_par_id = '$DistGrParId' and gpmbid = '$OCValue' and rbn = '$rbn' order by ppid asc";
															$SelectCurrPPaySql2 = mysql_query($SelectCurrPPayQuery2);
															if($SelectCurrPPaySql2 == true){
																if(mysql_num_rows($SelectCurrPPaySql2)>0){
																	$CurrPPayCount2 = 1;
																}
															}
															if($CurrPPayCount2 == 1){ 
															...
															<tr class='DpmAddR<?php echo $GrParRbn; ?>'>
																<td><input type='text' name='ChiDpmRAB<?php echo $GrParRbn; ?>' class='form-control small-tbox ronly tbox-c' readonly='' value='<?php echo $GrParRbn; ?>'/></td>
																<td><input name='ChiDpmAddQty<?php echo $GrParRbn; ?>PID<?php echo $OCValue; ?>[]' type='text' class='form-control input-md small-tbox tbox-rb <?php echo $ActCellClass; ?> ResCls DpmQty DpmChiQty<?php echo $GPMBId2; ?> DpmQty<?php echo $GrParRbn; ?>' data-rbn='<?php echo $GrParRbn; ?>' value='<?php echo $CurrPPayList2->qty; ?>' onKeyPress="return isDecimalValue(event,this);"/></td>
																...
																<td><input name='ChiDpmAddPerc<?php echo $GrParRbn; ?>PID<?php echo $OCValue; ?>[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmPerc' readonly='' data-rbn='<?php echo $GrParRbn; ?>' value='<?php echo $CurrPPayList2->percent; ?>'></td>
																...
																<td>
																	...
																	<input type='hidden' name='ChiParDpmMbIdList<?php echo $GrParRbn; ?>PID<?php echo $OCValue; ?>[]' value='<?php echo $CurrParIdArr[$RPMBId2][$CurrPPayList2->percent]; ?>'>
																</td>
															</tr>
															<?php	
																}
	...
	<script>
        ...
        <div class="fixed-bottom" style="text-align:center">
			...
			<div class="col-md-5"><textarea name="txt_remarks" id="txt_remarks" class="form-control" style="height:37px; border-color:#2593FB" placeholder="Enter your remarks here"></textarea></div>
			<div class="col-md-3"><button type="button" name="PayableAmt" id="btn_info" class="btn btn-success col-md-12" disabled="disabled" style="opacity:1; text-align:left">Total Amount : &nbsp; &#8377 <span class="badge" style="color:#000000; font-size:13px;" id="TotalPayableAmt"> <?php echo number_format(round(($SLMTotalPayableAmt + $DPMTotalPayableAmt),2), 2, '.', ','); ?> </span> </button></div>
		   	...
		   	<!--<div class="col-md-1"><input type="submit" name="btn_cancel" id="btn_cancel" value=" Cancel " class="btn btn-warning col-md-12"></div>-->
		   	<div class="col-md-1"><input type="submit" name="btn_back" id="btn_back" value=" Back " class="btn btn-danger col-md-12"></div>
...
var myMessages = ['info']; // define the messages types    	 
	...
	$(document).ready(function(){
		...
		$('body').on('change','.SlmQty',function(){
			$(".SlmQty").css("background","white");
			$(".SlmQty").css("color","#0234B7");
			$(this).CheckQtyExceeded(event);
			$(".loaddiv").hide();
...
<script>
	$(document).ready(function() {
		...
		$(".fixAll").change(function(event){ 
			...
			if(input > 100){
				$(this).val('0.00')
				BootstrapDialog.alert("Percentage should be less than or equal to 100");
				event.preventDefault();
				event.returnValue = false;
				//exit();
			}else{
				if(PType == 'S'){
					var AlrdyPaidPercS = Number($(this).attr("data-total_paid"));//alert();
					var BalanceToBePaidS = 100 - AlrdyPaidPercS;
					if(BalanceToBePaidS >= input){
						$(this).val(input.toFixed(2))
					}else{
						$(this).val('0.00')
						BootstrapDialog.alert("Percentage should be less than or equal to "+BalanceToBePaidS);
						event.preventDefault();
						event.returnValue = false;
					}
				}else{
			...
			$(".FixCurrPerc"+rbn).each(function(){ 
				var AlrdyPaidPercALL = Number($(this).attr("data-total_paid"));
				var BalanceToBePaidALL = 100 - AlrdyPaidPercALL;
				
				if(BalanceToBePaidALL >= input){
					if(PType == 'A'){
			...
			if(CurrRbn == rbn){ 
				...
				PrevParIdSpit.forEach(function(ParIdVal, ParIdKey) { 
					var sed = 0; var RowArr = []; var FirstVal = 1; var PercCheckArr = [];
					...
					$('#BalTable'+rbn).DataTable().column( 3 ).data().each( function ( value, index ) { 
						...
						if(Perc == ""){ Perc = 0; }else{ Perc = Number(Perc); }//alert(Perc);
						if(value == ""){ value = 0; }else{ value = Number(value); }
						//alert(CurrParIdVal+" = "+PrevParId);
						...
						if(CurrParIdVal == PrevParId){
							...
							if(Perc > 0){
								PercCheckArr[CurrParIdVal] = 1;
								//alert(Perc); alert(PercCheckArr.join("*"));
							}
						}
						sed++; FirstVal = Perc; //alert(RowArr.join("*"));
					});
					//alert(PercCheckArr.join("*"));
					//alert(CurrParIdVal);
					//alert(RowArr.join("*"));
					...
					if(RowAction == 1){
						...
						RowArr.forEach(function(elementQty, keyPercent) {  //alert("S");
							//if(keyPercent != 0){
								if(ItemType == 'st'){ 
									elementQty = (elementQty / 1000);
									elementQty = elementQty.toFixed(Decimal);
								...
								var SelectedMBDid = MBDetIdStrArr[keyPercent]; //alert(SelectedMBDid);
								//alert(CurrParIdVal+" = "+PercCheckArr[CurrParIdVal]);
								var SlmAmt = Number(elementQty)*Number(Rate)*Number(keyPercent)/100;  
								...
								if(PercCheckArr[CurrParIdVal] != undefined){
								$('.DpmPId'+CurrParIdVal).closest( "tr" ).after("<tr class='LDPMROW"+rbn+"'><td><input type='text' name='ChiDpmRAB"+rbn+"[]' class='form-control small-tbox ronly tbox-c' readonly='' value='"+rbn+"'/></td><td><input name='ChiDpmAddQty"+rbn+"PID"+CurrParIdVal+"[]' type='text' class='form-control input-md small-tbox tbox-rb ResCls DpmQty DpmChiQty"+CurrParIdVal+" DpmQty"+rbn+"' data-rbn='"+rbn+"' value='"+elementQty+"' onKeyPress='return isDecimalValue(event,this)'/></td><td><input name='ChiDpmAddRate"+rbn+"PID"+CurrParIdVal+"[]' type='text' class='form-control input-md small-tbox ronly tbox-r DpmRate' readonly='' value='"+Rate+"' /> </td><td><input name='ChiDpmAddPerc"+rbn+"PID"+CurrParIdVal+"[]' type='text' class='form-control input-md small-tbox tbox-r ResCls DpmPerc' readonly='' data-rbn='"+rbn+"' value='"+keyPercent+"'></td><td><input name='ChiDpmAddAmount"+rbn+"PID"+CurrParIdVal+"[]' type='text' class='form-control input-md small-tbox tbox-r ronly DpmAmt' readonly='' value='"+SlmAmt+"'></td><td><i class='fa faOk DpmDelRow DpmDelRow"+rbn+"' data-rbn='"+rbn+"'>&#xf058;</i><input type='hidden' name='ChiParDpmMbId"+rbn+"PID"+CurrParIdVal+"[]' value='"+ParMbId+"'><input type='hidden' name='ChiParDpmMbIdList"+rbn+"PID"+CurrParIdVal+"[]' value='"+SelectedMBDid+"'></td></tr>");
								}
							//}
		...
		$('.BalanceBtn').click(function(event) {
			$(this).CheckQtyExceeded(event);
			//$(this).CheckDpmQtyExceeded(event);
		});
		$("#btn_save").click(function(event){
			$(this).CheckQtyExceeded(event);
			//$(this).CheckDpmQtyExceeded(event);
		});
		$.fn.CheckQtyExceeded = function(event) {
			$(".loaddiv").hide();
			/// Since Last Part Validation
			var ParQty = $(".ParSlmAddQty").val(); 
			if((ParQty == '')||(ParQty == undefined)){
				ParQty = 0;
			}//alert(ParQty);
			var Decimal = Number($("#txt_decimal").val());
			var TotalChildQty = 0; var Error = 0;
			$(".SlmQty").each(function(){
				var ChildQty = $(this).val();
				TotalChildQty = Number(TotalChildQty) + Number(ChildQty);
			});
			if(TotalChildQty > 0){
				TotalChildQty = TotalChildQty.toFixed(Decimal);
			}
			if(Number(ParQty) != Number(TotalChildQty)){
				BootstrapDialog.alert("Since last Part rate released Qty should be equal to Abstract Qty.");
				$(".SlmQty").css("background","red");
				$(".SlmQty").css("color","white");
				Error++;
			}
			//alert(Decimal);
			/// Deduct previous Part Validation
			var ResRbn = $("#txt_result_2").val();
			$(".DpmQty"+ResRbn).each(function(){
				var DpmParQty = $(this).val();
				var DpmParId = $(this).attr("data-parid");
				var DpmTotalChiQty = 0; var DpmChiExe = 0;
				$(".DpmChiQty"+DpmParId).each(function(){
					var DpmChiQty = $(this).val();
					DpmTotalChiQty = Number(DpmTotalChiQty) + Number(DpmChiQty);
					DpmChiExe++;
				});
				if(DpmChiExe > 0){ 
					DpmTotalChiQty = DpmTotalChiQty.toFixed(Decimal);
					//alert(DpmParQty); alert(DpmTotalChiQty); 
					if(Number(DpmParQty) != Number(DpmTotalChiQty)){
						BootstrapDialog.alert("Deduct previous child Qty should be equal to Parent Qty.");
						$(".DpmChiQty"+DpmParId).css("background","red");
						$(".DpmChiQty"+DpmParId).css("color","white");
						Error++;
					}
				}
			});
		...
		/*$.fn.CheckDpmQtyExceeded = function(event) {
			$(".loaddiv").hide();
			$(".DpmParQty").each(function(){
				var DpmParQty = $(this).val();
				var DpmParId = $(this).attr("data-parid");
				var DpmTotalChiQty = 0; var DpmChiExe = 0;
				$(".DpmChiQty"+DpmParId).each(function(){
				...
				if(DpmChiExe > 0){
					if(DpmParQty != DpmTotalChiQty){
						BootstrapDialog.alert("Deduct previous child Qty should be equal to Parent Qty.");
						$(".DpmChiQty"+ParId).css("background","red");
						$(".DpmChiQty"+ParId).css("color","white");
						event.preventDefault();
						event.returnValue = false;
					}
				}
