...
foreach($PPayItemIdArr as $PPayArrKey => $PPayArrValue){
	$PPayItemId 	= $PPayArrValue; $ExCnt = 1; $MasterArr = array(); $ParentArr = array(); $ChildArr = array();
	...
	$GRParIdArr = array(); $GRParRowSpanArr = array();
	...
	$IsparentFirst = 0;   $TempArrPerc = array();
	foreach($GRParIdArr as $GRParIdKey=>$GRParIdValue){
		...
		/// GET ALLA THE LIST PART PAYMENT QTY DETAILS OF RESPECTIVE PARENT ID
		$IsChildFirst = 0;  $TempQty1 = 0;
		...
		if($SelectDetailsSql == true){
			if(mysql_num_rows($SelectDetailsSql) > 0){
				while($ChildList = mysql_fetch_object($SelectDetailsSql)){
					...
					$ChildPerc = $ChildList->percent+$MasterPerc;
					//if($MasterQty > $ChildQty){
						$TempQty1 = $TempQty1 + $ChildQty;
						if($TempArrPerc[$GRParIdValue][$ChildPerc] == ''){
							$TempArrPerc[$GRParIdValue][$ChildPerc] = $ChildQty;
							$MasterRowSpanCount++;
							$ParentRowSpanCount++;
						}else{
							$TempArrPerc[$GRParIdValue][$ChildPerc] = $TempArrPerc[$GRParIdValue][$ChildPerc]+ $ChildQty;
						}
						$ChildArr[$GRParIdValue][] = $ChildQty;
						$ChildArr[$GRParIdValue][] = $ChildPerc;
						$IsChildFirst++;  
					//}else{
						/// ONLY INCREASE THE KEY VALUE
					//}
				}
			}
		}
		$TempQty2 = 0;
		if($MasterQty > $TempQty1){
			$TempQty2 = $MasterQty - $TempQty1;
			
		}else{
			$TempQty2 = $MasterQty;
		}
		if($TempArrPerc[$GRParIdValue][$MasterPerc] == ''){
			$TempArrPerc[$GRParIdValue][$MasterPerc] = $TempQty2;
			$MasterRowSpanCount++;
			$ParentRowSpanCount++;
		}else{
			$TempArrPerc[$GRParIdValue][$MasterPerc] = $TempArrPerc[$GRParIdValue][$MasterPerc]+ $TempQty2;
		}
		$IsparentFirst++; 
		$GRParRowSpanArr[$GRParIdValue] = $ParentRowSpanCount;
		//print_r($TempArrPerc);echo "<br/>";
	}
	$Temp1 = 0; 
	
	/// FOR DISPLAYING OUTPUT OF RESPECTIVE ITEM
	foreach($ParentArr as $ParentArrkey => $ParentArrValue){
		...
		$ChildArrCount  = count($TempArrPerc[$LoopParentId]);//count($ChildArr[$LoopParentId]);
		//for($x1=0; $x1<$ChildArrCount; $x1=$x1+2){
		//print_r($TempArrPerc);echo "<br/>";
		//exit;
		foreach($TempArrPerc[$LoopParentId] as $TempArrPercKey => $TempArrPercValue){
			$LoopChildQty 	= $TempArrPercValue;//$ChildArr[$LoopParentId][$x1+0];
			$LoopChildPerc 	= $TempArrPercKey;//$ChildArr[$LoopParentId][$x1+1];
			$LoopChildAmt 	= round(($LoopChildQty * $SOQRate * $LoopChildPerc/100),2);
			/// FOR DISPLAYING MASTER DATA
			if($Temp1 == 0){
				$TDStr1 = "<td align='center' rowspan='".$MasterRowSpanCount."'>".$SOQItemNo."</td><td align='right' rowspan='".$MasterRowSpanCount."'>".$SOQRate."</td><td rowspan='".$MasterRowSpanCount."'></td><td rowspan='".$MasterRowSpanCount."'></td>";
			}else{
				$TDStr1 = "";
			}
			...
			$TDStr3 = "<td align='right'>".$LoopChildQty."</td><td align='right'>".$LoopChildPerc."</td><td>".$LoopParentId."</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>";
	...
	/*foreach($ParentArr as $ParentArrkey => $ParentArrValue){
		...
		for($x1=0; $x1<$ChildArrCount; $x1=$x1+2){
			...
			/// FOR DISPLAYING CHILD DATA
			$TDStr3 = "<td align='right'>".$LoopChildQty."</td><td align='right'>".$LoopChildPerc."</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>";
			echo "<tr>".$TDStr1.$TDStr2.$TDStr3."</tr>";
			$Temp1++; $Temp2++;	
		}
