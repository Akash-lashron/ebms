...
function GetPrevPaymentDetails($sheetid,$subdivid,$rate){
	...
	foreach($MasMBIdPArr4 as $key1 => $value1){
		...
		$CurrPayDetailStr .= '<tr><td width="60px" align="right">'.$Qty1.'</td><td width="60px" align="right">'.$Perc1.'</td><td width="60px" align="right">'.$Amount1.'</td></tr>';
		$BalancePerc = 100 - $Perc1;
		$BalanceAmount1 =  round(($Qty1 * $rate * $BalancePerc / 100),2);
		if($BalancePerc > 0){
			$BalPayDetailStr .= '<tr><td width="60px" align="right">'.$BalancePerc.'</td><td width="60px" align="right">'.$BalanceAmount1.'</td></tr>';
			$Line++;
		}
...
$PPayItemIdArr = array();
$SelectPartpaymentQuery = "select distinct subdivid from pp_qty_splt where sheetid = '$sheetid' and percent != '0' order by subdivid asc limit 1";
...
foreach($PPayItemIdArr as $PPayArrKey => $PPayArrValue){
	$PPayItemId 	= $PPayArrValue; $ExCnt = 1; $MasterArr = array(); $ParentArr = array(); $ChildArr = array(); $ChildArrSlm = array();
	$MasterRowSpanCount = 0;
	$ParentRowSpanCount = 0;
	...
	foreach($GRParIdArr as $GRParIdKey=>$GRParIdValue){
		...
		if($SelectMasterSql == true){
			if(mysql_num_rows($SelectMasterSql)>0){
				...
				$ParentArr[$GRParIdValue][1] = $MasterPerc;
				
				$ChildArr[$GRParIdValue][] = $MasterQty;
				$ChildArr[$GRParIdValue][] = $MasterPerc;
				$ChildArr[$GRParIdValue][] = $MasterRbn;
				$MasterRowSpanCount++;
				$ParentRowSpanCount++;
			}
		}
		// GET THE MASTER DATA OF PARENT ID FOR CURRENT RAB
		/*if($MasterExist == 0){
			$SelectMasterQuery = "select * from measurementbook_temp where measurementbookid = '$GRParIdValue'";
			$SelectMasterSql = mysql_query($SelectMasterQuery);
			if($SelectMasterSql == true){
				if(mysql_num_rows($SelectMasterSql)>0){
					$MasterExist = 1;
		...
		/// GET ALL THE LIST PART PAYMENT QTY DETAILS OF RESPECTIVE PARENT ID
		$IsChildFirst = 0; $TempArr = array(); $DpmRowSpanCnt = 0; $SlmRowSpanCnt = 0;
		$SelectDetailsQuery = "select * from pp_qty_splt where gr_par_id = '$GRParIdValue' and percent != '0'";// and rbn != '$rbn'";
		$SelectDetailsSql = mysql_query($SelectDetailsQuery);
		if($SelectDetailsSql == true){
			if(mysql_num_rows($SelectDetailsSql) > 0){
				while($ChildList = mysql_fetch_object($SelectDetailsSql)){
				
					$ChildQty = $ChildList->qty;
					$ChildPerc = $ChildList->percent;
					$ChildRbn = $ChildList->rbn;
					if($ChildRbn == $rbn){
						$ChildArrSlm[$GRParIdValue][] = $ChildQty;
						$ChildArrSlm[$GRParIdValue][] = $ChildPerc;
						$ChildArrSlm[$GRParIdValue][] = $ChildRbn;
						/*$IsChildFirst++;  
						$MasterRowSpanCount++;
						$ParentRowSpanCount++;*/
						$SlmRowSpanCnt++;
					}else{
						$ChildArr[$GRParIdValue][] = $ChildQty;
						$ChildArr[$GRParIdValue][] = $ChildPerc;
						$ChildArr[$GRParIdValue][] = $ChildRbn;
						/*$IsChildFirst++;  
						$MasterRowSpanCount++;
						$ParentRowSpanCount++;*/
						$DpmRowSpanCnt++;
				...
				if($DpmRowSpanCnt >= $SlmRowSpanCnt){
					$MasterRowSpanCount = $MasterRowSpanCount + $DpmRowSpanCnt;
					$ParentRowSpanCount = $ParentRowSpanCount + $DpmRowSpanCnt;
				}else{
					$MasterRowSpanCount = $MasterRowSpanCount + $SlmRowSpanCnt;
					$ParentRowSpanCount = $ParentRowSpanCount + $SlmRowSpanCnt;
				}
			}
		}
		$IsparentFirst++; 
		$GRParRowSpanArr[$GRParIdValue] = $ParentRowSpanCount;
		$DPMRowSpanArr[$GRParIdValue] = $DpmRowSpanCnt + 1; /// +1 is for PARENT ROW DETAILS DISPLAY WHICH IS FROM MEASUREMENTBOOK TABLE
		$SLMRowSpanArr[$GRParIdValue] = $SlmRowSpanCnt + 1; /// +1 is for PARENT ROW DETAILS DISPLAY WHICH IS FROM MEASUREMENTBOOK TABLE
	}
	$Temp1 = 0; 
	print_r($GRParRowSpanArr);
	/// FOR DISPLAYING OUTPUT OF RESPECTIVE ITEM IN DPM (DEDUCT PREVIOUS PAID) ROW SPAN IS GREATER THAN SLM(SINCE LAST PAID)
	
	foreach($ParentArr as $ParentArrkey => $ParentArrValue){
		$SlmRowSpan = $SLMRowSpanArr[$ParentArrkey];
		$DpmRowSpan = $DPMRowSpanArr[$ParentArrkey];
		if($DpmRowSpan >= $SlmRowSpan){
			$Temp2 = 0;
			$LoopParentId 	= $ParentArrkey;
			$LoopParentQty 	= $ParentArrValue[0];
			$LoopParentPerc = $ParentArrValue[1];
			$ChildArrCount  = count($ChildArr[$LoopParentId]);
			for($x1=0; $x1<$ChildArrCount; $x1=$x1+3){
				$LoopChildQty 	= $ChildArr[$LoopParentId][$x1+0];
				$LoopChildPerc 	= $ChildArr[$LoopParentId][$x1+1];
				$LoopChildRbn 	= $ChildArr[$LoopParentId][$x1+2];
				$LoopChildAmt 	= round(($LoopChildQty*$SOQRate*$LoopChildPerc/100),2);
				/// FOR DISPLAYING MASTER DATA
				if($Temp1 == 0){
					$TDStr1 = "<td align='center' rowspan='".$MasterRowSpanCount."'>".$SOQItemNo."-".$MasterRowSpanCount."</td><td align='right' rowspan='".$MasterRowSpanCount."'>".$SOQRate."</td><td rowspan='".$MasterRowSpanCount."'></td><td rowspan='".$MasterRowSpanCount."'></td>";
				}else{
					$TDStr1 = "";
				}
				/// FOR DISPLAYING PARENT DATA
				if($Temp2 == 0){
					$TDStr2 = "<td align='right' rowspan='".$GRParRowSpanArr[$LoopParentId]."'>".$LoopParentQty."</td>";
				}else{
					$TDStr2 = "";
				}
				/// FOR DISPLAYING CHILD DATA
				$TDStr3 = "<td align='right'>".$LoopChildQty."-".$LoopParentId."</td><td align='right'>".$LoopChildPerc."</td><td align='right'>".$LoopChildAmt."</td><td align='center'>".$LoopChildRbn."</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>";
				echo "<tr>".$TDStr1.$TDStr2.$TDStr3."</tr>";
				$Temp1++; $Temp2++;	
			}
		}
	}
							...
							/*$PPayItemIdArr = array();
							$SelectPartpaymentQuery = "select distinct subdivid from pp_qty_splt where sheetid = '$sheetid' and rbn = '$rbn'";
							$SelectPartpaymentSql = mysql_query($SelectPartpaymentQuery);
							if($SelectPartpaymentSql == true){
								if(mysql_num_rows($SelectPartpaymentSql)>0){
									while($PPayItemIdList = mysql_fetch_object($SelectPartpaymentSql)){
							...
							foreach($PPayItemIdArr as $PPayArrKey => $PPayArrValue){
								if($Line>20){
									echo "<tr><td colspan='7' style='border-bottom:1px solid white;border-left:1px solid white;border-right:1px solid white;' class='labelprint' align='center'>Page ".$Page."</td></tr>";
									echo "</table>";
									$Page++;
								...
								if($SelectMasterSql == true){
									if(mysql_num_rows($SelectMasterSql)>0){ 
										...
										$ExpMasPPayFlag = explode("*",$MasPPayFlag);
										$PartPayParId   = $ExpMasPPayFlag[2];
										$PartPayParRAB 	= $ExpMasPPayFlag[1];
										$PPayRefRABArr[$PartPayParId] = $PartPayParRAB;
									}}
								}
								...
								if($SelectDetailSql == true){
									if(mysql_num_rows($SelectDetailSql)>0){ 
										...
										while($DetailList = mysql_fetch_object($SelectDetailSql)){
											...
											$Masgr_par_id 	= $DetailList->gr_par_id;
											$PartPayParRAB = $PPayRefRABArr[$Masgr_par_id];
											
											if($MasPercent != 0){
												if($MasRbn == $rbn){
													$CurrPayDetailStr .= '<tr><td width="60px" align="center">'.$PartPayParRAB.'</td><td width="60px" align="right">'.$MasQty.'</td><td width="60px" align="right">'.$MasPercent.'</td><td width="60px" align="right">'.$MasAmount.'</td></tr>';
													$LineTemp1++;
												}else{
													$PrevPayDetailStr .= '<tr><td width="60px" align="right">'.$MasQty.'</td><td width="60px" align="right">'.$MasPercent.'</td><td width="60px" align="right">'.$MasAmount.'</td></tr>';
													$LineTemp2++;
												}
											}
