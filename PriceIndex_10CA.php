...
if (isset($_POST["submit"])) 
{
	...
	if($select_esc_sql == true)
	{
		if(mysql_num_rows($select_esc_sql)>0)
		{
			...
			$tca_fromdate  	= $EscIdList->tca_fromdate ;
			$tca_todate  	= $EscIdList->tca_todate ;
			if(($tca_fromdate != $pi_from_period) || ($tca_todate != $pi_to_period))
			{
				$update_esc_query = "update escalation set tca_fromdate = '$pi_from_period', tca_todate = '$pi_to_period', modifieddate = NOW() where sheetid = '$sheetid' and rbn = '$rbn' and quarter = '$quarter'";
	...
	if($temp == 0)
	{
		$insert_est_query 	= "insert into escalation set sheetid = '$sheetid', quarter = '$quarter', rbn='$rbn', tca_fromdate = '$pi_from_period', tca_todate = '$pi_to_period', modifieddate = NOW(), staffid = '$staffid', active = 1, flag = 0";
		$insert_est_sql 	= mysql_query($insert_est_query);
		$esc_id 			= mysql_insert_id();
	}
	$delete_pi_query = "delete pi, pidt from price_index pi JOIN price_index_detail pidt ON pi.pid = pidt.pid where pi.sheetid = '$sheetid' and pi.esc_rbn='$rbn' and pi.quarter='$quarter' and pi.esc_id='$esc_id' and pi.type = 'TCA'";
 ...
 <script>
	...
	var prev_edit_row 	= 0;
     ...
     { 
            ...
            var data;
            var i, j;
			document.form.txt_no_of_month.value = "";
...
//alert(data);
                ...
                else
                {
                   	...
                   	for(i = 0; i < name.length; i+=8)
                   	{
						...
						if(base_index_code == "CIo"){
							var avg_price_index_code = "CI";
						}else if(base_index_code == "CsIo"){
							var avg_price_index_code = "CsI";
						}else if(base_index_code == "SIo"){
							var avg_price_index_code = "SI";
						}else if(base_index_code == "SsIo"){
							var avg_price_index_code = "SsI";
						}
						...
						var txt_box4 = document.createElement("input");
							...
							txt_box4.id = "txt_base_price_code"+bid;
						...
						var txt_box5 = document.createElement("input");
							...
							txt_box5.id = "txt_base_price_rate"+bid;
						...
						{	
							...
							var txt_box6 = document.createElement("input");
								...
								txt_box6.value = "";
								txt_box6.setAttribute('class', "extraItemTextbox "+avg_price_index_code); 
						...
						var txt_box9 = document.createElement("input");
							...
							txt_box9.value = avg_price_index_code;
							txt_box9.setAttribute('class', "extraItemTextboxDisable"); 
		...
		/*function check_price_index(month1,month3)
		{
            ...
            strURL = "find_check_priceindex.php?sheetid=" + document.form.cmb_shortname.value+"&month1="+month1+"&month3="+month3+"&type=TCA";
            xmlHttp.open('POST', strURL, true);
            xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xmlHttp.onreadystatechange = function ()
            {
                if (xmlHttp.readyState == 4)
                {
                    ...
                    if (data == "")
                    {
						swal("", "No Records Found in check", "");
                    }
					else if(data == 1)
					{
     ...
     { 
            ...
            strURL = "find_priceindex.php?sheetid=" + document.form.cmb_shortname.value+"&month1="+month1+"&month3="+month3+"&type=TCA";
            xmlHttp.open('POST', strURL, true);
            xmlHttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xmlHttp.onreadystatechange = function ()
            {
	 ...
	 function SetMonthField()
	 {
		...
		var cell5	=	row1.insertCell(4);
			cell5.innerHTML = " Price Index Code ";
			cell5.rowSpan = 2;
		...
		index++;
		   $('.date-picker').datepicker({
				...
				onClose: function(dateText, inst) { 
					...
					var Sheetid = document.getElementById("cmb_shortname").value;
					var Rbn 	= document.getElementById("txt_rbn").value;
					var Quarter = document.getElementById("txt_quarter").value;
					var MonthNo = document.getElementById("txt_no_of_month").value;
					var MonthYearArr = [];
					$("input[name='txt_month[]']").each(function() {
						var MonthYear = $(this).val();
						MonthYearArr.push(MonthYear);
					});
					var MonthYearArrStr = JSON.stringify(MonthYearArr);
					
					$.ajax({ 
						type: 'POST', 
						url: 'FindEscPriceIndexData.php', 
						data: { Sheetid: Sheetid, Rbn: Rbn, Quarter: Quarter, MonthNo: MonthNo, Type:'TCA', MonthYearArrStr: MonthYearArrStr }, 
						dataType: 'json',
						success: function (data) { 
							if(data != null){
								$("input[name='txt_avg_price_index_code[]']").each(function() {
									var PiCode = $(this).val(); var x = 0;
									$("input[name='txt_month[]']").each(function() {
										var PiMonthYear = $(this).val();
										var PiMonthCode = PiMonthYear+PiCode;
										var PiValue = data[PiMonthCode];
										$("."+PiCode).eq(x).val(PiValue);
										x++;
									});
								});
