...
//alert(data);
                ...
                else
                {
                   	...
                   	for(i = 0; i < name.length; i+=8)
                   	{
						...
						if(base_index_code == "LIo"){
							var avg_price_index_code = "LI";
						}if(base_index_code == "MIo"){
							var avg_price_index_code = "MI";
						}
		   ...
		   $('.date-picker').datepicker({
				...
				onClose: function(dateText, inst) { 
					...
					$.ajax({ 
						...
						data: { Sheetid: Sheetid, Rbn: Rbn, Quarter: Quarter, MonthNo: MonthNo, Type:'TCC', MonthYearArrStr: MonthYearArrStr  }, 
						dataType: 'json',
						success: function (data) { 
							if(data != null){
								var y = 0;
								$("input[name='txt_avg_price_index_code[]']").each(function() {
									var PiCode = $(this).val(); var x = 0; var Total = 0; 
									$("input[name='txt_month[]']").each(function() {
										var PiMonthYear = $(this).val();
										var PiMonthCode = PiMonthYear+PiCode;
										var PiValue = data[PiMonthCode];
										$("."+PiCode).eq(x).val(PiValue);
										Total = Number(Total)+Number(PiValue);
										x++; 
									});
									var Avg = Number(Total) / Number(x);
									Avg = Avg.toFixed(2);
									$("input[name='txt_avg_price_index_rate[]']").eq(y).val(Avg);
									y++;
								});
