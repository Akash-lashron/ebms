...
$schdulesql ="SELECT DISTINCT sno,sch_id, subdiv_id, sheet_id, shortnotes, description, total_quantity, rate, per, total_amt, subdiv_id, page_no, measure_type FROM schdule where sheet_id= '$sheetid' AND  sno != '0' ";
...
$rebate_percent = mysql_result($rebate_perc_query,0,'rebate_percent');
 ...
 {
     header('Location: ShortNotesGenerate.php');
 }
 
 
 $success = 0;
 if(isset($_POST['save']) == 'Save'){
 	$SchidList = $_POST['txt_schid'];
	for($i=0; $i<count($SchidList); $i++){
		$schid 	= $SchidList[$i];
		$snotes = $_POST['txt_snotes_'.$schid];
		$UpdateQuery = "update schdule set shortnotes = '$snotes' where sch_id = '$schid'";
            ...
            <div class="content">  
                ...
                <div class="container_12">  
                    <div class="grid_12" align="center"> 
                        <blockquote class="bq1" id="bq1" style="overflow:auto; position:relative">
                            <div class="container">
                                <?php 
							...
							<table width="90%" class="table1 table2">
								<tr class="heading" style="background:#0063C6">
								...
								{ 
                                  ...
                                  ?>
									...
									<?php if($List->total_quantity > 0){ ?>
									<tr>
										<td colspan="6">
										<textarea name="txt_snotes_<?php echo $List->sch_id; ?>" class="textboxdisplay" style="width:100%; border-color:#51A8FF"><?php echo $List->shortnotes; ?></textarea>
										<input type="hidden" name="txt_schid[]" value="<?php echo $List->sch_id; ?>">
										</td>
									</tr>
									<?php } ?>
