...
*/
$selectmbook_detail = " select DISTINCT fromdate, todate, abstmbookno FROM measurementbook WHERE sheetid = '$sheetid' AND flag = '2' AND rbn = '$rbn'";
...
$selectmbookno_sql = "select mbno, startpage, endpage, mbookorder from mymbook WHERE mtype = 'S' AND sheetid = '$sheetid' AND staffid = '$staffid' AND rbn = '$rbn' AND genlevel = 'composite'";
...
/*$selectmbookno = "select mbname, old_id from oldmbook WHERE mbook_type = 'S' AND sheetid = '$sheetid'";
...
if(mysql_num_rows($selectmbookno_sql)>0)
{
	...
	$selectnewmbookno = "select DISTINCT mbno from mbookgenerate WHERE sheetid = '$sheetid' AND flag = '2' AND mbno != '$mbookno'";
...
{
	$selectmbookno = "select DISTINCT mbno from mbookgenerate WHERE sheetid = '$sheetid' AND flag = '2'";
	$selectmbookno_sql = mysql_query($selectmbookno);
...
function display_carry($sumst,$mbookno,$mpage,$newmbookno,$decimal)
{
	...
	$row_co = "<tr height='' class='labelbold'>";
	$row_co = $row_co."<td width='' colspan='6' class='labelcenter' style='text-align:right'>"."C/o to Page ".($page+1)."/ Steel MB No. ".$mbookno."</td>";
...
$wodataquery = "SELECT sheet_id, sheet_name, work_order_no, work_name, computer_code_no, tech_sanction, name_contractor, agree_no, rbn FROM sheet WHERE sheet_id = '$sheetid' ";
...
<?php
			...
			$table = $table . "<td class='labelbold labelheadblue' width = '50px'>CC No.</td>";
			$table = $table . "<td class='label' width = '150px'>". $ccno . "</td>";
            $table = $table . "</tr>";
            $table = $table . "</table>";
			
            //$table1 = $table1 . "<table width='1087px' border='0' bgcolor='#BDBDBD' cellpadding='2' cellspacing='2' align='center'>";
            $table1 = $table1 . "<tr height='' class='labelbold' style='' bgcolor='#e5e3e3'>";
            $table1 = $table1 . "<td width='8%' rowspan='2' class='labelcenter headingfont' style=''>Date of Measure<br/>ment</td>";
            $table1 = $table1 . "<td width='4%' rowspan='2' class='labelcenter headingfont' style=''>Item No.</td>";
            $table1 = $table1 . "<td width='15%' rowspan='2' class='labelcenter headingfont' style=''>Description of work</td>";
            $table1 = $table1 . "<td width='3%' rowspan='2' class='labelcenter headingfont' style=''>Dia of Rod<font style='font-size: 9px;'> [mm]</font></td>";
            $table1 = $table1 . "<td width='3%' rowspan='2' class='labelcenter headingfont' style=''>Nos</td>";
            $table1 = $table1 . "<td width='4%' rowspan='2' class='labelcenter headingfont' style='word-wrap:break-all;'><font style='font-size: 10px;'>Length</font><br/>[m]</td>";
            $table1 = $table1 . "<td colspan='9' width='54%' class='labelcenter headingfont' style=''>Total Length in Meters</td>";
            ...
            $table1 = $table1 . "</tr>";
            $table1 = $table1 . "<tr height='' class='labelbold' bgcolor='#e5e3e3'>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>8</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>10</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>12</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>16</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>20</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>25</td>";
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>28</td>";            
            $table1 = $table1 . "<td width='7%' class='labelcenter headingfont' style=''>32</td>"; 
			$table1 = $table1 . "<td width='6%' class='labelcenter headingfont' style=''>36</td>";  
            $table1 = $table1 . "</tr>";
           // $table1 = $table1 . "</table>";
            ?>
...
<?php		
		...
		if($DiaSum_16_item != ""){ $total_weight_16 = round(($DiaSum_16_item * 1.580),$pre_decimal); }
		if($DiaSum_20_item != ""){ $total_weight_20 = round(($DiaSum_20_item * 2.470),$pre_decimal); }
		if($DiaSum_25_item != ""){ $total_weight_25 = round(($DiaSum_25_item * 3.860),$pre_decimal); }
		if($DiaSum_28_item != ""){ $total_weight_28 = round(($DiaSum_28_item * 4.830),$pre_decimal); }
		if($DiaSum_32_item != ""){ $total_weight_32 = round(($DiaSum_32_item * 6.313),$pre_decimal); }
		if($DiaSum_36_item != ""){ $total_weight_36 = round(($DiaSum_36_item * 8.000),$pre_decimal); }
...
?>
		   ...
		   <tr height='' class="label">
			...
			<tr height='' class="label">
				...
				<td width='7%' align="right">1.580</td>
				<td width='7%' align="right">2.470</td>
				<td width='7%' align="right">3.860</td>
				<td width='7%' align="right">4.830</td>
				<td width='7%' align="right">6.313</td>
				<td width='6%' align="right">8.000</td>
			...
			<tr height='' bgcolor="" class="label labelbold">
				...
				<td width='' colspan="6" align="right"><?php echo getabstractpage($sheetid,$pre_subdivid); ?></td>
                <td width='' align="left"></td>
         	</tr>
...
<?php
		...
		if($DiaSum_16 != ""){ $total_weight_16 = round(($DiaSum_16 * 1.580),$decimal); }
		if($DiaSum_20 != ""){ $total_weight_20 = round(($DiaSum_20 * 2.470),$decimal); }
		if($DiaSum_25 != ""){ $total_weight_25 = round(($DiaSum_25 * 3.860),$decimal); }
		if($DiaSum_28 != ""){ $total_weight_28 = round(($DiaSum_28 * 4.830),$decimal); }
		if($DiaSum_32 != ""){ $total_weight_32 = round(($DiaSum_32 * 6.313),$decimal); }
		if($DiaSum_36 != ""){ $total_weight_36 = round(($DiaSum_36 * 8.000),$decimal); }
...
<?php
		...
		if($DiaSum_16_item != ""){ $total_weight_16 = round(($DiaSum_16_item * 1.580),$pre_decimal); }else { $total_weight_16 = ""; }
		if($DiaSum_20_item != ""){ $total_weight_20 = round(($DiaSum_20_item * 2.470),$pre_decimal); }else { $total_weight_20 = ""; }
		if($DiaSum_25_item != ""){ $total_weight_25 = round(($DiaSum_25_item * 3.860),$pre_decimal); }else { $total_weight_25 = ""; }
		if($DiaSum_28_item != ""){ $total_weight_28 = round(($DiaSum_28_item * 4.830),$pre_decimal); }else { $total_weight_28 = ""; }
		if($DiaSum_32_item != ""){ $total_weight_32 = round(($DiaSum_32_item * 6.313),$pre_decimal); }else { $total_weight_32 = ""; }
		if($DiaSum_36_item != ""){ $total_weight_36 = round(($DiaSum_36_item * 8.000),$pre_decimal); }else { $total_weight_36 = ""; }
		$totalweight_KGS 	= 	round(($total_weight_8+$total_weight_10+$total_weight_12+$total_weight_16+$total_weight_20+$total_weight_25+$total_weight_28+$total_weight_32+$total_weight_36),$pre_decimal);
		$totalweight_MT 	= 	round(($totalweight_KGS/1000),$pre_decimal);
if($currentline>25)
{
...
<?php
	...
	for($x1=0; $x1<count($ExplodeResult); $x1++)
	{
		...
		$insertMbgenerate_sql 	= 	"insert into mbookgenerate (mbgeneratedate, staffid, sheetid, divid, subdivid, fromdate, todate, mbno, mbpage, mbtotal, pay_percent, flag, rbn, active, userid) 
														values (NOW(), '$staffid', '$sheetid', '$divid', '$subdivid', '$fromdate', '$todate', '$mbookno', '$mbookpage', '$ItemQty', '0', '2', '$rbn', '1', '$userid')";
...
<table align="center" style="border:none;" class="printbutton">
                <tr style="border:none">
                   <td align="center" colspan="15" style="border:none;"><br/><input type="button" name="Back" value="Back" id="back" class="backbutton" onclick="goBack();" /> </td>
                </tr>
            </table>	
 </form>
</body>
