<?
	require_once("ajax_table.class.php");
	$obj = new ajax_table();
	if(isset($_POST) && count($_POST)){
		
		// whats the action ??
		$temp = $_POST['temp'];
		$pcode = $_POST['pcode'];
		$menutypeid = $_POST['menutypeid'];
		$menutype = $_POST['menutype'];
		$menucode = $_POST['menucode'];
		$action = $_POST['action'];
		unset($_POST['action']);

		if($action == "save")
		{		
			// remove 'action' key from array, we no longer need it

			// Never ever believe on end user, he could be a evil minded
			$escapedPost = array_map('mysql_real_escape_string', $_POST);
			$escapedPost = array_map('htmlentities', $escapedPost);
			$res = $obj->save($escapedPost,$temp,$menutype,$pcode);
			
			if($res)
			{
				$escapedPost["success"] = "1";
				$escapedPost["id"] = $res;
				echo json_encode($escapedPost);
			}
			else
				echo $obj->error("save");
		}
		else if($action == "del")
		{
			$id = $_POST['rid'];
			$res = $obj->delete_record($id,$temp);
			if($res)
				echo json_encode(array("success" => "1","id" => $id));	
			else
				echo $obj->error("delete");
		}
		else if($action == "update")
		{
			
			$escapedPost = array_map('mysql_real_escape_string', $_POST);
			$escapedPost = array_map('htmlentities', $escapedPost);

			$id = $obj->update_record($escapedPost,$temp);
			if($id)
				echo json_encode(array_merge(array("success" => "1","id" => $id),$escapedPost));	
			else
				echo $obj->error("update");
		}
		else if($action == "get")
		{
			
			$escapedPost = array_map('mysql_real_escape_string', $_POST);
			$escapedPost = array_map('htmlentities', $escapedPost);

			$id = $obj->getRecords("X",$menutypeid);
			if($id)
				echo json_encode($id);
				//echo json_encode(array("success" => "1","id" => $id));
			else
				echo $obj->error("get");
		}
		else if($action == "getsingle")
		{
			
			$escapedPost = array_map('mysql_real_escape_string', $_POST);
			$escapedPost = array_map('htmlentities', $escapedPost);

			$id = $obj->getRecords("Y",$menucode);
			if($id)
				echo json_encode($id);
				//echo json_encode(array("success" => "1","id" => $id));
			else
				echo $obj->error("getsingle");
		}
		else if($action == "updatetd")
		{
			
			$escapedPost = array_map('mysql_real_escape_string', $_POST);
			$escapedPost = array_map('htmlentities', $escapedPost);

			$id = $obj->update_column($escapedPost,$temp);
			if($id)
				echo json_encode(array_merge(array("success" => "1","id" => $id),$escapedPost));	
			else
				echo $obj->error("updatetd");
		}
	}
?>