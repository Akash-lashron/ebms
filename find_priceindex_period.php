...
$select_period_query = "select * from escalation where sheetid = '$sheetid' and quarter = '$quarter' and rbn = '$rbn'";
$select_period_sql = mysql_query($select_period_query);
if($select_period_sql == true)
{
	if(mysql_num_rows($select_period_sql)>0)
	{
		$PList = mysql_fetch_object($select_period_sql);
		$esc_id 		= $PList->esc_id;
		$fromdate1 		= date_create($PList->tcc_fromdate); // Default function in php
		$todate1		= date_create($PList->tcc_todate);	 // Default function in php
		$fromdate2 		= date_create($PList->tca_fromdate); // Default function in php
		$todate2		= date_create($PList->tca_todate);	 // Default function in php
		
		$tcc_fromdate 	= date_format($fromdate1,'d-m-Y');   // Default function in php
		$tcc_todate 	= date_format($todate1,'d-m-Y');	 // Default function in php
		$tca_fromdate 	= date_format($fromdate2,'d-m-Y');   // Default function in php
		$tca_todate 	= date_format($todate2,'d-m-Y');	 // Default function in php
		$period_str = $esc_id."*@*".$tcc_fromdate."*@*".$tcc_todate."*@*".$tca_fromdate."*@*".$tca_todate;
	}
	else
	{
		$period_str = "";
