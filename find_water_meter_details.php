...
$select_recovery_query 	= 	"SELECT * FROM water_recovery WHERE sheetid = '$sheetid' AND meter_no = '$meterno'";
...
if($select_recovery_sql == true) 
{
	...
	$e_limit 		= 	$List->w_limit;
	$factor 		= 	$List->factor;
	$er_date 		= 	dt_display($List->er_date);
	$recoverydata 	= $meter_no."*".$imr."*".$imr_date."*".$rate."*".$meter_rent."*".$e_limit."*".$er_date."*".$factor; 
}

$select_recovery_query_2 	= 	"SELECT * FROM generate_waterbill WHERE sheetid = '$sheetid' AND meter_no = '$meterno'";
